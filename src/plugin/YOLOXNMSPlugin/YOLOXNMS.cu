...
void YOLOXDetectionInference(cudaStream_t stream, const int N,
                             const int C1,  // C1
                             const int C2,  // 4 + 1 + num_cls
                             const int numClasses, const int keep_topK, const int topK, bool use_p6,
    ...
    if (use_p6) {
        ...
        parseYOLOXBBoxes<<<CUDA_GET_BLOCKS(nbThreads), CUDA_NUM_THREADS, 0, stream>>>(
            nbThreads, numClasses, C1, C2, conf_thresh, layer_sizes, strides, inputData, (float *)bboxData,
            (float *)confData);
    } else {
    ...
    status = gatherTopDetections(stream, true, false, N, C1, numClasses, topK, keep_topK, DataType::kFLOAT,
                                 DataType::kFLOAT, indices, scores, bboxData, nullptr, topDetections);

    assert(status == STATUS_SUCCESS);
}

int YOLOXDetectionOutput::enqueue(int batchSize, void const *const *inputs, void *const *outputs, void *workspace,
