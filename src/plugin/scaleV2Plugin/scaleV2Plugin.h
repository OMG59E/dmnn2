...
public:
    ScaleV2(int axis, int num_axes, int bias_term) : axis_{axis}, num_axes_{num_axes}, bias_term_{bias_term} {}
    ...
    size_t getSerializationSize() const noexcept override {
        return sizeof(int32_t) * 6 + sizeof(DataType);
    }
    ...
    const char* getPluginType() const noexcept override {
        return SCALEV2_PLUGIN_NAME;
    }
    const char* getPluginVersion() const noexcept override {
        return SCALEV2_PLUGIN_VERSION;
    }
