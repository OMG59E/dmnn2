...
__global__ void ScaleForward(const int n, const Dtype* in, const Dtype* scale, const int scale_dim, const int inner_dim,
                             Dtype* out) {
...
int ScaleV2::enqueue(int32_t batchSize, void const* const* inputs, void* const* outputs, void* workspace,
    ...
    if (data_type_ == DataType::kFLOAT) {
        ...
        ScaleForward<<<CUDA_GET_BLOCKS(count), CUDA_NUM_THREADS, 0, stream>>>(count, bottom_data, scale_data, scale_dim,
                                                                              inner_dim, top_data);
