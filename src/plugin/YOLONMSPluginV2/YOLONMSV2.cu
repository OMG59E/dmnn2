...
__global__ void parseBBoxes(const int nbThreads, const int numClasses, const int numPts, const int C2,
                            const float conf_thresh, const float *inputData, float *bboxData, float *landData,
                            float *confData) {
...
void YOLODetectionInference(cudaStream_t stream, const int N,
                            const int C1,  // C1
                            const int C2,  // 4 + 1 + num_cls
                            const int numClasses, const int numPts, const int keep_topK, const int topK,
                            const float nms_thresh, const float conf_thresh, const float *inputData, void *workspace,
                            float *topDetections) {
    ...
    parseBBoxes<<<CUDA_GET_BLOCKS(nbThreads), CUDA_NUM_THREADS, 0, stream>>>(nbThreads, numClasses, numPts, C2,
                                                                             conf_thresh, inputData, (float *)bboxData,
                                                                             (float *)landData, (float *)confData);
    ...
    pluginStatus_t status =
        permuteData(stream, num_scores, numClasses, C1, 1, DataType::kFLOAT, false, confData, scores);
    ...
    status = sortScoresPerClass(stream, N, numClasses, C1,
                                -1,  // no background_id
                                conf_thresh, DataType::kFLOAT, scores, indices, sortingWorkspace);
    assert(status == STATUS_SUCCESS);

    // NMS
    status = allClassNMS(stream, N, numClasses, C1, topK, nms_thresh, true, false, DataType::kFLOAT, DataType::kFLOAT,
                         bboxData, scores, indices, postNMSScores, postNMSIndices, false);
    assert(status == STATUS_SUCCESS);
    ...
    status = gatherTopDetectionsV2(stream, true, N, C1, numClasses, numPts, topK, keep_topK, DataType::kFLOAT,
                                   DataType::kFLOAT, indices, scores, bboxData, landData, nullptr, topDetections);
    assert(status == STATUS_SUCCESS);
}
...
int YOLODetectionOutputV2::enqueue(int batchSize, void const *const *inputs, void *const *outputs, void *workspace,
    ...
    YOLODetectionInference(stream, batchSize, C1, C2, param.num_classes, param.num_pts, param.keep_topK, param.topK,
