...
__global__ void yolo_box_kernel(const int nbThreads, const float *anchors, const int num_anchors, const int stride,
                                const int N, const int C, const int H, const int W, const int layer_h,
...
pluginStatus_t YOLOBoxInference(cudaStream_t stream, const void *anchors, const int num_anchors, const int stride,
                                const int N, const int C, const int H, const int W, const void *inputData,
                                void *outputData) {
    int layer_h = int(sqrt(H));
    const int nbThreads = N * C * H;  // 3 * 6400 3 * 1600 3 * 400
    yolo_box_kernel<<<CUDA_GET_BLOCKS(nbThreads), CUDA_NUM_THREADS>>>(nbThreads, (const float *)anchors, num_anchors,
                                                                      stride, N, C, H, W, layer_h,
                                                                      (const float *)inputData, (float *)outputData);
    CUDACHECK(cudaGetLastError());
    return STATUS_SUCCESS;
}

int YOLOBox::enqueue(int32_t batchSize, void const *const *inputs, void *const *outputs, void *workspace,
                     cudaStream_t stream) noexcept {
    void *outputData = outputs[0];
    const auto *inputData = reinterpret_cast<const float *>(inputs[0]);
    assert(mAnchors_.count == 6);
    pluginStatus_t status = YOLOBoxInference(stream, mAnchors_.values, mAnchors_.count, mParam_.stride, batchSize, C_,
                                             H_, W_, inputData, outputData);
