...
pluginStatus_t detectionInference(cudaStream_t stream, const int N, const int C1, const int C2,
                                  ...
                                  const int topK, const int keepTopK, const float confidenceThreshold,
                                  const float nmsThreshold, const CodeTypeSSD codeType, const DataType DT_BBOX,
                                  const void* locData, const void* priorData, const DataType DT_SCORE,
    ...
    status =
        gatherTopDetections(stream, shareLocation, true, N, numPredsPerClass, numClasses, topK, keepTopK,
                            DataType::kFLOAT, DataType::kFLOAT, indices, scores, bboxData, keepCount, topDetections);
    if (STATUS_SUCCESS != status) {
