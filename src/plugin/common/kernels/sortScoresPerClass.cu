 ...
 * @FilePath: /dmnn2/src/plugin/common/kernels/sortScoresPerClass.cu
 * @Description:
 *
...
pluginStatus_t sortScoresPerClass_gpu(cudaStream_t stream, const int num, const int num_classes,
                                      ...
                                      const float confidence_threshold, void* conf_scores_gpu, void* index_array_gpu,
                                      void* workspace) {
    ...
    prepareSortData<T_SCORE, BS><<<GS, BS, 0, stream>>>(num, num_classes, num_preds_per_class, background_label_id,
                                                        confidence_threshold, (T_SCORE*)conf_scores_gpu,
                                                        (T_SCORE*)temp_scores, (int*)temp_idx, (int*)d_offsets);
...
typedef pluginStatus_t (*sspcFunc)(cudaStream_t, const int, const int, const int, const int, const float, void*, void*,
                                   void*);
...
pluginStatus_t sortScoresPerClass(cudaStream_t stream, const int num, const int num_classes,
                                  const int num_preds_per_class, const int background_label_id,
                                  const float confidence_threshold, const DataType DT_SCORE, void* conf_scores_gpu,
                                  void* index_array_gpu, void* workspace) {
    sspcLaunchConfig lc = sspcLaunchConfig(DT_SCORE);
    for (unsigned i = 0; i < sspcFuncVec.size(); ++i) {
        if (lc == sspcFuncVec[i]) {
            return sspcFuncVec[i].function(stream, num, num_classes, num_preds_per_class, background_label_id,
