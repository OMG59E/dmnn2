...
pluginStatus_t allClassNMSV2(cudaStream_t stream, int num, int num_classes, int num_preds_per_class, int top_k,
                             ...
                             void *afterLKNMS_index_array, bool flipXY = false);
...
pluginStatus_t detectionInference(cudaStream_t stream, int N, int C1, int C2, bool shareLocation,
                                  ...
                                  int numClasses, int topK, int keepTopK, float confidenceThreshold, float nmsThreshold,
                                  CodeTypeSSD codeType, DataType DT_BBOX, const void *locData, const void *priorData,
                                  DataType DT_SCORE, const void *confData, void *keepCount, void *topDetections,
                                  void *workspace, bool isNormalized = true, bool confSigmoid = false);

pluginStatus_t detectionInferenceV2(cudaStream_t stream, int N, int C1, int C2, int C3, bool shareLocation,
                                    bool varianceEncodedInTarget, int backgroundLabelId, int numPredsPerClass,
                                    int numClasses, int numPts, int topK, int keepTopK, float confidenceThreshold,
                                    const float nmsThreshold, CodeTypeSSD codeType, const DataType DT_BBOX,
                                    const void *locData, const void *priorData, const void *ptData, DataType DT_SCORE,
                                    const void *confData, void *keepCount, void *topDetections, void *workspace,
                                    bool isNormalized = true, bool confSigmoid = false);

pluginStatus_t gatherTopDetections(cudaStream_t stream, bool shareLocation, bool useClip, int numImages,
                                   int numPredsPerClass, int numClasses, int topK, int keepTopK, DataType DT_BBOX,
...
pluginStatus_t decodeBBoxes(cudaStream_t stream, int nthreads, CodeTypeSSD code_type, bool variance_encoded_in_target,
                            ...
                            bool clip_bbox, DataType DT_BBOX, const void *loc_data, const void *prior_data,
                            void *bbox_data);
