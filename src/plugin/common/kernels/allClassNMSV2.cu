 ...
 * @LastEditTime: 2024-10-12 10:19:21
 * @FilePath: /dmnn2/src/plugin/common/kernels/allClassNMSV2.cu
 * @Description:
 *
...
__global__ void allClassNMSV2_kernel(const int num, const int num_classes, const int num_preds_per_class,
                                     ...
                                     T_SCORE *beforeNMS_scores, int *beforeNMS_index_array, T_SCORE *afterNMS_scores,
                                     int *afterNMS_index_array, T_BBOX *pts_data, T_SCORE *beforePtsNMS_scores,
                                     int *beforePtsNMS_index_array, T_SCORE *afterPtsNMS_scores,
                                     int *afterPtsNMS_index_array, bool flipXY = false) {
    ...
    for (int i = 0; i < num; i++) {
        ...
        // const int pts_idx_offset = share_location ? (i * num_preds_per_class) : (i * num_classes *
        // num_preds_per_class);
...
#pragma unroll
        for (int t = 0; t < TSIZE; t++) {
            ...
            if (item_idx < max_idx) {
                ...
                // ptsIndex[t] = beforePtsNMS_index_array[item_idx];

                if (loc_bboxIndex[t] != -1) {
                    const int bbox_data_idx =
                        share_location ? (loc_bboxIndex[t] % num_preds_per_class + bbox_idx_offset) : loc_bboxIndex[t];
                    ...
                    //                    pts[t].l1_x = flipXY ? pts_data[pts_data_idx * 8 + 1]
                    //                                         : pts_data[pts_data_idx * 8 + 0];
                    //                    pts[t].l1_y = flipXY ? pts_data[pts_data_idx * 8 + 0]
                    //                                         : pts_data[pts_data_idx * 8 + 1];
                    //                    pts[t].l2_x = flipXY ? pts_data[pts_data_idx * 8 + 3]
                    //                                         : pts_data[pts_data_idx * 8 + 2];
                    //                    pts[t].l2_y = flipXY ? pts_data[pts_data_idx * 8 + 2]
                    //                                         : pts_data[pts_data_idx * 8 + 3];
                    //                    pts[t].l3_x = flipXY ? pts_data[pts_data_idx * 8 + 5]
                    //                                         : pts_data[pts_data_idx * 8 + 4];
                    //                    pts[t].l3_y = flipXY ? pts_data[pts_data_idx * 8 + 4]
                    //                                         : pts_data[pts_data_idx * 8 + 5];
                    //                    pts[t].l4_x = flipXY ? pts_data[pts_data_idx * 8 + 7]
                    //                                         : pts_data[pts_data_idx * 8 + 6];
                    //                    pts[t].l4_y = flipXY ? pts_data[pts_data_idx * 8 + 6]
                    //                                         : pts_data[pts_data_idx * 8 + 7];
        ...
        // int ref_pts_idx = share_location ? (beforePtsNMS_index_array[ref_item_idx] % num_preds_per_class +
        // pts_idx_offset) : beforePtsNMS_index_array[ref_item_idx];
        ...
        while ((ref_bbox_idx != -1) && ref_item_idx < max_idx) {
            ...
            //            ref_pts.l1_x = flipXY ? pts_data[ref_pts_idx * 8 + 1]
            //                                  : pts_data[ref_pts_idx * 8 + 0];
            //            ref_pts.l1_y = flipXY ? pts_data[ref_pts_idx * 8 + 0]
            //                                  : pts_data[ref_pts_idx * 8 + 1];
            //            ref_pts.l2_x = flipXY ? pts_data[ref_pts_idx * 8 + 3]
            //                                  : pts_data[ref_pts_idx * 8 + 2];
            //            ref_pts.l2_y = flipXY ? pts_data[ref_pts_idx * 8 + 2]
            //                                  : pts_data[ref_pts_idx * 8 + 3];
            //            ref_pts.l3_x = flipXY ? pts_data[ref_pts_idx * 8 + 5]
            //                                  : pts_data[ref_pts_idx * 8 + 4];
            //            ref_pts.l3_y = flipXY ? pts_data[ref_pts_idx * 8 + 4]
            //                                  : pts_data[ref_pts_idx * 8 + 5];
            //            ref_pts.l4_x = flipXY ? pts_data[ref_pts_idx * 8 + 7]
            //                                  : pts_data[ref_pts_idx * 8 + 6];
            //            ref_pts.l4_y = flipXY ? pts_data[ref_pts_idx * 8 + 6]
            //                                  : pts_data[ref_pts_idx * 8 + 7];
            ...
            // ref_pts_idx = share_location ? (beforePtsNMS_index_array[ref_item_idx] % num_preds_per_class +
            // pts_idx_offset)
            //        : beforePtsNMS_index_array[ref_item_idx];
        }
        ...
        for (int t = 0; t < TSIZE; t++) {
            ...
            if (read_item_idx < max_idx) {
                ...
                // afterPtsNMS_scores[write_item_idx] = kept_bboxinfo_flag[cur_idx] ? beforePtsNMS_scores[read_item_idx]
                // : 0.0f; afterPtsNMS_index_array[write_item_idx] = kept_bboxinfo_flag[cur_idx] ? ptsIndex[t] : -1;
            }
        }
    }
}
...
pluginStatus_t allClassNMSV2_gpu(cudaStream_t stream, const int num, const int num_classes,
                                 ...
                                 void *afterNMS_index_array, void *landmark_data, void *beforeLKNMS_scores,
                                 void *beforeLKNMS_index_array, void *afterLKNMS_scores, void *afterLKNMS_index_array,
                                 bool flipXY = false) {
#define P(tsize) allClassNMSV2_kernel<T_SCORE, T_BBOX, (tsize)>

    void (*kernel[8])(const int, const int, const int, const int, const float, const bool, const bool, float *,
                      T_SCORE *, int *, T_SCORE *, int *, float *, T_SCORE *, int *, T_SCORE *, int *, bool) = {
    ...
    kernel[t_size - 1]<<<GS, BS, BS * t_size * sizeof(bool), stream>>>(
        num, num_classes, num_preds_per_class, top_k, nms_threshold, share_location, isNormalized, (T_BBOX *)bbox_data,
        (T_SCORE *)beforeNMS_scores, (int *)beforeNMS_index_array, (T_SCORE *)afterNMS_scores,
        (int *)afterNMS_index_array, (T_BBOX *)landmark_data, (T_SCORE *)beforeLKNMS_scores,
        (int *)beforeLKNMS_index_array, (T_SCORE *)afterLKNMS_scores, (int *)afterLKNMS_index_array, flipXY);

    CUDACHECK(cudaGetLastError());
    return STATUS_SUCCESS;
}

// allClassNMSV2 LAUNCH CONFIG
typedef pluginStatus_t (*nmsLKFunc)(cudaStream_t, const int, const int, const int, const int, const float, const bool,
                                    const bool, void *, void *, void *, void *, void *, void *, void *, void *, void *,
                                    void *, bool);

struct nmsV2LaunchConfigSSD {
    DataType t_score;
    DataType t_bbox;
    nmsLKFunc function;

    nmsV2LaunchConfigSSD(DataType t_score, DataType t_bbox) : t_score(t_score), t_bbox(t_bbox) {}
    nmsV2LaunchConfigSSD(DataType t_score, DataType t_bbox, nmsLKFunc function)
        : t_score(t_score), t_bbox(t_bbox), function(function) {}
    bool operator==(const nmsV2LaunchConfigSSD &other) {
        return t_score == other.t_score && t_bbox == other.t_bbox;
    }
};

static std::vector<nmsV2LaunchConfigSSD> nmsV2FuncVec;

bool nmsV2Init() {
    nmsV2FuncVec.push_back(nmsV2LaunchConfigSSD(DataType::kFLOAT, DataType::kFLOAT, allClassNMSV2_gpu<float, float>));
    return true;
}

static bool initialized = nmsV2Init();
...
pluginStatus_t allClassNMSV2(cudaStream_t stream, int num, int num_classes, int num_preds_per_class, int top_k,
                             ...
                             DataType DT_BBOX, void *bbox_data, void *beforeNMS_scores, void *beforeNMS_index_array,
                             void *afterNMS_scores, void *afterNMS_index_array, void *pts_data,
                             void *beforeLKNMS_scores, void *beforeLKNMS_index_array, void *afterLKNMS_scores,
                             void *afterLKNMS_index_array, bool flipXY) {
    nmsV2LaunchConfigSSD lc = nmsV2LaunchConfigSSD(DT_SCORE, DT_BBOX, allClassNMSV2_gpu<float, float>);
    for (unsigned i = 0; i < nmsV2FuncVec.size(); ++i) {
        if (lc == nmsV2FuncVec[i]) {
            return nmsV2FuncVec[i].function(
                stream, num, num_classes, num_preds_per_class, top_k, nms_threshold, share_location, isNormalized,
                bbox_data, beforeNMS_scores, beforeNMS_index_array, afterNMS_scores, afterNMS_index_array, pts_data,
                beforeLKNMS_scores, beforeLKNMS_index_array, afterLKNMS_scores, afterLKNMS_index_array, flipXY);
        }
    }
