...
__launch_bounds__(nthds_per_cta) __global__
    void gatherTopDetectionsV2_kernel(const bool shareLocation, const int numImages, const int numPredsPerClass,
                                      const int numClasses, const int numPts, const int topK, const int keepTopK,
                                      const int *indices, const T_SCORE *scores, const T_BBOX *bboxData,
                                      const T_BBOX *ptsData, int *keepCount, T_BBOX *topDetections) {
    if (keepTopK > topK)
        return;
    for (int i = blockIdx.x * nthds_per_cta + threadIdx.x; i < numImages * keepTopK; i += gridDim.x * nthds_per_cta) {
         ...
         * These data will only show up at the end of keepTopK bounding boxes since the bounding boxes were sorted
         * previously. It is also not going to affect the count of valid bounding boxes (keepCount). These data will
         * probably never be used (because we have keepCount).
         */
        ...
        if (index == -1) {
            ...
            for (int k = 0; k < step; k++) {
                if (k == 0) {
                    ...
                    topDetections[i * step + k] =
                        ptsData[ptsId + k - 7];  // max(min(ptsData[ptsId + k - 7], T_BBOX(1.)), T_BBOX(0.));
                }
            }
...
pluginStatus_t gatherTopDetectionsV2_gpu(cudaStream_t stream, const bool shareLocation, const int numImages,
                                         const int numPredsPerClass, const int numClasses, const int numPts,
                                         const int topK, const int keepTopK, const void *indices, const void *scores,
                                         const void *bboxData, const void *ptsData, void *keepCount,
                                         void *topLKDetections) {
    // cudaMemsetAsync(keepCount, 0, numImages * sizeof(int), stream);
    const int BS = 32;
    const int GS = 32;
    gatherTopDetectionsV2_kernel<T_BBOX, T_SCORE, BS><<<GS, BS, 0, stream>>>(
        shareLocation, numImages, numPredsPerClass, numClasses, numPts, topK, keepTopK, (int *)indices,
        (T_SCORE *)scores, (T_BBOX *)bboxData, (T_BBOX *)ptsData, (int *)keepCount, (T_BBOX *)topLKDetections);
...
typedef pluginStatus_t (*gtdLKFunc)(cudaStream_t, const bool, const int, const int, const int, const int, const int,
                                    const int, const void *, const void *, const void *, const void *, void *, void *);
...
struct gtdV2LaunchConfig {
    ...
    gtdV2LaunchConfig(DataType t_bbox, DataType t_score) : t_bbox(t_bbox), t_score(t_score) {}
...
bool gtdV2Init() {
    gtdV2FuncVec.push_back(
        gtdV2LaunchConfig(DataType::kFLOAT, DataType::kFLOAT, gatherTopDetectionsV2_gpu<float, float>));
    return true;
}
...
pluginStatus_t gatherTopDetectionsV2(cudaStream_t stream, bool shareLocation, int numImages, int numPredsPerClass,
                                     ...
                                     DataType DT_SCORE, const void *indices, const void *scores, const void *bboxData,
                                     const void *ptsData, void *keepCount, void *topLKDetections) {
    gtdV2LaunchConfig lc = gtdV2LaunchConfig(DT_BBOX, DT_SCORE);
    for (unsigned i = 0; i < gtdV2FuncVec.size(); ++i) {
        if (lc == gtdV2FuncVec[i]) {
            return gtdV2FuncVec[i].function(stream, shareLocation, numImages, numPredsPerClass, numClasses, numPts,
                                            topK, keepTopK, indices, scores, bboxData, ptsData, keepCount,
                                            topLKDetections);
        }
    }
