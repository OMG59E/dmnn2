...
__launch_bounds__(nthds_per_cta) __global__
    void gatherTopDetections_kernel(const bool shareLocation, const bool useClip, const int numImages,
                                    const int numPredsPerClass, const int numClasses, const int topK,
                                    const int keepTopK, const int *indices, const T_SCORE *scores,
                                    const T_BBOX *bboxData, int *keepCount, T_BBOX *topDetections) {
    if (keepTopK > topK)
        return;
...
pluginStatus_t gatherTopDetections_gpu(cudaStream_t stream, const bool shareLocation, const bool useClip,
                                       const int numImages, const int numPredsPerClass, const int numClasses,
                                       const int topK, const int keepTopK, const void *indices, const void *scores,
                                       const void *bboxData, void *keepCount, void *topDetections) {
    // cudaMemsetAsync(keepCount, 0, numImages * sizeof(int), stream);
    const int BS = 32;
    const int GS = 32;
    gatherTopDetections_kernel<T_BBOX, T_SCORE, BS><<<GS, BS, 0, stream>>>(
        shareLocation, useClip, numImages, numPredsPerClass, numClasses, topK, keepTopK, (int *)indices,
        (T_SCORE *)scores, (T_BBOX *)bboxData, (int *)keepCount, (T_BBOX *)topDetections);
...
typedef pluginStatus_t (*gtdFunc)(cudaStream_t, const bool, const bool, const int, const int, const int, const int,
                                  const int, const void *, const void *, const void *, void *, void *);
...
struct gtdLaunchConfig {
    ...
    gtdLaunchConfig(DataType t_bbox, DataType t_score) : t_bbox(t_bbox), t_score(t_score) {}
...
bool gtdInit() {
    gtdFuncVec.push_back(gtdLaunchConfig(DataType::kFLOAT, DataType::kFLOAT, gatherTopDetections_gpu<float, float>));
    return true;
}

static bool initialized = gtdInit();

pluginStatus_t gatherTopDetections(cudaStream_t stream, const bool shareLocation, const bool useClip,
                                   const int numImages, const int numPredsPerClass, const int numClasses,
                                   const int topK, const int keepTopK, const DataType DT_BBOX, const DataType DT_SCORE,
                                   const void *indices, const void *scores, const void *bboxData, void *keepCount,
                                   void *topDetections) {
    gtdLaunchConfig lc = gtdLaunchConfig(DT_BBOX, DT_SCORE);
    for (unsigned i = 0; i < gtdFuncVec.size(); ++i) {
        if (lc == gtdFuncVec[i]) {
            return gtdFuncVec[i].function(stream, shareLocation, useClip, numImages, numPredsPerClass, numClasses, topK,
                                          keepTopK, indices, scores, bboxData, keepCount, topDetections);
        }
    }
