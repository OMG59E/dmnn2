 ...
 * @LastEditTime: 2024-10-12 10:14:56
 * @FilePath: /dmnn2/src/plugin/common/kernels/decodeBBoxes.cu
 * @Description:
 *
...
__launch_bounds__(nthds_per_cta) __global__
    void decodeBBoxes_kernel(const int nthreads, const CodeTypeSSD code_type, const bool variance_encoded_in_target,
                             const int num_priors, const bool share_location, const int num_loc_classes,
                             const int background_label_id, const bool clip_bbox, const T_BBOX* loc_data,
    ...
    for (int index = blockIdx.x * nthds_per_cta + threadIdx.x; index < nthreads; index += nthds_per_cta * gridDim.x) {
        ...
        // If bounding box was not shared among all the classes and the bounding box is corresponding to the background
        // class
        ...
        // Encoding method: CodeTypeSSD::CENTER_SIZE
            ...
            // Use [x_topleft, y_topleft, x_bottomright, y_bottomright] as coordinates for final decoded bounding box
            // output
            switch (i) {
                case 0:
...
pluginStatus_t decodeBBoxes_gpu(cudaStream_t stream, const int nthreads, const CodeTypeSSD code_type,
                                const bool variance_encoded_in_target, const int num_priors, const bool share_location,
                                const int num_loc_classes, const int background_label_id, const bool clip_bbox,
    ...
    decodeBBoxes_kernel<T_BBOX, BS><<<GS, BS, 0, stream>>>(
        nthreads, code_type, variance_encoded_in_target, num_priors, share_location, num_loc_classes,
        background_label_id, clip_bbox, (const T_BBOX*)loc_data, (const T_BBOX*)prior_data, (T_BBOX*)bbox_data);
    CUDACHECK(cudaGetLastError());
    return STATUS_SUCCESS;
}

// decodeBBoxes LAUNCH CONFIG
typedef pluginStatus_t (*dbbFunc)(cudaStream_t, const int, const CodeTypeSSD, const bool, const int, const bool,
                                  const int, const int, const bool, const void*, const void*, void*);

struct dbbLaunchConfig {
    DataType t_pts;
    dbbFunc function;

    dbbLaunchConfig(DataType t_bbox) : t_pts(t_bbox) {}
    dbbLaunchConfig(DataType t_bbox, dbbFunc function) : t_pts(t_bbox), function(function) {}
    bool operator==(const dbbLaunchConfig& other) {
        return t_pts == other.t_pts;
    }
};
...
bool decodeBBoxesInit() {
    dbbFuncVec.push_back(dbbLaunchConfig(DataType::kFLOAT, decodeBBoxes_gpu<float>));
    return true;
}

static bool initialized = decodeBBoxesInit();

pluginStatus_t decodeBBoxes(cudaStream_t stream, const int nthreads, const CodeTypeSSD code_type,
                            const bool variance_encoded_in_target, const int num_priors, const bool share_location,
                            const int num_loc_classes, const int background_label_id, const bool clip_bbox,
                            const DataType DT_BBOX, const void* loc_data, const void* prior_data, void* bbox_data) {
    ...
    for (unsigned i = 0; i < dbbFuncVec.size(); ++i) {
        if (lc == dbbFuncVec[i]) {
            return dbbFuncVec[i].function(stream, nthreads, code_type, variance_encoded_in_target, num_priors,
                                          share_location, num_loc_classes, background_label_id, clip_bbox, loc_data,
                                          prior_data, bbox_data);
        }
    }
