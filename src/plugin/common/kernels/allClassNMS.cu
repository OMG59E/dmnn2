...
#define P(tsize) allClassNMS_kernel<T_SCORE, T_BBOX, (tsize)>
    ...
    void (*kernel[8])(const int, const int, const int, const int, const float,
                      ...
                      int *, bool) = { P(1), P(2), P(3), P(4), P(5), P(6), P(7), P(8) };
...
struct nmsLaunchConfigSSD {
    ...
    nmsLaunchConfigSSD(DataType t_score, DataType t_bbox, nmsFunc function) : t_score(t_score), t_bbox(t_bbox), function(function) {}
    bool operator==(const nmsLaunchConfigSSD &other) { return t_score == other.t_score && t_bbox == other.t_bbox; }
};
