...
size_t detectionInferenceWorkspaceSize(int N, int C1, int C2, int numClasses, int topK, DataType DT_BBOX,
                                       DataType DT_SCORE) {
    size_t wss[7];
    ...
    wss[4] = detectionForwardPostNMSSize(N, numClasses, topK);
    wss[5] = detectionForwardPostNMSSize(N, numClasses, topK);
    wss[6] = std::max(sortScoresPerClassWorkspaceSize(N, numClasses, C1, DT_SCORE),
                      sortScoresPerImageWorkspaceSize(N, numClasses * topK, DT_SCORE));
    return calculateTotalWorkspaceSize(wss, 7);
}

size_t detectionInferenceWorkspaceSize(int N, int C1, int C2, int numClasses, int numPts, int topK, DataType DT_BBOX,
                                       DataType DT_SCORE) {
    size_t wss[8];
    wss[0] = detectionForwardBBoxDataSize(N, C1 * 4, DT_BBOX);
    ...
    wss[7] = std::max(sortScoresPerClassWorkspaceSize(N, numClasses, C1, DT_SCORE),
                      sortScoresPerImageWorkspaceSize(N, numClasses * topK, DT_SCORE));
    return calculateTotalWorkspaceSize(wss, 7);
}

size_t detectionInferenceWorkspaceSize(bool shareLocation, int N, int C1, int C2, int C3, int numClasses,
                                       int numPredsPerClass, int numPts, int topK, DataType DT_BBOX, DataType DT_SCORE,
                                       DataType DT_PTS) {
    size_t wss[8];
    wss[0] = detectionForwardBBoxDataSize(N, C1, DT_BBOX);
    wss[1] = detectionForwardBBoxPermuteSize(shareLocation, N, C1, DT_BBOX);
