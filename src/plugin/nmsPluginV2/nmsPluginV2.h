...
public:
    ...
    size_t getWorkspaceSize(int maxBatchSize) const noexcept override {
        return detectionInferenceWorkspaceSize(param.shareLocation, maxBatchSize, C1, C2, C3, param.numClasses,
                                               numPriors, param.numPts, param.topK, DataType::kFLOAT, DataType::kFLOAT,
                                               DataType::kFLOAT);
    }
    int enqueue(int batchSize, void const *const *inputs, void *const *outputs, void *workspace,
        ...
        pluginStatus_t status = detectionInferenceV2(
            stream, batchSize, C1, C2, C3, param.shareLocation, param.varianceEncodedInTarget, param.backgroundLabelId,
            numPriors, param.numClasses, param.numPts, param.topK, param.keepTopK, param.confidenceThreshold,
            param.nmsThreshold, param.codeType, DataType::kFLOAT, locData, priorData, landData, DataType::kFLOAT,
            confData, keepCount, topLKDetections, workspace, param.isNormalized, param.confSigmoid);
        assert(status == STATUS_SUCCESS);
        return 0;
    }
    size_t getSerializationSize() const noexcept override {
        return sizeof(DetectionOutputV2Parammeters) + sizeof(int) * 4;
    }
    ...
    const char *getPluginType() const noexcept override {
        return NMS_PLUGIN_V2_NAME;
    }
    const char *getPluginVersion() const noexcept override {
        return NMS_PLUGIN_V2_VERSION;
    }
