...
template <typename T>
__global__ void focus_kernel(const int nbThreads, const T* input, const int iChannels, const int iHeight,
                             const int iWidth, T* output, const int oChannels, const int oHeight, const int oWidth) {
    CUDA_KERNEL_LOOP(idx, nbThreads) {
        ...
        output[((dn * size) + (0 * oHeight * oWidth)) + (oc % 3 * oHeight * oWidth) + (oh * oWidth) + ow] =
            input[(dn * size) + (oc % 3 * iHeight * iWidth) + (2 * oh + 0) * iWidth + (2 * ow + 0)];
        output[((dn * size) + (3 * oHeight * oWidth)) + (oc % 3 * oHeight * oWidth) + (oh * oWidth) + ow] =
            input[(dn * size) + (oc % 3 * iHeight * iWidth) + (2 * oh + 1) * iWidth + (2 * ow + 0)];
        output[((dn * size) + (6 * oHeight * oWidth)) + (oc % 3 * oHeight * oWidth) + (oh * oWidth) + ow] =
            input[(dn * size) + (oc % 3 * iHeight * iWidth) + (2 * oh + 0) * iWidth + (2 * ow + 1)];
        output[((dn * size) + (9 * oHeight * oWidth)) + (oc % 3 * oHeight * oWidth) + (oh * oWidth) + ow] =
            input[(dn * size) + (oc % 3 * iHeight * iWidth) + (2 * oh + 1) * iWidth + (2 * ow + 1)];
    }
}
...
int Focus::enqueue(int32_t batchSize, void const* const* inputs, void* const* outputs, void* workspace,
    ...
    if (data_type_ == DataType::kFLOAT) {
        ...
        focus_kernel<<<CUDA_GET_BLOCKS(nbThreads), CUDA_NUM_THREADS, 0, stream>>>(
            nbThreads, bottom_data, iChannels, iHeight, iWidth, top_data, oChannels, oHeight, oWidth);
