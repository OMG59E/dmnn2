 ...
 * @LastEditTime: 2024-10-12 14:01:27
 * @FilePath: /dmnn2/src/plugin/slicePlugin/slicePlugin.cu
 * @Description:
 *
...
__global__ void SliceKernel(const int nthreads, const Dtype *in_data, const int num_slices, const int slice_size,
                            const int bottom_slice_axis, const int top_slice_axis, const int offset_slice_axis,
                            Dtype *out_data) {
...
void SliceForward(cudaStream_t stream, const Dtype *bottom_data, int num_slices, int slice_axis, int slice_size,
                  int bottom_slice_axis, const std::vector<nv::DimsCHW> &output_shapes, void *const *outputs) {
    ...
    for (int i = 0; i < output_shapes.size(); ++i) {
        ...
        SliceKernel<<<CUDA_GET_BLOCKS(nthreads), CUDA_NUM_THREADS, 0, stream>>>(
            nthreads, bottom_data, num_slices, slice_size, bottom_slice_axis, top_slice_axis, offset_slice_axis,
            top_data);
        offset_slice_axis += top_slice_axis;
    }
}

int Slice::enqueue(int32_t batchSize, void const *const *inputs, void *const *outputs, void *workspace,
    ...
    assert(top_shapes_.size() == num_output_);

    if (data_type_ == DataType::kFLOAT) {
        auto bottom_data = reinterpret_cast<const float *>(inputs[0]);
        SliceForward(stream, bottom_data, num_slices, axis_, slice_size, bottom_slice_axis, top_shapes_, outputs);
