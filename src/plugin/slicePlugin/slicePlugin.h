...
public:
    Slice(int axis, int slice_dim, std::vector<int> slice_points, DataType data_type, int channel_in, int height_in,
          int width_in)
    ...
    size_t getSerializationSize() const noexcept override {
        return sizeof(int) * (3 + num_output_ - 1) + sizeof(nv::DimsCHW) + sizeof(DataType);
    }
    ...
    IPluginV2IOExt *clone() const noexcept override {
        auto *plugin = new Slice(axis_, slice_dim_, slice_points_, data_type_, bottom_shape_.c(), bottom_shape_.h(),
                                 bottom_shape_.w());
