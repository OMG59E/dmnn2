...
struct __align__(4 * sizeof(T)) BoxCorner {
    ...
    __device__ BoxCorner<T> clip(T low, T high) const {
        return {
            lt_mp(y1, low) ? low : (gt_mp(y1, high) ? high : y1), lt_mp(x1, low) ? low : (gt_mp(x1, high) ? high : x1),
            lt_mp(y2, low) ? low : (gt_mp(y2, high) ? high : y2), lt_mp(x2, low) ? low : (gt_mp(x2, high) ? high : x2)};
    }
