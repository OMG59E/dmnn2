...
__device__ void MapNMSData(EfficientNMSParameters param, int idx, int imageIdx, const Tb* __restrict__ boxesInput,
                           const Tb* __restrict__ anchorsInput, const int* __restrict__ topClassData,
                           const int* __restrict__ topAnchorsData, const int* __restrict__ topNumData,
                           const T* __restrict__ sortedScoresData, const int* __restrict__ sortedIndexData, T& scoreMap,
                           int& classMap, BoxCorner<T>& boxMap, int& boxIdxMap) {
...
__device__ void WriteNMSResult(EfficientNMSParameters param, int* __restrict__ numDetectionsOutput,
                               ...
                               BoxCorner<T>* __restrict__ nmsBoxesOutput, T threadScore, int threadClass,
                               BoxCorner<T> threadBox, int imageIdx, unsigned int resultsCounter) {
...
__global__ void EfficientNMS(EfficientNMSParameters param, const int* topNumData, int* outputIndexData,
    ...
    for (int tile = 0; tile < numTiles; tile++) {
        ...
        MapNMSData<T, Tb>(param, boxIdx[tile], imageIdx, boxesInput, anchorsInput, topClassData, topAnchorsData,
                          topNumData, sortedScoresData, sortedIndexData, threadScore[tile], threadClass[tile],
                ...
                if (resultsCounter >= param.numOutputBoxes) {
                    ...
                    if (write) {
                        ...
                        if (param.outputONNXIndices) {
                            WriteONNXResult(param, outputIndexData, nmsIndicesOutput, imageIdx, threadClass[tile],
                                            boxIdxMap[tile]);
                        } else {
                            WriteNMSResult<T>(param, numDetectionsOutput, nmsScoresOutput, nmsClassesOutput,
                                              nmsBoxesOutput, threadScore[tile], threadClass[tile], threadBox[tile],
...
cudaError_t EfficientNMSLauncher(EfficientNMSParameters& param, int* topNumData, int* outputIndexData,
                                 int* outputClassData, int* sortedIndexData, T* sortedScoresData, int* topClassData,
                                 int* topAnchorsData, const void* boxesInput, const void* anchorsInput,
                                 int* numDetectionsOutput, T* nmsScoresOutput, int* nmsClassesOutput,
    ...
    if (param.boxCoding == 0) {
        EfficientNMS<T, BoxCorner<T>><<<gridSize, blockSize, 0, stream>>>(
            param, topNumData, outputIndexData, outputClassData, sortedIndexData, sortedScoresData, topClassData,
            topAnchorsData, (BoxCorner<T>*)boxesInput, (BoxCorner<T>*)anchorsInput, numDetectionsOutput,
            nmsScoresOutput, nmsClassesOutput, nmsIndicesOutput, (BoxCorner<T>*)nmsBoxesOutput);
        ...
        EfficientNMS<T, BoxCenterSize<T>><<<gridSize, blockSize, 0, stream>>>(
            param, topNumData, outputIndexData, outputClassData, sortedIndexData, sortedScoresData, topClassData,
            topAnchorsData, (BoxCenterSize<T>*)boxesInput, (BoxCenterSize<T>*)anchorsInput, numDetectionsOutput,
            nmsScoresOutput, nmsClassesOutput, nmsIndicesOutput, (BoxCorner<T>*)nmsBoxesOutput);
    }
...
__global__ void EfficientNMSFilter(EfficientNMSParameters param, const T* __restrict__ scoresInput,
                                   int* __restrict__ topNumData, int* __restrict__ topIndexData,
                                   int* __restrict__ topAnchorsData, T* __restrict__ topScoresData,
...
__global__ void EfficientNMSDenseIndex(EfficientNMSParameters param, int* __restrict__ topNumData,
                                       int* __restrict__ topIndexData, int* __restrict__ topAnchorsData,
                                       int* __restrict__ topOffsetsStartData, int* __restrict__ topOffsetsEndData,
...
cudaError_t EfficientNMSFilterLauncher(EfficientNMSParameters& param, const T* scoresInput, int* topNumData,
                                       int* topIndexData, int* topAnchorsData, int* topOffsetsStartData,
                                       int* topOffsetsEndData, T* topScoresData, int* topClassData,
                                       cudaStream_t stream) {
    ...
    if (param.scoreThreshold < kernelSelectThreshold) {
        ...
        CUDACHECK(cudaMemcpyAsync(topScoresData, scoresInput, param.batchSize * param.numScoreElements * sizeof(T),
                                  cudaMemcpyDeviceToDevice, stream));

        EfficientNMSDenseIndex<T><<<gridSize, blockSize, 0, stream>>>(param, topNumData, topIndexData, topAnchorsData,
                                                                      topOffsetsStartData, topOffsetsEndData,
                                                                      topScoresData, topClassData);
    } else {
        EfficientNMSFilter<T><<<gridSize, blockSize, 0, stream>>>(param, scoresInput, topNumData, topIndexData,
                                                                  topAnchorsData, topScoresData, topClassData);

        EfficientNMSFilterSegments<<<1, param.batchSize, 0, stream>>>(param, topNumData, topOffsetsStartData,
                                                                      topOffsetsEndData);
    }
...
pluginStatus_t EfficientNMSDispatch(EfficientNMSParameters param, const void* boxesInput, const void* scoresInput,
                                    const void* anchorsInput, void* numDetectionsOutput, void* nmsBoxesOutput,
                                    void* nmsScoresOutput, void* nmsClassesOutput, void* nmsIndicesOutput,
    ...
    if (param.outputONNXIndices) {
        ...
        CUDACHECK(cudaMemsetAsync(numDetectionsOutput, 0x00, param.batchSize * sizeof(int), stream));
        CUDACHECK(cudaMemsetAsync(nmsScoresOutput, 0x00, param.batchSize * param.numOutputBoxes * sizeof(T), stream));
        CUDACHECK(
    ...
    CUDACHECK(cudaMemsetAsync(topNumData, 0x00, countersTotalSize * sizeof(int), stream));
    cudaError_t status = cudaGetLastError();
    LOG_ASSERT(status == STATUS_SUCCESS);
    ...
    status = cub::DeviceSegmentedRadixSort::SortPairsDescending(
        sortedWorkspaceData, sortedWorkspaceSize, scoresDB, indexDB, param.batchSize * param.numScoreElements,
        param.batchSize, topOffsetsStartData, topOffsetsEndData, param.scoreBits > 0 ? (10 - param.scoreBits) : 0,
        param.scoreBits > 0 ? 10 : sizeof(T) * 8, stream);
    LOG_ASSERT(status == STATUS_SUCCESS);

    status = EfficientNMSLauncher<T>(param, topNumData, outputIndexData, outputClassData, indexDB.Current(),
                                     scoresDB.Current(), topClassData, topAnchorsData, boxesInput, anchorsInput,
                                     (int*)numDetectionsOutput, (T*)nmsScoresOutput, (int*)nmsClassesOutput,
                                     (int*)nmsIndicesOutput, nmsBoxesOutput, stream);
    LOG_ASSERT(status == STATUS_SUCCESS);
...
pluginStatus_t EfficientNMSInference(EfficientNMSParameters param, const void* boxesInput, const void* scoresInput,
    ...
    if (param.datatype == DataType::kFLOAT) {
        ...
        return EfficientNMSDispatch<float>(param, boxesInput, scoresInput, anchorsInput, numDetectionsOutput,
                                           nmsBoxesOutput, nmsScoresOutput, nmsClassesOutput, nmsIndicesOutput,
                                           workspace, stream);
