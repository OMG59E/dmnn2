...
void UpsampleForward(cudaStream_t stream, const float* bottom, const int count, const int scale, const int oC,
                     const int oH, const int oW, float* top) {
    upscale<float><<<CUDA_GET_BLOCKS(count), CUDA_NUM_THREADS, 0, stream>>>(bottom, top, count, scale, oC, oH, oW);
    CUDACHECK(cudaStreamSynchronize(stream));
}

int Upsample::enqueue(int batchSize, void const* const* inputs, void* const* outputs, void* workspace,
