...
public:
    ...
    size_t getWorkspaceSize(int maxBatchSize) const noexcept override {
        ...
        wss[6] =
            std::max(sortScoresPerClassWorkspaceSize(maxBatchSize, param_.num_classes, H_ * W_, DataType::kFLOAT),
    ...
    size_t getSerializationSize() const noexcept override {
        return sizeof(CenterFaceOutputParameters) + sizeof(int) * 2;
    }
    ...
    const char *getPluginType() const noexcept override {
        return CENTER_NMS_PLUGIN_NAME;
    }
    const char *getPluginVersion() const noexcept override {
        return CENTER_NMS_PLUGIN_VERSION;
    }
    ...
    void setPluginNamespace(const char *pluginNamespace) noexcept override {
        mPluginNamespace_ = pluginNamespace;
    }
    const char *getPluginNamespace() const noexcept override {
        return mPluginNamespace_.c_str();
    }
    ...
    void configurePlugin(const Dims *inputDims, int nbInputs, const Dims *outputDims, int nbOutputs,
        ...
        assert(inputDims[0].d[0] == param_.num_classes && inputDims[1].d[0] == 2 && inputDims[2].d[0] == 2 &&
...
private:
    pluginStatus_t ctFaceNMSInference(cudaStream_t stream, int batch, const float *hmData, const float *scaleData,
                                      const float *offsetData, const float *landDataRaw, void *workspace,
                                      float *topDetections);
