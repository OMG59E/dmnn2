...
__global__ void ctdecode_kernel(const int nbThreads, int H, int W, int num_pts, float conf_thresh, const float* hmData,
...
pluginStatus_t CenterFaceOutput::ctFaceNMSInference(cudaStream_t stream, int batch, const float* hmData,
                                                    const float* scaleData, const float* offsetData,
                                                    const float* landDataRaw, void* workspace, float* topDetections) {
    ...
    ctdecode_kernel<<<CUDA_GET_BLOCKS(locCount), CUDA_NUM_THREADS, 0, stream>>>(
        locCount, H_, W_, param_.num_pts, param_.confidence_threshold, hmData, scaleData, offsetData, landDataRaw,
        (float*)bboxData, (float*)scoreData, (float*)landData);
    ...
    pluginStatus_t status =
        sortScoresPerClass(stream, batch, param_.num_classes, H_ * W_, -1, param_.confidence_threshold,
                           DataType::kFLOAT, scoreData, indices, sortingWorkspace);
    ...
    // NMS
    status = allClassNMS(stream, batch, param_.num_classes, H_ * W_, param_.topK, param_.nms_threshold, true, true,
                         DataType::kFLOAT, DataType::kFLOAT, bboxData, scoreData, indices, postNMSScores,
                         postNMSIndices, false);
    if (STATUS_SUCCESS != status) {
    ...
    status = sortScoresPerImage(stream, batch, param_.num_classes * param_.topK, DataType::kFLOAT, postNMSScores,
                                postNMSIndices, scoreData, indices, sortingWorkspace);
    if (STATUS_SUCCESS != status) {
        LOG_ERROR("Failed to sortScoresPerImage");
        return status;
    }

    status = gatherTopDetectionsV2(stream, true, batch, H_ * W_, param_.num_classes, param_.num_pts, param_.topK,
                                   param_.keep_topK, DataType::kFLOAT, DataType::kFLOAT, indices, scoreData, bboxData,
                                   landData, nullptr, topDetections);
    if (STATUS_SUCCESS != status) {
...
int CenterFaceOutput::enqueue(int32_t batchSize, void const* const* inputs, void* const* outputs, void* workspace,
    ...
    ctFaceNMSInference(stream, batchSize, heatmap_data, scale_data, offset_data, landmark_data, workspace,
                       topDetections);
    return 0;
}