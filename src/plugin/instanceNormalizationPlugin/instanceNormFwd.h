...
struct Instance_norm_kernel_params {
    ...
    enum { PIXELS_PER_THREAD_IN_REGISTERS = getPixelsPerThreadInRegisters<StorageType, SM>() };
    enum { PIXELS_PER_THREAD_IN_SMEM = getPixelsPerThreadInSmem<StorageType, SM>() };
...
void instanceNormBufferSizesDispatch(const InstanceNormFwdContext &context, const InstanceNormFwdParams &params,
                                     ...
                                     int32_t input_data_type = 1, int32_t output_data_type = 1);
