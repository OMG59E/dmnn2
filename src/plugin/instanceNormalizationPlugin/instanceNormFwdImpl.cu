...
#pragma unroll
        ...
        int32_t global_thread_c_input = (IS_INPUT_INT8 || IS_OUTPUT_INT8)
                                            ? thread_in_cta_c * ELEMENTS_PER_LDG +
        ...
        const Input_Data_Type *gmem_src =
            &reinterpret_cast<const Input_Data_Type *>(params.gmem_src)[global_thread_c_input + global_batch_offset];
...
template <typename Kernel_params>
void instanceNormBufferSizes(const InstanceNormFwdParams &params, size_t &size_sums, size_t &size_counts,
                             size_t &size_retired_ctas) {
