...
#pragma unroll
            ...
            int32_t global_thread_c_input = (IS_INPUT_INT8 || IS_OUTPUT_INT8) ? thread_in_cta_c * ELEMENTS_PER_LDG
                                                                                + (cta_c %
                                                                                   32)  // handle C_ELEMENTS_PER_CTA == 16 case
            ...
            const Input_Data_Type *gmem_src = &reinterpret_cast<const Input_Data_Type *>(params.gmem_src)[
                    global_thread_c_input + global_batch_offset];
...
#pragma unroll
            ...
            Output_Data_Type *const gmem_dst = &reinterpret_cast<Output_Data_Type *>(params.gmem_dst)[
                    global_thread_c_output + global_batch_offset];
...
#pragma unroll
    ...
    template<typename Kernel_params>
    int32_t instance_norm_fwd_launch(const InstanceNormFwdContext &context, InstanceNormFwdParams &params,
                                     cudaStream_t stream) {
...
#define KERNEL_RUN(OUTER_LOOPS, DESIRED_OCCUPANCY)  \
         {                            \
             CUDACHECK(cudaMemsetAsync(params.gmem_retired_ctas, 0, size_retired_ctas, stream)); \
             if( smem_size > 0 ) \
        ...
        int32_t smem_driven_fwd_occupancy = min(int32_t(context.sm_shared_size) / (int32_t) total_smem_bytes,
                                                (int32_t) 2);
        ...
        if (max_grid / int32_t(grid_dim.x) > 1) {
            ...
            grid_dim.y =
                    int32_t(grid_dim.y) > params.c_blks * params.n ? params.c_blks * params.n : int32_t(grid_dim.y);
        } else {
            grid_dim.y = 1;
        }
    ...
    void instanceNormBufferSizesDispatch(const InstanceNormFwdContext &context, const InstanceNormFwdParams &params,
        ...
        if (input_data_type == 2 && output_data_type == 2) {
            switch (context.sm_version) {
                case 700:
                    return instanceNormBufferSizes<kernel_params_32_int8_sm_700>(params, size_sums, size_counts,
                                                                                 size_retired_ctas);
                    break;
                case 720:
                    return instanceNormBufferSizes<kernel_params_32_int8_sm_720>(params, size_sums, size_counts,
                                                                                 size_retired_ctas);
                    break;
                case 750:
                    return instanceNormBufferSizes<kernel_params_32_int8_sm_750>(params, size_sums, size_counts,
                                                                                 size_retired_ctas);
                    break;
                case 800:
                    return instanceNormBufferSizes<kernel_params_32_int8_sm_800>(params, size_sums, size_counts,
                                                                                 size_retired_ctas);
                    break;
                case 860:
                    return instanceNormBufferSizes<kernel_params_32_int8_sm_860>(params, size_sums, size_counts,
                                                                                 size_retired_ctas);
                    break;
                default:
                    return instanceNormBufferSizes<kernel_params_32_int8>(params, size_sums, size_counts,
                                                                          size_retired_ctas);
                    break;
            }
            ...
            return instanceNormBufferSizes<kernel_params_32_fp16_int8>(params, size_sums, size_counts,
                                                                       size_retired_ctas);
    ...
    int32_t
    instanceNormFwdDispatch(const InstanceNormFwdContext &context, InstanceNormFwdParams &params, cudaStream_t stream,
                            int32_t input_data_type, int32_t output_data_type) {
        assert(context.sm_version >= 600);
        if (input_data_type == 2 && output_data_type == 2) {
            switch (context.sm_version) {
                case 700:
                    return instance_norm_fwd_launch<kernel_params_32_int8_sm_700>(context, params, stream);
                    break;
                case 720:
                    return instance_norm_fwd_launch<kernel_params_32_int8_sm_720>(context, params, stream);
                    break;
                case 750:
                    return instance_norm_fwd_launch<kernel_params_32_int8_sm_750>(context, params, stream);
                    break;
                case 800:
                    return instance_norm_fwd_launch<kernel_params_32_int8_sm_800>(context, params, stream);
                    break;
                case 860:
                    return instance_norm_fwd_launch<kernel_params_32_int8_sm_860>(context, params, stream);
                    break;
                default:
                    return instance_norm_fwd_launch<kernel_params_32_int8>(context, params, stream);
                    break;
            }
        } else if (input_data_type == 1 && output_data_type == 2) {
