...
public:
    explicit PriorBox(PriorBoxParameters param) : mParam(param), mOwnsParamMemory(true) {
        ...
        /*
         * If we have maxSizes, as long as all the maxSizes meets assertion requirement, we add one bounding box per
         * maxSize The final number of prior boxes per grid cell on feature map numPriors = tmpAR.size() *
         * param.numMinSize If numMaxSize == 0 (tmpAR.size() + 1) * param.numMinSize If param.numMinSize ==
         * param.numMaxSize
         */
    ...
    PriorBox(PriorBoxParameters param, int numPriors, int H, int W, Weights minSize, Weights maxSize,
    ...
    PriorBox(const void *buffer, size_t length) : mOwnsParamMemory(true) {
