...
void YOLODetectionInference(cudaStream_t stream, const int N,
                            ...
                            const int numClasses, const int keep_topK, const int topK, const float nms_thresh,
                            const float conf_thresh, const float* inputData, void* workspace, float* topDetections) {
...
int YOLODetectionOutput::enqueue(int batchSize, void const* const* inputs, void* const* outputs, void* workspace,
...
int YOLODetectionOutputDynamic::enqueue(nvinfer1::PluginTensorDesc const* inputDesc,
