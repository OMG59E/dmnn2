...
__global__ void caffe_gpu_interp2_kernel(const int n, const float rheight, const float rwidth, const int channels,
                                         const Dtype *data1, const int x1, const int y1, const int height1,
                                         const int width1, const int Height1, const int Width1, Dtype *data2,
    ...
    if (index < n) {
        ...
        if (packed) {
            ...
            Dtype *pos2 = &data2[channels * ((y2 + h2) * Width2 + (x2 + w2))];
            for (int c = 0; c < channels; ++c) {
                pos2[0] = h0lambda * (w0lambda * pos1[0] + w1lambda * pos1[channels * w1p]) +
...
void caffe_gpu_interp2(cudaStream_t stream, const int channels, const float *data1, const int x1, const int y1,
                       const int height1, const int width1, const int Height1, const int Width1, float *data2,
                       const int x2, const int y2, const int height2, const int width2, const int Height2,
                       const int Width2) {
    ...
    caffe_gpu_interp2_kernel<float, false><<<CUDA_GET_BLOCKS(num_kernels), CUDA_NUM_THREADS, 0, stream>>>(
        num_kernels, rheight, rwidth, channels, data1, x1, y1, height1, width1, Height1, Width1, data2, x2, y2, height2,
        width2, Height2, Width2);
    CUDACHECK(cudaDeviceSynchronize());
    CUDACHECK(cudaPeekAtLastError());
}

int Interp::enqueue(int32_t batchSize, const void *const *inputs, void *const *outputs, void *,
    ...
    caffe_gpu_interp2(stream, batchSize * mChannels, input, -mPadBegin, -mPadBegin, height_in_eff, width_in_eff,
                      mIHeight, mIWidth, output, 0, 0, mOHeight, mOWidth, mOHeight, mOWidth);
    return 0;
}
