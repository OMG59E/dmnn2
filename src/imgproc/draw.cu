...
__global__ void line_hv_kernel(const int nbThreads, unsigned char *src, int channels, int src_h, int src_w,
                               nv::ColorType src_color_type, int line_h, int line_w, int x1, int y1, nv::Color color) {
    CUDA_KERNEL_LOOP(idx, nbThreads) {
        ...
        switch (src_color_type) {
            case nv::COLOR_TYPE_BGR888_PACKED:
                src[(dy + y1) * src_w * channels + (dx + x1) * channels + 0] = color.b;
                src[(dy + y1) * src_w * channels + (dx + x1) * channels + 1] = color.g;
                src[(dy + y1) * src_w * channels + (dx + x1) * channels + 2] = color.r;
                break;
            case nv::COLOR_TYPE_RGB888_PACKED:
                src[(dy + y1) * src_w * channels + (dx + x1) * channels + 0] = color.r;
                src[(dy + y1) * src_w * channels + (dx + x1) * channels + 1] = color.g;
                src[(dy + y1) * src_w * channels + (dx + x1) * channels + 2] = color.b;
                break;
...
__global__ void rectangle_kernel(const int nbThreads, unsigned char *src, int channels, int src_h, int src_w,
                                 nv::BoundingBox bbox, nv::ColorType src_color_type, nv::Color color) {
    CUDA_KERNEL_LOOP(idx, nbThreads) {
...
void line_hv(nv::Image &src, const nv::Point &p1, const nv::Point &p2, const nv::Color &color) {
    // clip
    ...
    line_hv_kernel<<<CUDA_GET_BLOCKS(nbThreads), CUDA_NUM_THREADS>>>(nbThreads, (unsigned char *)(src.gpu_data),
                                                                     src.channels(), src.h(), src.w(), src.colorType,
                                                                     line_h, line_w, x1, y1, color);
    CUDACHECK(cudaDeviceSynchronize());
    CUDACHECK(cudaGetLastError());
}

void circle(nv::Image &src, const nv::Point &p1, int radius, const nv::Color &color, int thickness) {}

void rectangle(nv::Image &src, const nv::BoundingBox &bbox, const nv::Color &color, int thickness) {
    if (thickness > 0) {
        line_hv(src, nv::Point(bbox.x1, bbox.y1), nv::Point(bbox.x1 - 1 + thickness, bbox.y2), color);
        line_hv(src, nv::Point(bbox.x1, bbox.y1), nv::Point(bbox.x2, bbox.y1 - 1 + thickness), color);
        line_hv(src, nv::Point(bbox.x1, bbox.y2 + 1 - thickness), nv::Point(bbox.x2, bbox.y2), color);
        line_hv(src, nv::Point(bbox.x2 + 1 - thickness, bbox.y1), nv::Point(bbox.x2, bbox.y2), color);
    } else {
        // thickness = 3;
        ...
        rectangle_kernel<<<CUDA_GET_BLOCKS(nbThreads), CUDA_NUM_THREADS>>>(
            nbThreads, (unsigned char *)(src.gpu_data), src.channels(), src.h(), src.w(), bbox, src.colorType, color);
        CUDACHECK(cudaDeviceSynchronize());
        CUDACHECK(cudaGetLastError());
    }
}
