/*
 * @Author: xingwg
 * @Date: 2024-11-01 16:34:18
 * @LastEditTime: 2024-12-13 15:58:56
 * @FilePath: /dmnn2/src/codecs/nv_decoder.cc
 * @Description:
 *
    ...
    if (pOPInfo->codec == cudaVideoCodec_AV1) {
        if (pOPInfo->av1.operating_points_cnt > 1) {
            ...
            if (m_nOperatingPoint >= pOPInfo->av1.operating_points_cnt)
                m_nOperatingPoint = 0;
            LOG_ERROR("AV1 SVC clip: operating point count {}",
                      pOPInfo->av1.operating_points_cnt);
            LOG_ERROR(
                "Selected operating point: {}, IDC {:#x} bOutputAllLayers {}",
                m_nOperatingPoint,
...
int NvDecoder::HandleVideoSequence(CUVIDEOFORMAT *pVideoFormat) {
    ...
    m_videoInfo << "\nVideo Input Information:" << std::endl
                << "\tCodec        : "
                << GetVideoCodecString(pVideoFormat->codec) << std::endl
                ...
                << std::endl
                << "\tBit depth    : "
                << pVideoFormat->bit_depth_luma_minus8 + 8 << std::endl;
    ...
    CUDA_DRVAPI_CALL(cuvidGetDecoderCaps(&decodecaps));
    ...
    if ((pVideoFormat->coded_width > decodecaps.nMaxWidth) ||
        ...
        errorString << std::endl
                    ...
                    << "x" << pVideoFormat->coded_height << std::endl
    ...
    CUDA_DRVAPI_CALL(cuCtxPushCurrent(m_cuContext));
    CUDA_DRVAPI_CALL(cuvidCreateDecoder(&m_hDecoder, &videoDecodeCreateInfo));
    CUDA_DRVAPI_CALL(cuCtxPopCurrent(nullptr));
    double elapsedTime =
        duration<double, std::micro>(high_resolution_clock::now() - start)
            .count();
    LOG_INFO("Session Initialization Time: {:.2f}us", elapsedTime);
...
int NvDecoder::ReconfigureDecoder(CUVIDEOFORMAT *pVideoFormat) {
    if (pVideoFormat->bit_depth_luma_minus8 !=
        ...
        LOG_ERROR("Reconfigure Not supported for bit depth change {}",
                  int(CUDA_ERROR_NOT_SUPPORTED));
        return 0;
    }

    if (pVideoFormat->chroma_format != m_videoFormat.chroma_format) {
        LOG_ERROR("Reconfigure Not supported for chroma format change {}",
                  int(CUDA_ERROR_NOT_SUPPORTED));
        return 0;
    }
    ...
    bool bDisplayRectChange =
        !(pVideoFormat->display_area.bottom ==
              m_videoFormat.display_area.bottom &&
    ...
    if ((pVideoFormat->coded_width > m_nMaxWidth) ||
        ...
        if ((m_eCodec != cudaVideoCodec_VP9) || m_bReconfigExternal) {
            LOG_ERROR("Reconfigure Not supported when width/height > "
                      "maxwidth/maxheight {}",
                      int(CUDA_ERROR_NOT_SUPPORTED));
            return 0;
        }
    ...
    if (!bDecodeResChange && !m_bReconfigExtPPChange) {
        // if the coded_width/coded_height hasn't changed but display resolution
        // has changed, then need to update width/height for correct output
        // without cropping. Example : 1920x1080 vs 1920x1088
        if (bDisplayRectChange) {
            m_nWidth = pVideoFormat->display_area.right -
                       pVideoFormat->display_area.left;
            m_nLumaHeight = pVideoFormat->display_area.bottom -
                            pVideoFormat->display_area.top;
            m_nChromaHeight = (int)ceil(m_nLumaHeight *
                                        GetChromaHeightFactor(m_eOutputFormat));
            m_nNumChromaPlanes = GetChromaPlaneCount(m_eOutputFormat);
        }
    ...
    // Dont change display rect and get scaled output from decoder. This will
    // help display app to present apps smoothly
    ...
    // If external reconfigure is called along with resolution change even if
    // post processing params is not changed, do full reconfigure params update
    if ((m_bReconfigExternal && bDecodeResChange) || m_bReconfigExtPPChange) {
        ...
        if (!(m_cropRect.r && m_cropRect.b) &&
            ...
            if (m_resizeDim.w && m_resizeDim.h) {
                reconfigParams.display_area.left =
                    pVideoFormat->display_area.left;
                reconfigParams.display_area.top =
                    pVideoFormat->display_area.top;
    ...
    auto start = high_resolution_clock::now();
    CUDA_DRVAPI_CALL(cuCtxPushCurrent(m_cuContext));
    CUDA_DRVAPI_CALL(cuvidReconfigureDecoder(m_hDecoder, &reconfigParams));
    CUDA_DRVAPI_CALL(cuCtxPopCurrent(NULL));
    double elapsedTime =
        duration<double, std::micro>(high_resolution_clock::now() - start)
            .count();
    LOG_INFO("Session Reconfigure Time: {:.2f}us", elapsedTime);
    return nDecodeSurface;
}
...
int NvDecoder::HandlePictureDecode(CUVIDPICPARAMS *pPicParams) {
    if (!m_hDecoder) {
        LOG_ERROR("Decoder not initialized. {}", int(CUDA_ERROR_NOT_SUPPORTED));
        return 0;
    }
    m_nPicNumInDecodeOrder[pPicParams->CurrPicIdx] = m_nDecodePicCnt++;
    CUDA_DRVAPI_CALL(cuCtxPushCurrent(m_cuContext));
    CUDA_DRVAPI_CALL(cuvidDecodePicture(m_hDecoder, pPicParams));
...
int NvDecoder::HandlePictureDisplay(CUVIDPARSERDISPINFO *pDispInfo) {
    ...
    if (m_bExtractSEIMessage) {
        if (m_SEIMessagesDisplayOrder[pDispInfo->picture_index].pSEIData) {
            ...
            uint32_t seiNumMessages =
                m_SEIMessagesDisplayOrder[pDispInfo->picture_index]
                    .sei_message_count;
            ...
            if (m_fpSEI) {
                for (uint32_t i = 0; i < seiNumMessages; i++) {
                    if (m_eCodec == cudaVideoCodec_H264 ||
                        cudaVideoCodec_H264_SVC || cudaVideoCodec_H264_MVC ||
                        cudaVideoCodec_HEVC) {
                        switch (seiMessagesInfo[i].sei_message_type) {
                        case SEI_TYPE_TIME_CODE: {
                            HEVCSEITIMECODE *timecode =
                                (HEVCSEITIMECODE *)seiBuffer;
                            fwrite(timecode, sizeof(HEVCSEITIMECODE), 1,
                                   m_fpSEI);
                            break;
                        }
                        case SEI_TYPE_USER_DATA_UNREGISTERED: {
                            fwrite(seiBuffer,
                                   seiMessagesInfo[i].sei_message_size, 1,
                                   m_fpSEI);
                            break;
                        }
                        }
    ...
    CUDA_DRVAPI_CALL(cuCtxPushCurrent(m_cuContext));
    CUDA_DRVAPI_CALL(cuvidMapVideoFrame(m_hDecoder, pDispInfo->picture_index,
                                        &dpSrcFrame, &nSrcPitch,
                                        &videoProcessingParameters));

    CUVIDGETDECODESTATUS DecodeStatus;
    memset(&DecodeStatus, 0, sizeof(DecodeStatus));
    CUresult result = cuvidGetDecodeStatus(m_hDecoder, pDispInfo->picture_index,
                                           &DecodeStatus);
    if (result == CUDA_SUCCESS &&
        (DecodeStatus.decodeStatus == cuvidDecodeStatus_Error ||
         DecodeStatus.decodeStatus == cuvidDecodeStatus_Error_Concealed))
        LOG_ERROR("Decode Error occurred for picture {}\n",
        ...
        if ((unsigned)++m_nDecodedFrame > m_vpFrame.size()) {
            ...
            if (m_bUseDeviceFrame) {
                if (m_bDeviceFramePitched) {
                    ...
                    CUDA_DRVAPI_CALL(
                        cuMemAlloc((CUdeviceptr *)&pFrame, GetFrameSize()));
                }
            } else {
    ...
    m.dstPitch =
        m_nDeviceFramePitch ? m_nDeviceFramePitch : GetWidth() * m_nBPP;
    m.WidthInBytes = GetWidth() * m_nBPP;
    m.Height = m_nLumaHeight;
    CUDA_DRVAPI_CALL(cuMemcpy2DAsync(&m, m_cuvidStream));

    // Copy chroma plane
    // NVDEC output has luma height aligned by 2. Adjust chroma offset by
    // aligning height
    ...
    CUDA_DRVAPI_CALL(cuvidUnmapVideoFrame(m_hDecoder, dpSrcFrame));
    return 1;
}
...
int NvDecoder::GetSEIMessage(CUVIDSEIMESSAGEINFO *pSEIMessageInfo) {
    ...
    if ((pSEIMessageInfo->picIdx < 0) ||
        ...
        LOG_ERROR("Invalid picture index ({})\n", pSEIMessageInfo->picIdx);
        return 0;
    }
    for (uint32_t i = 0; i < seiNumMessages; i++) {
    ...
    if (!m_pCurrSEIMessage) {
        LOG_ERROR("Out of Memory, Allocation failed for m_pCurrSEIMessage");
        return 0;
    }
    m_pCurrSEIMessage->pSEIData = malloc(totalSEIBufferSize);
    if (!m_pCurrSEIMessage->pSEIData) {
        LOG_ERROR("Out of Memory, Allocation failed for SEI Buffer");
        return 0;
    }
    ...
    if (!m_pCurrSEIMessage->pSEIMessage) {
        ...
        m_pCurrSEIMessage->pSEIData = nullptr;
        return 0;
    }
...
NvDecoder::NvDecoder(CUcontext cuContext, bool bUseDeviceFrame,
    ...
    : m_cuContext(cuContext), m_bUseDeviceFrame(bUseDeviceFrame),
      m_eCodec(eCodec), m_bDeviceFramePitched(bDeviceFramePitched),
      m_bExtractSEIMessage(extract_user_SEI_Message), m_nMaxWidth(maxWidth),
    ...
    CUDA_DRVAPI_CALL(cuvidCtxLockCreate(&m_ctxLock, cuContext));
    ...
    if (m_bExtractSEIMessage) {
        ...
        memset(&m_SEIMessagesDisplayOrder, 0,
               sizeof(m_SEIMessagesDisplayOrder));
    }
    ...
    videoParserParameters.pfnDisplayPicture =
        m_bForce_zero_latency ? nullptr : HandlePictureDisplayProc;
    videoParserParameters.pfnGetOperatingPoint = HandleOperatingPointProc;
    videoParserParameters.pfnGetSEIMsg =
        m_bExtractSEIMessage ? HandleSEIMessagesProc : nullptr;
    CUDA_DRVAPI_CALL(
        cuvidCreateVideoParser(&m_hParser, &videoParserParameters));
}
...
NvDecoder::~NvDecoder() {
    ...
    double elapsedTime =
        duration<double, std::micro>(high_resolution_clock::now() - start)
            .count();
    LOG_INFO("Session Deinitialization Time: {:.2f}us", elapsedTime);
    NvDecoder::addDecoderSessionOverHead(getDecoderSessionID(), elapsedTime);
}
...
int NvDecoder::Decode(const uint8_t *pData, int nSize, int nFlags,
    ...
    CUDA_DRVAPI_CALL(cuvidParseVideoData(m_hParser, &packet));
