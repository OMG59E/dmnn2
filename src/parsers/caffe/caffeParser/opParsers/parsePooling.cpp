...
ILayer *parsePooling(INetworkDefinition &network, const trtcaffe::LayerParameter &msg,
    ...
    PoolingType type =
        p.has_pool() && p.pool() == trtcaffe::PoolingParameter::AVE ? PoolingType::kAVERAGE : PoolingType::kMAX;
    auto layer = network.addPooling(*tensors[msg.bottom(0)], type, DimsHW{kernelH, kernelW});

    if (layer) {
        int stride = p.has_stride() ? p.stride() : 1;
        layer->setStride(
            DimsHW{p.has_stride_h() ? int(p.stride_h()) : stride, p.has_stride_w() ? int(p.stride_w()) : stride});
        ...
        layer->setPaddingMode(PaddingMode::kCAFFE_ROUND_UP);  // caffe pool use ceil mode by
                                                              // default
        ...
        if (p.has_torch_pooling() ? p.torch_pooling() : false)
            layer->setPaddingMode(PaddingMode::kCAFFE_ROUND_DOWN);  // facebook torch pool use
                                                                    // floor mode

        tensors[msg.top(0)] = layer->getOutput(0);

        layer->setAverageCountExcludesPadding(false);  // unlike other frameworks, caffe use inclusive counting for
                                                       // padded averaging
    }
    return layer;
}
