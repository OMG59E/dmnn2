...
ILayer *parseConvolution(INetworkDefinition &network, const trtcaffe::LayerParameter &msg,
    ...
    Weights kernelWeights =
        weightFactory.isInitialized()
            ...
            : weightFactory.allocateWeights(kernelW * kernelH * CbyG, std::normal_distribution<float>(0.0F, std_dev));
    ...
    if (layer) {
        ...
        int strideW =
            p.has_stride_w() ? p.stride_w() : p.stride_size() > 1 ? p.stride(1) : p.stride_size() > 0 ? p.stride(0) : 1;
