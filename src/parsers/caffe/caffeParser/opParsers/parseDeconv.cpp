 ...
 * @LastEditTime: 2024-12-13 12:48:16
 * @FilePath: /dmnn2/src/parsers/caffe/caffeParser/opParsers/parseDeconv.cpp
 * @Description:
 * @
...
ILayer *parseDeconvolution(INetworkDefinition &network, const trtcaffe::LayerParameter &msg,
    ...
    if (dilationH != 1 || dilationW != 1) {
        LOG_ERROR("Dilated deconvolution is not supported.");
        return nullptr;
    }
    ...
    Weights kernelWeights = weightFactory.isInitialized()
                                ? weightFactory(msg.name(), WeightType::kGENERIC)
                                : weightFactory.allocateWeights(kernelW * kernelH * C * nbOutputs / nbGroups,
                                                                std::normal_distribution<float>(0.0F, std_dev));
    Weights biasWeights = !p.has_bias_term() || p.bias_term()
    ...
    auto *layer = network.addDeconvolution(*tensors[msg.bottom(0)], nbOutputs, DimsHW{kernelH, kernelW}, kernelWeights,
                                           biasWeights);

    if (layer) {
        int strideW = p.has_stride_w() ? p.stride_w() : p.stride_size() > 0 ? p.stride(0) : 1;
        int strideH =
            p.has_stride_h() ? p.stride_h() : p.stride_size() > 1 ? p.stride(1) : p.stride_size() > 0 ? p.stride(0) : 1;
