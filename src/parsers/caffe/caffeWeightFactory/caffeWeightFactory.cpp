 ...
 * @LastEditTime: 2024-10-12 09:39:17
 * @FilePath: /dmnn2/src/parsers/caffe/caffeWeightFactory/caffeWeightFactory.cpp
 * @Description:
 * @
...
void *convertInternal(void **ptr, int64_t count, bool *mOK) {
    if (!ptr) {
        LOG_ERROR("convertInternal: ptr is nullptr");
        return nullptr;
    }
    if (!(*ptr)) {
        LOG_ERROR("convertInternal: *ptr is nullptr");
    ...
    if (!count) {
        LOG_ERROR("convertInternal: count is 0");
        return nullptr;
    }
...
Weights CaffeWeightFactory::operator()(const std::string &layerName,
    ...
    if (!blobMsg) {
        ...
        LOG_ERROR("Attempting to access NULL weights");
        return getNullWeights();
    }
...
bool CaffeWeightFactory::checkForNans(const void *values, int count,
    ...
    for (int i = 0; i < count; i++) {
        if (std::isnan(float(v[i]))) {
            LOG_ERROR("{}: Nan detected in weights", layerName);
            return false;
        }
    }
    return true;
