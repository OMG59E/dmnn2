...
NodeImportResult activationHelper(IImporterContext *ctx, const onnx::NodeProto &node,
                                  std::vector<TensorOrWeights> &inputs, nvinfer1::ActivationType op, float *alpha,
                                  float *beta) {
...
NodeImportResult argMinMaxHelper(IImporterContext *ctx, const onnx::NodeProto &node,
    ...
    if (needCast) {
        LOG_WARNING(
            "TensorRT is using FLOAT32 precision to run an INT32 "
    ...
    for (int32_t i = 0; i < firstRank; i++) {
        ...
        if (firstDim != secondDim && firstDim != 1 && secondDim != 1) {
            if (firstDim == -1 || secondDim == -1) {
                LOG_WARNING(
                    "Found dynamic dimensions when checking for "
                    "broadcast compatibility! TensorRT may fail at "
                    "build-time if the final shapes do not conform to "
                    "broadcasting rules.");
            } else {
                MAKE_ERROR(
                    "Found incompatible shapes for tensors that need to "
                    "be broadcastable!",
                    ErrorCode::kUNSUPPORTED_NODE);
            }
        }
...
std::vector<int32_t> calculateBias(const nvinfer1::Dims &daDims, const nvinfer1::Dims &idxDims,
...
nvinfer1::ITensor *constantOfShape(IImporterContext *ctx, const onnx::NodeProto &node, nvinfer1::ITensor *constant,
...
int32_t *convertINT64(const int64_t *weightValues, nvinfer1::Dims shape, IImporterContext *ctx) {
    ...
    int32_t *int32Weights{reinterpret_cast<int32_t *>(ctx->createTempWeights(onnx::TensorProto::INT32, shape).values)};
...
bool convertOnnxPadding(IImporterContext *ctx, int32_t nbInputDims, const std::vector<int32_t> &onnxPadding,
    ...
    startTensor = addConstant(ctx, start, onnx::TensorProto::INT32, nvinfer1::Dims{1, {nbInputDims}})->getOutput(0);
    totalPaddingTensor =
        addConstant(ctx, totalPadding, onnx::TensorProto::INT32, nvinfer1::Dims{1, {nbInputDims}})->getOutput(0);
...
onnx2trt::ShapedWeights createZeroShifts(const onnx2trt::ShapedWeights &shiftInt8, int32_t type,
                                         IImporterContext *ctx) {
    const auto *v = static_cast<const int8_t *>(shiftInt8.values);
    if (!std::all_of(v, v + shiftInt8.count(), [](int8_t x) { return x == 0; })) {
        LOG_WARNING(
            "TensorRT currenly supports only zero shifts values for "
...
DataType *convertINT32Data(const int32_t *weightValues, nvinfer1::Dims shape, int32_t onnxdtype,
                           IImporterContext *ctx) {
    const size_t nbWeights = volume(shape);
...
float *convertDouble(const double *weightValues, nvinfer1::Dims shape, IImporterContext *ctx) {
    ...
    if (!ctxImpl->isConvertDoubleLogged()) {
        LOG_WARNING(
            "Your ONNX model has been generated with double-typed "
    ...
    const size_t nbWeights = volume(shape);
    float *floatWeights{reinterpret_cast<float *>(ctx->createTempWeights(onnx::TensorProto::FLOAT, shape).values)};
...
bool convertOnnxWeights(const onnx::TensorProto &onnxTensor, onnx2trt::ShapedWeights *weights, IImporterContext *ctx) {
...
nvinfer1::ITensor *flattenTensor(IImporterContext *ctx, onnx::NodeProto const &node, nvinfer1::ITensor &tensor,
                                 int axis, bool regLayer) {
...
nvinfer1::ITensor *gatherDimension(IImporterContext *ctx, nvinfer1::ITensor *shapeTensor, int dim,
                                   nvinfer1::Dims shape) {
...
void generatePadding(nvinfer1::Dims inputShape, nvinfer1::Dims outputShape, nvinfer1::Dims kernelSize,
                     nvinfer1::Dims strides, nvinfer1::Dims dilations, const int nbSpatialDims,
                     nvinfer1::Dims &begPadding, nvinfer1::Dims &endPadding, nvinfer1::Dims &outputPadding,
...
nvinfer1::ITensor *globalPoolingHelper(IImporterContext *ctx, onnx::NodeProto const &node, nvinfer1::ITensor &tensor,
...
nvinfer1::ITensor *greaterLessOrEqual(IImporterContext *ctx, const onnx::NodeProto &node, nvinfer1::ITensor *inputA,
    ...
    nvinfer1::ITensor *equal =
        &elementwiseHelper(ctx, node, {inputA, inputB}, nvinfer1::ElementWiseOperation::kEQUAL).value().at(0).tensor();
    nvinfer1::ITensor *result =
        &elementwiseHelper(ctx, node, {firstOp, equal}, nvinfer1::ElementWiseOperation::kOR).value().at(0).tensor();
    return result;
}
...
nvinfer1::IPluginCreator *importPluginCreator(IImporterContext *ctx, std::string const &pluginName,
    ...
    if (creator == nullptr && safetyPluginRegistry != nullptr) {
        creator =
            safetyPluginRegistry->getPluginCreator(pluginName.c_str(), pluginVersion.c_str(), pluginNamespace.c_str());
    }
...
NodeImportResult instanceNormPluginHelper(IImporterContext *ctx, onnx::NodeProto const &node,
    ...
    ASSERT(inputs.at(1).is_weights() && "The scale tensor is required to be an initializer.",
           ErrorCode::kUNSUPPORTED_NODE);
    ASSERT(inputs.at(2).is_weights() && "The bias tensor is required to be an initializer.",
           ErrorCode::kUNSUPPORTED_NODE);
    ...
    ASSERT(plugin != nullptr &&
               "InstanceNormalization plugin was not found in "
               "the plugin registry!",
           ErrorCode::kUNSUPPORTED_NODE);
...
NodeImportResult normalizationHelper(IImporterContext *ctx, const onnx::NodeProto &node,
                                     std::vector<TensorOrWeights> &inputs) {
...
bool parseExternalWeights(IImporterContext *ctx, std::string file, std::string path, int64_t offset, int64_t length,
    ...
    if (illegalDir) {
        LOG_ERROR(
            "Relative paths to parent (../) are not allowed in ONNX "
...
NodeImportResult poolingHelper(IImporterContext *ctx, onnx::NodeProto const &node, std::vector<TensorOrWeights> &inputs,
    ...
    if (ctx->getOpsetVersion() >= 10) {
        ...
        for (size_t i = 0; i < dilations.size(); i++)
            ASSERT((dilations[i] == 1) &&
                       "This version of TensorRT does not "
...
void setAttr(nvinfer1::Dims *trtAttr, onnx::AttributeProto const *onnxAttr, int nbSpatialDims, int defaultVal) {
...
nvinfer1::ITensor *sliceAcrossAxis(IImporterContext *ctx, const onnx::NodeProto &node, nvinfer1::ITensor *data,
                                   const int axis) {
...
nvinfer1::ITensor *squeezeTensor(IImporterContext *ctx, const onnx::NodeProto &node, nvinfer1::ITensor &tensor,
...
nvinfer1::ITensor *transposeTensor(IImporterContext *ctx, const onnx::NodeProto &node, nvinfer1::ITensor &tensor,
...
NodeImportResult unaryHelper(IImporterContext *ctx, const onnx::NodeProto &node, TensorOrWeights &input,
    ...
    ASSERT(validUnaryType &&
               "This version of TensorRT does not support the "
...
NodeImportResult convMultiInput(IImporterContext *ctx, const onnx::NodeProto &node,
                                std::vector<TensorOrWeights> &inputs) {
    ...
    getKernelParams(ctx, node, &filter_dim, &strides, &beg_padding, &end_padding, paddingMode, exclude_padding,
                    &dilations);
...
nvinfer1::ITensor *unsqueezeTensor(IImporterContext *ctx, const onnx::NodeProto &node, nvinfer1::ITensor &tensor,
                                   const std::vector<int> &axes, bool regLayer) {
    const auto dims = shapeOf(tensor);
...
int64_t volume(const nvinfer1::Dims &dims) {
    std::for_each(dims.d, dims.d + dims.nbDims,
                  [](int32_t d) { assert(d >= 0 && "volume makes no sense for dynamic shapes"); });
...
static ShapeTensor clamp(IImporterContext *ctx, const ShapeTensor &x, const ShapeTensor &lowerBound,
...
NodeImportResult addScatterLayer(IImporterContext *ctx, onnx::NodeProto const &node,
    ...
    if (mode == nvinfer1::ScatterMode::kELEMENT) {
        ...
        ASSERT(dataDims.nbDims == indicesDims.nbDims && dataDims.nbDims == updatesDims.nbDims &&
        ...
        for (int32_t i = 0; i < dataDims.nbDims; ++i) {
            if (indicesDims.d[i] != -1 && dataDims.d[i] != -1) {
                ASSERT(indicesDims.d[i] <= dataDims.d[i] && "Indices dimensions must be less than data dimensions!",
                       ErrorCode::kUNSUPPORTED_NODE);
            }
            if (updatesDims.d[i] != -1 && dataDims.d[i] != -1) {
                ASSERT(updatesDims.d[i] <= dataDims.d[i] && "Updates dimensions must be less than data dimensions!",
                       ErrorCode::kUNSUPPORTED_NODE);
            }
        }
    }
...
nvinfer1::IElementWiseLayer *modWithIntegerInputs(IImporterContext *ctx, nvinfer1::ITensor *input0,
                                                  nvinfer1::ITensor *input1, bool fmod) {
    ...
    return ctx->network()->addElementWise(
        *input0,
        *ctx->network()
             ->addElementWise(*input1, *ctx->network()->addElementWise(*input0, *input1, divOp)->getOutput(0),
                              eOp::kPROD)
             ->getOutput(0),
        eOp::kSUB);
}

nvinfer1::IElementWiseLayer *modWithFPInputs(IImporterContext *ctx, nvinfer1::ITensor *input0,
                                             nvinfer1::ITensor *input1, nvinfer1::ITensor *divResult, bool sameSign) {
    using eOp = nvinfer1::ElementWiseOperation;
    ...
    return ctx->network()->addElementWise(
        *input0,
        *ctx->network()
             ->addElementWise(*input1, *ctx->network()->addUnary(*divResult, roundOp)->getOutput(0), eOp::kPROD)
             ->getOutput(0),
        eOp::kSUB);
}
