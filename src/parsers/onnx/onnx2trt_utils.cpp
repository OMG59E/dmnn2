/***
 * @Author: xingwg
 * @Date: 2024-12-12 17:19:42
 * @LastEditTime: 2024-12-12 17:21:43
 * @FilePath: /dmnn2/src/parsers/onnx/onnx2trt_utils.cpp
 * @Description:
 * @
...
Status notInvalidType(TensorOrWeights const &input,
                      std::vector<std::string> const &invalidTypes) {
    bool invalid = std::any_of(invalidTypes.begin(), invalidTypes.end(),
                               [&](std::string invalidType) {
                                   return input.getType() == invalidType;
                               });
    if (invalid) {
        return MAKE_ERROR("Found invalid input type of " + input.getType(),
...
nvinfer1::ITensor *addClip(IImporterContext *ctx, nvinfer1::ITensor *input,
                           float clip) {
    if (clip >= 0.f) {
        nvinfer1::IActivationLayer *layer = ctx->network()->addActivation(
            *input, nvinfer1::ActivationType::kCLIP);
        layer->setAlpha(-clip);
        layer->setBeta(clip);
        return layer->getOutput(0);
    }
...
NodeImportResult argMinMaxHelper(IImporterContext *ctx,
    ...
    if (needCast) {
        LOG_WARNING("TensorRT is using FLOAT32 precision to run an INT32 "
                    "ArgMax / ArgMin. Rounding errors may occur for large "
                    "integer values");
    ...
    ASSERT((!selectLastIndex ||
            (selectLastIndex && ctx->getOpsetVersion() >= 12)) &&
               "Per-opset 12 ONNX does not support the select_last_index "
               "attribute.",
           ErrorCode::kUNSUPPORTED_NODE);
    ...
    if (selectLastIndex) {
        ...
        ShapeTensor const sizes =
            computeSliceSizes(ctx, starts, ends, steps, dims);

        nvinfer1::ISliceLayer *slice =
            addSlice(ctx, *tensor, starts, sizes, steps);
    ...
    // If selectLastIndex is true, the TopK operation was performed on reversed
    // data on the provided axis. Convert reversed indices back to forward
    // indices by calculating the following: indices = shape(tensor)[axis] -
    // indices - 1
    if (selectLastIndex) {
        ...
        auto const tensorDimOnAxis = constantOfShape(
            ctx, node, &dimOnAxis.tensor(ctx), &indicesDims.tensor(ctx));

        // Create constant of shape indicesDims with values of 1
        auto const ones = constantOfShape(
            ctx, node, &shapeVector(1).tensor(ctx), &indicesDims.tensor(ctx));

        std::vector<TensorOrWeights> newInputs{tensorDimOnAxis, indices, ones};
        indices = &elementwiseHelper(ctx, node, newInputs,
...
Status broadcastTensor(IImporterContext *ctx, nvinfer1::ITensor *&t,
                       const int nbDims) {
    ASSERT(ctx->getOpsetVersion() >= 7 &&
               "Pre-opset 7 broadcasting is unsupported in this version of the "
               "ONNX parser",
           ErrorCode::kUNSUPPORTED_NODE);
    ...
    if (nbInputDims < nbDims) {
        nvinfer1::IShuffleLayer *reshape = addShuffle(
            ctx, *t,
            concat(ctx,
                   fillShapeVector(ctx, 1, shapeVector(nbDims - nbInputDims)),
                   shapeOf(*t)));
...
Status broadcastTensors(IImporterContext *ctx, nvinfer1::ITensor *&t1,
                        nvinfer1::ITensor *&t2) {
...
Status broadcastTensors(IImporterContext *ctx, nvinfer1::ITensor *&t1,
    ...
    const int maxDims =
        std::max({t1->getDimensions().nbDims, t2->getDimensions().nbDims,
                  t3->getDimensions().nbDims});
...
Status isBroadcastValid(IImporterContext *ctx, const nvinfer1::Dims &firstShape,
                        const nvinfer1::Dims &secondShape) {
    const auto firstRank = firstShape.nbDims;
    const auto secondRank = secondShape.nbDims;
    if (firstRank != secondRank) {
        return MAKE_ERROR("Cannot broadcast shapes that have different ranks!",
                          ErrorCode::kUNSUPPORTED_NODE);
    }
    for (int32_t i = 0; i < firstRank; i++) {
        ...
        if (firstDim != secondDim && firstDim != 1 && secondDim != 1) {
            if (firstDim == -1 || secondDim == -1) {
                LOG_WARNING("Found dynamic dimensions when checking for "
                            "broadcast compatibility! TensorRT may fail at "
                            "build-time if the final shapes do not conform to "
                            "broadcasting rules.");
            } else {
                MAKE_ERROR("Found incompatible shapes for tensors that need to "
                           "be broadcastable!",
                           ErrorCode::kUNSUPPORTED_NODE);
            }
        }
...
int32_t getBias(const std::vector<int32_t> &dimension_count,
                const std::vector<int32_t> &pitches, int32_t axis) {
    int32_t result{0};
...
void incrementOuterDimension(std::vector<int32_t> &dimensionCount,
                             nvinfer1::Dims idxDims) {
...
std::vector<int32_t> calculateBias(const nvinfer1::Dims &daDims,
                                   const nvinfer1::Dims &idxDims,
                                   const std::vector<int32_t> &pitches,
                                   int32_t axis) {
...
bool canUseNDResize(size_t const scaleSize, float const *scaleFactors,
                    size_t const n) {
    // Linear resize supports up to 3D resize on the outermost dimensions (n =
    // 3).
    if (scaleSize > n) {
...
nvinfer1::ITensor *castHelper(IImporterContext *ctx, nvinfer1::ITensor *input,
...
nvinfer1::ITensor *constantOfShape(IImporterContext *ctx,
                                   const onnx::NodeProto &node,
                                   nvinfer1::ITensor *constant,
...
Status convertAxis(int &axis, int nbDims) {
    ...
    ASSERT((axis >= 0 && axis <= nbDims) &&
               "Axis must be in the range [0, nbDims].",
           ErrorCode::kUNSUPPORTED_NODE);
    return Status::success();
}
...
bool convertDtype(int32_t onnx_dtype, nvinfer1::DataType *trt_dtype) {
    ...
    case onnx::TensorProto::UINT8:
        *trt_dtype = nvinfer1::DataType::kUINT8;
        break;
    ...
    case onnx::TensorProto::INT32:
        *trt_dtype = nvinfer1::DataType::kINT32;
        break;
    // See convertOnnxWeights for sanity check if all values can be safetly
    // downcasted to INT32
    case onnx::TensorProto::INT64:
        *trt_dtype = nvinfer1::DataType::kINT32;
        break;
    default:
        std::cerr << "Unsupported ONNX data type: " << getDtypeName(onnx_dtype)
                  << " (" << std::to_string(onnx_dtype) << ")" << std::endl;
        return false;
    }
    return true;
}

int32_t *convertINT64(const int64_t *weightValues, nvinfer1::Dims shape,
                      IImporterContext *ctx) {
    auto ctxImpl = static_cast<ImporterContext *>(ctx);
    if (!ctxImpl->isConvertINT64Logged()) {
        LOG_WARNING("Your ONNX model has been generated with INT64 weights, "
    ...
    int32_t *int32Weights{reinterpret_cast<int32_t *>(
        ctx->createTempWeights(onnx::TensorProto::INT32, shape)
            .values)};

    bool outOfBounds{false};
    for (size_t i = 0; i < nbWeights; i++) {
        if (weightValues[i] > static_cast<int64_t>(INT32_MAX) ||
            weightValues[i] < static_cast<int64_t>(INT32_MIN)) {
            int32Weights[i] = static_cast<int32_t>(std::max(
                std::min(weightValues[i], static_cast<int64_t>(INT32_MAX)),
...
bool convertOnnxPadding(IImporterContext *ctx, int32_t nbInputDims,
                        const std::vector<int32_t> &onnxPadding,
                        nvinfer1::ITensor *&startTensor,
                        nvinfer1::ITensor *&totalPaddingTensor) {
    ...
    const auto diff =
        nbInputDims - static_cast<int32_t>(onnxPadding.size() / 2U);
    if (diff < 0) {
        return false;
    }
    ...
    startTensor = addConstant(ctx, start, onnx::TensorProto::INT32,
                              nvinfer1::Dims{1, {nbInputDims}})
                      ->getOutput(0);
    totalPaddingTensor =
        addConstant(ctx, totalPadding, onnx::TensorProto::INT32,
                    nvinfer1::Dims{1, {nbInputDims}})
            ->getOutput(0);
...
bool shiftIsAllZeros(const ShapedWeights &shiftInt8) {
    ...
    auto allZeros =
        std::all_of(v, v + shiftInt8.count(), [](int8_t x) { return x == 0; });
    return allZeros;
}

onnx2trt::ShapedWeights
createZeroShifts(const onnx2trt::ShapedWeights &shiftInt8, int32_t type,
                 IImporterContext *ctx) {
    const auto *v = static_cast<const int8_t *>(shiftInt8.values);
    if (!std::all_of(v, v + shiftInt8.count(),
                     [](int8_t x) { return x == 0; })) {
        LOG_WARNING("TensorRT currenly supports only zero shifts values for "
                    "QuatizeLinear/DequantizeLinear ops");
    }
...
nvinfer1::ITensor *createZeroTensor(IImporterContext *ctx,
                                    nvinfer1::ITensor *data) {
    nvinfer1::ITensor *zero =
        addConstant(ctx, std::vector<float>{0.f},
                    onnx::TensorProto::FLOAT, {0, {1}})
            ->getOutput(0);
    zero = castHelper(ctx, zero, data->getType());
    broadcastTensors(ctx, zero, data);
    zero = ctx->network()
               ->addElementWise(*data, *zero,
...
DataType *convertINT32Data(const int32_t *weightValues, nvinfer1::Dims shape,
                           int32_t onnxdtype, IImporterContext *ctx) {
    const size_t nbWeights = volume(shape);
    DataType *newWeights{reinterpret_cast<DataType *>(
        ctx->createTempWeights(onnxdtype, shape).values)};
...
int32_t *convertUINT8(const uint8_t *weightValues, nvinfer1::Dims shape,
                      IImporterContext *ctx) {
    const size_t nbWeights = volume(shape);
    int32_t *int32Weights{reinterpret_cast<int32_t *>(
        ctx->createTempWeights(onnx::TensorProto::INT32, shape)
...
float *convertDouble(const double *weightValues, nvinfer1::Dims shape,
                     IImporterContext *ctx) {
    auto ctxImpl = static_cast<ImporterContext *>(ctx);
    if (!ctxImpl->isConvertDoubleLogged()) {
        LOG_WARNING("Your ONNX model has been generated with double-typed "
                    "weights, while TensorRT does not natively support "
                    "double. "
                    "Attempting to cast down to float.");
        ctxImpl->setConvertDoubleLogged(true);
    }
    const size_t nbWeights = volume(shape);
    float *floatWeights{reinterpret_cast<float *>(
        ctx->createTempWeights(onnx::TensorProto::FLOAT, shape)
            .values)};

    bool outOfBounds{false};
    const double floatMax =
        static_cast<double>(std::numeric_limits<float>::max());
    const double floatMin =
        static_cast<double>(std::numeric_limits<float>::lowest());
    for (size_t i = 0; i < nbWeights; i++) {
        if (weightValues[i] > floatMax || weightValues[i] < floatMin) {
            floatWeights[i] = static_cast<float>(
                std::max(std::min(weightValues[i], floatMax), floatMin));
    ...
    if (outOfBounds && !ctxImpl->isConvertDoubleOutOfBoundsLogged()) {
        LOG_WARNING(
            "One or more weights outside the range of FLOAT was clamped");
...
bool convertOnnxWeights(const onnx::TensorProto &onnxTensor,
    ...
    // ONNX weight values can be stored in either the TensorProto itself, or in
    // an external file in the case of large models. Check for this here.
    ...
    if (dataLocation == 1) {
        ...
        // onnxTensor.external_data() is a String : String map that holds
        // metadata about how to read from an external file
        ...
        if (!parseExternalWeights(ctx, location, ctx->getOnnxFileLocation(),
                                  offset, length, dataBuf, nbytes)) {
            return false;
        }

        // For weights parsed from external files, createTempWeights is
        // necessary to keep them in scope
        ...
        if (onnxDtype == onnx::TensorProto::INT64) {
            ...
            dataPtr = convertINT64(reinterpret_cast<const int64_t *>(dataPtr),
                                   shape, ctx);
            ...
            dataPtr = convertUINT8(reinterpret_cast<const uint8_t *>(dataPtr),
                                   shape, ctx);
            nbytes = nbytes * (sizeof(int32_t) / sizeof(uint8_t));
            onnxDtype = onnx::TensorProto::INT32;
        } else if (onnxDtype == onnx::TensorProto::DOUBLE) {
            // Cast DOUBLE weights to FLOAT.
            dataPtr = convertDouble(reinterpret_cast<const double *>(dataPtr),
                                    shape, ctx);
        ...
        if (externalWeights.size_bytes() != nbytes) {
            LOG_ERROR(
                "Unexpected size for the external weights! Expected size: "
                << externalWeights.size_bytes() << " bytes (shape = " << shape
                << "). Actual size: " << nbytes << " bytes.");
            return false;
        }
    ...
    if (onnxDtype == onnx::TensorProto::INT64) {
        if (onnxTensor.raw_data().size() > 0) {
            dataPtr = convertINT64(
                reinterpret_cast<const int64_t *>(onnxTensor.raw_data().data()),
                shape, ctx);
        ...
        if (onnxTensor.raw_data().size() > 0) {
            dataPtr = convertUINT8(
                reinterpret_cast<const uint8_t *>(onnxTensor.raw_data().data()),
                shape, ctx);
            nbytes = onnxTensor.raw_data().size() *
        ...
        if (onnxTensor.raw_data().size() > 0) {
            dataPtr = convertDouble(
                reinterpret_cast<const double *>(onnxTensor.raw_data().data()),
                shape, ctx);
            nbytes =
                onnxTensor.raw_data().size() / (sizeof(double) / sizeof(float));
        } else if (onnxTensor.double_data().size() > 0) {
            dataPtr =
                convertDouble(onnxTensor.double_data().data(), shape, ctx);
            nbytes = onnxTensor.double_data().size() * sizeof(float);
        }
        onnxDtype = onnx::TensorProto::FLOAT;
    }

    // Check for supported types that can be found in the int32_data field in
    // the TensorProto
    // https://github.com/onnx/onnx/blob/master/onnx/onnx.proto#L528
    else if (onnxDtype == onnx::TensorProto::INT32 ||
             onnxDtype == onnx::TensorProto::FLOAT16 ||
             onnxDtype == onnx::TensorProto::INT8 ||
             onnxDtype == onnx::TensorProto::BOOL) {
        if (onnxTensor.raw_data().size() > 0) {
            ...
            case onnx::TensorProto::FLOAT16:
                dataPtr = convertINT32Data<uint16_t>(
                    onnxTensor.int32_data().data(), shape, onnxDtype, ctx);
                break;
            case onnx::TensorProto::INT8:
            ...
            case onnx::TensorProto::BOOL:
                ...
                break;
            default:
                LOG_ERROR("Found unsupported datatype ("
                          << onnxDtype << ") when importing initializer: "
                          << onnxTensor.name());
                break;
            }
    ...
    if (trt_weights.size_bytes() != nbytes) {
        LOG_ERROR("Size mismatch when importing initializer: "
                  << onnxTensor.name() << ". Expected size: " << nbytes
...
nvinfer1::ITensor *convertToScalar(IImporterContext *ctx,
                                   nvinfer1::ITensor *inpTensor) {
    ...
    if (tensorVolume != 1) {
        LOG_VERBOSE(
            "Cannot convert tensor to scalar. Note: Tensor dimensions were: "
    ...
    // Do not need to call setZeroIsPlaceholder, since reshape dimensions are
    // empty.
    return reshape->getOutput(0);
}

nvinfer1::ITensor &convertToTensor(TensorOrWeights &input,
                                   IImporterContext *ctx) {
    if (input.is_tensor()) {
        return input.tensor();
    }
...
nvinfer1::ITensor *convertToScalar(TensorOrWeights &input,
                                   IImporterContext *ctx) {
    if (input.is_tensor()) {
    ...
    if (volume(weights.shape) != 1) {
        LOG_VERBOSE(
            "Cannot convert weights to scalar. Note: Tensor dimensions were: "
            << weights.shape << ", with volume: " << volume(weights.shape));
        return nullptr;
    }

    return ctx->network()
        ->addConstant(nvinfer1::Dims{0, {0}}, weights)
        ->getOutput(0);
}

int divCeil(int n, int d) { return (n - 1) / d + 1; }

bool elementwiseCheck(const std::vector<TensorOrWeights> &inputs,
                      const nvinfer1::ElementWiseOperation op) {
    switch (op) {
    ...
    case nvinfer1::ElementWiseOperation::kXOR:
        if (!std::all_of(
                ...
                [](const TensorOrWeights &input) { return input.isBool(); })) {
            return false;
        }
        break;
    ...
    case nvinfer1::ElementWiseOperation::kPOW:
        if (std::any_of(inputs.begin(), inputs.end(),
                        [](const TensorOrWeights &input) {
                            return input.isBool() || input.isInt32();
                        })) {
            return false;
        }
        break;
...
NodeImportResult elementwiseHelper(IImporterContext *ctx,
    ...
    ASSERT(
        elementwiseCheck(inputs, binary_op) &&
    ...
    if (inputTensors.size() == 1) {
        // Note: Single input must be wrapped in identity to avoid messing up
        // network outputs
        return {{identity(ctx, combined)}};
    }
    for (size_t i = 1; i < inputTensors.size(); ++i) {
        nvinfer1::ITensor *tensor = inputTensors.at(i);
        ASSERT((tensor->getDimensions().nbDims ==
                combined->getDimensions().nbDims) &&
                   "The number of dimensions should remain the same adding "
                   "inputs.",
               ErrorCode::kUNSUPPORTED_NODE);
        auto *layer =
            ctx->network()->addElementWise(*combined, *tensor, binary_op);
        ctx->registerLayer(layer, node);
...
nvinfer1::ITensor *flattenTensor(IImporterContext *ctx,
                                 onnx::NodeProto const &node,
                                 nvinfer1::ITensor &tensor, int axis,
                                 bool regLayer) {
    const auto dims = shapeOf(tensor);
    const auto d0 = product(ctx, dims, 0, axis, 1);
    const auto d1 = product(ctx, dims, axis, dims.size(), 1);

    // ShuffleLayer here interprets dim extent 0 as empty dim to support empty
    // tensor
    nvinfer1::IShuffleLayer *flattenLayer = addShuffle(
        ctx, tensor, concat(ctx, d0, d1), /*zeroIsPlaceholder=*/false);
    if (regLayer) {
...
nvinfer1::ITensor *gatherDimension(IImporterContext *ctx,
                                   nvinfer1::ITensor *shapeTensor, int dim,
                                   nvinfer1::Dims shape) {
    auto &axisValue =
        *addConstantScalar(ctx, dim,
                           onnx::TensorProto_DataType_INT32, shape)
             ->getOutput(0);
...
void generatePadding(nvinfer1::Dims inputShape, nvinfer1::Dims outputShape,
                     nvinfer1::Dims kernelSize, nvinfer1::Dims strides,
                     nvinfer1::Dims dilations, const int nbSpatialDims,
                     nvinfer1::Dims &begPadding, nvinfer1::Dims &endPadding,
                     nvinfer1::Dims &outputPadding,
                     nvinfer1::PaddingMode paddingMode) {
    nvinfer1::Dims totalPadding{nbSpatialDims, {}};
    // Pre and post padding calculated as per
    // https://github.com/onnx/onnx/blob/master/docs/Operators.md#ConvTranspose
    // Note that output shape is inconsistent in the spec - can either be in
    // full dimensions form (i.e. NCHW) or just spatial dimensions form (i.e.
    // HW). Calculate potential offset here.
    auto const outputOffset = outputShape.nbDims - nbSpatialDims;
    for (int32_t i = 0; i < nbSpatialDims; i++) {
        totalPadding.d[i] = strides.d[i] * (inputShape.d[2 + i] - 1) +
                            outputPadding.d[i] +
                            ((kernelSize.d[i] - 1) * dilations.d[i] + 1) -
                            outputShape.d[outputOffset + i];
...
nvinfer1::ITensor *getAxisLength(IImporterContext *ctx,
                                 nvinfer1::ITensor *inpTensor, int axis,
                                 nvinfer1::Dims shape) {
    ...
    if (d >= 0) {
        return addConstantScalar(
                   ctx, d, onnx::TensorProto_DataType_INT32, shape)
            ->getOutput(0);
    } else {
        nvinfer1::ITensor *inpShape =
            ctx->network()->addShape(*inpTensor)->getOutput(0);
        return gatherDimension(ctx, inpShape, axis, shape);
    }
}

int getConvOutputSize(int input_size, int filter_size, int stride,
                      int dilation_rate, int total_padding);
...
void getKernelParams(IImporterContext *ctx,
                     onnx::NodeProto const &onnx_node,
                     nvinfer1::Dims *kernel_size, nvinfer1::Dims *strides,
                     nvinfer1::Dims *beg_padding, nvinfer1::Dims *end_padding,
                     nvinfer1::PaddingMode &paddingMode,
                     bool &count_exclude_padding, nvinfer1::Dims *dilations,
                     nvinfer1::Dims *output_padding,
                     const bool poolingCeilMode) {
    ...
    paddingMode = poolingCeilMode ? nvinfer1::PaddingMode::kEXPLICIT_ROUND_UP
    ...
    if (onnx_auto_pad != "SAME_LOWER" && onnx_auto_pad != "SAME_UPPER") {
        ...
        // If auto_pad is SAME_LOWER or SAME_UPPER, input padding should be
        // calculated "pads" attribute should not be specified
        assert((!attrs.count("pads")) &&
               "The attribute pads should not be specified.");
...
float getSingleValueAsFloat(void *data, bool fp16) {
    if (fp16) {
        return static_cast<float>(
            reinterpret_cast<half_float::half const *>(data)[0]);
    }
    return static_cast<float *>(data)[0];
}

nvinfer1::ITensor *globalPoolingHelper(IImporterContext *ctx,
                                       onnx::NodeProto const &node,
                                       nvinfer1::ITensor &tensor,
                                       nvinfer1::ReduceOperation op) {
    nvinfer1::Dims dims = tensor.getDimensions();
    // Generate a bitmask of all 1s except the last 2 bits (N and C axes)
    uint32_t reduceAxes = ((1 << dims.nbDims) - 1) & ~0b11;
    auto *layer = ctx->network()->addReduce(tensor, op, reduceAxes,
                                            /*keepDimensions=*/true);
    ctx->registerLayer(layer, node);
    return layer->getOutput(0);
}

nvinfer1::ITensor *greaterLessOrEqual(IImporterContext *ctx,
                                      const onnx::NodeProto &node,
                                      nvinfer1::ITensor *inputA,
                                      nvinfer1::ITensor *inputB, bool greater) {
    nvinfer1::ElementWiseOperation op =
        greater ? nvinfer1::ElementWiseOperation::kGREATER
                : nvinfer1::ElementWiseOperation::kLESS;
    nvinfer1::ITensor *firstOp =
        &elementwiseHelper(ctx, node, {inputA, inputB}, op)
             .value()
             .at(0)
             .tensor();
    nvinfer1::ITensor *equal =
        &elementwiseHelper(ctx, node, {inputA, inputB},
                           nvinfer1::ElementWiseOperation::kEQUAL)
             .value()
             .at(0)
             .tensor();
    nvinfer1::ITensor *result =
        &elementwiseHelper(ctx, node, {firstOp, equal},
...
importPluginCreator(IImporterContext *ctx, std::string const &pluginName,
    ...
    creator = pluginRegistry.getPluginCreator(
        pluginName.c_str(), pluginVersion.c_str(), pluginNamespace.c_str());
#endif  // ENABLE_STD_PLUGIN

#if ENABLE_SAFE_PLUGIN
    auto safetyPluginRegistry = nvinfer1::getBuilderSafePluginRegistry(
...
std::unique_ptr<nvinfer1::IPluginV2, PluginDeleter>
createPlugin(const std::string &name, nvinfer1::IPluginCreator *pluginCreator,
...
bool isDynamic(const nvinfer1::Dims &shape) {
    return std::any_of(shape.d, shape.d + shape.nbDims,
                       [](int dim) { return dim < 0; });
}

NodeImportResult
instanceNormPluginHelper(IImporterContext *ctx,
                         onnx::NodeProto const &node,
                         std::vector<TensorOrWeights> &inputs) {
    // Scales and biases must be initializers
    ASSERT(inputs.at(1).is_weights() &&
               "The scale tensor is required to be an initializer.",
           ErrorCode::kUNSUPPORTED_NODE);
    ASSERT(inputs.at(2).is_weights() &&
               "The bias tensor is required to be an initializer.",
           ErrorCode::kUNSUPPORTED_NODE);
    nvinfer1::ITensor *tensorPtr = &convertToTensor(inputs.at(0), ctx);
    int32_t nbDims = tensorPtr->getDimensions().nbDims;
    ASSERT(nbDims >= 3 && nbDims <= 5 &&
               "TensorRT only supports InstanceNormalization on 3D, 4D, or 5D "
               "tensors!",
           ErrorCode::kUNSUPPORTED_NODE);
    ...
    int32_t const relu{
        0};  // the ONNX instance norm op does not use the relu parameter
    float const alpha{
        0.F};  // the ONNX instance norm op does not use the alpha parameter
    ...
    f.emplace_back("scales", scale_weights.values,
                   nvinfer1::PluginFieldType::kFLOAT32, scale_weights.count());
    f.emplace_back("bias", bias_weights.values,
                   nvinfer1::PluginFieldType::kFLOAT32, bias_weights.count());
    ...
    ASSERT(plugin != nullptr && "InstanceNormalization plugin was not found in "
                                "the plugin registry!",
           ErrorCode::kUNSUPPORTED_NODE);
...
nvinfer1::ITensor *iota(IImporterContext *ctx, ShapeTensor iotaDims,
                        int32_t axis) {
    std::vector<int32_t> deltaVals(iotaDims.size(), 0);
    deltaVals[axis] = 1;
    auto *iota =
        ctx->network()->addFill({0, {0}}, nvinfer1::FillOperation::kLINSPACE);
    auto *alpha =
        addConstantScalar(ctx, 0, onnx::TensorProto::INT32)
            ->getOutput(0);
    auto *delta =
        addConstant(ctx, deltaVals, onnx::TensorProto::INT32,
                    {1, {iotaDims.size()}})
            ->getOutput(0);
...
bool isTransposeRequired(nvinfer1::Dims const &shape,
...
nvinfer1::Dims makeDims(int nbDims, int val) {
    // Zero all the dimensions, so that unused dimensions are deterministic even
...
NodeImportResult normalizationHelper(IImporterContext *ctx,
                                     const onnx::NodeProto &node,
                                     std::vector<TensorOrWeights> &inputs) {
    ...
    ASSERT(nbDims >= 3 && "Input to normalization should be at least 3D!",
           ErrorCode::kINVALID_NODE);

    // Need to broadcast scale and bias to the input shape. Note that normal
    // broadcasting rules cannot be applied as scale and bias are 1D and need to
    // be broadcasted to shape [1, S, 1, 1, ...].
    uint32_t axesMask{0};
...
nvinfer1::Dims insertDimension(const nvinfer1::Dims &dims, const int axis,
                               const int value) {
    assert(dims.nbDims < nvinfer1::Dims::MAX_DIMS &&
           axis < nvinfer1::Dims::MAX_DIMS);
...
std::vector<float> parseLSTMActivationValues(
    const std::vector<nvinfer1::ActivationType> &activationTypes,
    const std::vector<float> &activationValues, bool isAlpha) {
    size_t actIndex{0};
    std::vector<float> tmpActs{};
    for (size_t i = 0; i < activationTypes.size(); ++i) {
        float defaultVal = isAlpha
                               ? getActivationDefaultAlpha(activationTypes[i])
                               : getActivationDefaultBeta(activationTypes[i]);
...
std::string normalizePath(std::string const &path) {
    ...
    // Loop through path, split on all path seperator tokens, and append to
    // normPath if applicable.
    while (i < n) {
...
bool parseExternalWeights(IImporterContext *ctx, std::string file,
                          std::string path, int64_t offset, int64_t length,
                          std::vector<char> &weightsBuf, size_t &size) {
    // Accessing parent directories (i.e. ../) is not allowed. Normalize path
    // first.
    ...
    if (illegalDir) {
        LOG_ERROR("Relative paths to parent (../) are not allowed in ONNX "
                  "external weights! Normalized path is: "
                  << normalizedFile);
        return false;
    }
    // The weight paths in the ONNX model are relative paths to the main ONNX
    // file.
#ifdef _MSC_VER
...
NodeImportResult poolingHelper(IImporterContext *ctx,
                               onnx::NodeProto const &node,
                               std::vector<TensorOrWeights> &inputs,
    ...
    ASSERT(((nbSpatialDims == 1 && needToExpandDims) || nbSpatialDims == 2 ||
            nbSpatialDims == 3) &&
               "The attribute kernel_shape misaligns with the shape of the "
               "input tensor.",
           ErrorCode::kUNSUPPORTED_NODE);
    ...
    if (ctx->getOpsetVersion() >= 10) {
        ...
        const auto dilations =
            attrs.get<std::vector<int>>("dilations", std::vector<int>(2, 1));
        for (size_t i = 0; i < dilations.size(); i++)
            ASSERT((dilations[i] == 1) && "This version of TensorRT does not "
                                          "support dilations other than 1.",
                   ErrorCode::kUNSUPPORTED_NODE);  // Do not support pooling
                                                   // dilations currently
    }

    getKernelParams(ctx, node, &kernel_size, &strides, &beg_padding,
                    &end_padding, paddingMode, exclude_padding, nullptr,
                    nullptr, ceilMode);
    ...
    nvinfer1::IPoolingLayer *poolingLayer =
        ctx->network()->addPoolingNd(*tensorPtr, type, kernel_size);
...
NodeImportResult reduceTensor(IImporterContext *ctx,
    ...
    if (attrs.count("axes")) {
        ...
        ASSERT(inputAxes.is_weights() && "Axis input must be an initializer!",
               ErrorCode::kUNSUPPORTED_NODE);
...
nvinfer1::ITensor *reshapeTensor(IImporterContext *ctx,
                                 nvinfer1::ITensor &tensor,
                                 nvinfer1::Dims shape) {
...
NodeImportResult
scaleHelper(IImporterContext *ctx, const onnx::NodeProto &node,
            nvinfer1::ITensor &tensor_, nvinfer1::ScaleMode mode,
            const nvinfer1::Weights &shift, const nvinfer1::Weights &scale,
            const nvinfer1::Weights &power, const char *shiftName,
    ...
    if (needToReshape) {
        if (origShape.size() < 4) {
            ...
            const ShapeTensor collapsedDim =
                product(ctx, origShape, 3, origShape.size(), 1);
            const ShapeTensor collapsedShape = concat(
                ctx, gather(ctx, origShape, iotaShapeVector(3)), collapsedDim);
            tensorPtr = &reshape(ctx, *tensorPtr, collapsedShape);
        }
    }

    auto *layer =
        ctx->network()->addScaleNd(*tensorPtr, mode, shift, scale, power, 1);
    ASSERT(layer && "Failed to add a Scale layer.",
           ErrorCode::kUNSUPPORTED_NODE);
    ...
    if (needToReshape) {
        ...
        ASSERT(tensorPtr && "Failed to reshape tensor.",
               ErrorCode::kUNSUPPORTED_NODE);
    }
    return {{tensorPtr}};
}

void setAttr(nvinfer1::Dims *trtAttr,
             onnx::AttributeProto const *onnxAttr,
             int nbSpatialDims, int defaultVal) {
...
nvinfer1::ITensor *sliceAcrossAxis(IImporterContext *ctx,
                                   const onnx::NodeProto &node,
                                   nvinfer1::ITensor *data, const int axis) {
    ShapeTensor starts, sizes, strides;
    ShapeTensor axisLength =
        ShapeTensor(*getAxisLength(ctx, data, axis, {1, {1}}));
    const int nbDims = data->getDimensions().nbDims;

    std::vector<int64_t> values(nbDims, 0);
    starts = ShapeTensor(1, std::move(values));
    sizes = axis == 0 ? shapeVector(1)
                      : ShapeTensor(*getAxisLength(ctx, data, 0, {1, {1}}));
    ...
    for (int i = 1; i < nbDims; i++) {
        if (i == axis) {
            ...
            ShapeTensor currLength =
                ShapeTensor(*getAxisLength(ctx, data, i, {1, {1}}));
...
nvinfer1::ITensor *squeezeTensor(IImporterContext *ctx,
                                 const onnx::NodeProto &node,
                                 nvinfer1::ITensor &tensor,
                                 const std::vector<int> &axes, bool regLayer) {
    const auto dims = shapeOf(tensor);

    // Set subscripts to ShapeTensor containing positions of axes to be kept.
    // For example, if there are 6 dimensions and axes = {1,5}, set subscripts
    // to {0,2,3,4}.
    std::vector<int64_t> subscripts(dims.size());
    std::iota(subscripts.begin(), subscripts.end(), 0);
    auto p =
        std::remove_if(subscripts.begin(), subscripts.end(), [axes](int x) {
            return std::find(axes.begin(), axes.end(), x) != axes.end();
        });
...
nvinfer1::ITensor *transposeTensor(IImporterContext *ctx,
                                   const onnx::NodeProto &node,
                                   nvinfer1::ITensor &tensor,
                                   nvinfer1::Permutation const &perm) {
...
NodeImportResult unaryHelper(IImporterContext *ctx,
    ...
    case nvinfer1::UnaryOperation::kNEG: {
        ...
        if (inputType == nvinfer1::DataType::kINT32) {
            // Calculate the rank of the input, and set all size to one and rely
            // on broadcasting
            nvinfer1::ITensor *zeroTensor =
                addConstant(ctx, std::vector<int32_t>{0},
                            onnx::TensorProto::INT32, {0, {1}})
                    ->getOutput(0);
            CHECK(broadcastTensors(ctx, zeroTensor, tensorPtr));
            std::vector<TensorOrWeights> layerInputs = {zeroTensor, tensorPtr};
            return elementwiseHelper(ctx, node, layerInputs,
                                     nvinfer1::ElementWiseOperation::kSUB);
        }
        validUnaryType = (inputType != nvinfer1::DataType::kBOOL &&
                          inputType != nvinfer1::DataType::kUINT8);
    ...
    case nvinfer1::UnaryOperation::kISINF: {
        validUnaryType = (inputType == nvinfer1::DataType::kFLOAT ||
                          inputType == nvinfer1::DataType::kHALF);
        break;
    }
    default: {
        // By default TRT does not support BOOL, INT32, UINT8 types for Unary
        // operations.
        validUnaryType = (inputType != nvinfer1::DataType::kBOOL &&
                          inputType != nvinfer1::DataType::kINT32 &&
                          inputType != nvinfer1::DataType::kUINT8);
    }
    }

    ASSERT(validUnaryType && "This version of TensorRT does not support the "
                             "given operator with the given input data type.",
           ErrorCode::kUNSUPPORTED_NODE);
...
NodeImportResult convMultiInput(IImporterContext *ctx,
                                const onnx::NodeProto &node,
                                std::vector<TensorOrWeights> &inputs) {
    ASSERT(inputs.size() >= 2 && "Convolution require at least 2 inputs.",
           ErrorCode::kUNSUPPORTED_NODE);
    ...
    if (needToExpandDims) {
        ...
        ASSERT(input_tensor_ptr && "Failed to unsqueeze tensor.",
               ErrorCode::kUNSUPPORTED_NODE);
    ...
    getKernelParams(ctx, node, &filter_dim, &strides, &beg_padding,
                    &end_padding, paddingMode, exclude_padding, &dilations);
    ...
    auto const checkSpatialDims = [&nbSpatialDims,
                                   &filter_dim](nvinfer1::Dims const &dims) {
        // Check that the number of spatial dimensions and the kernel shape
        // matches up.
        if (nbSpatialDims != dims.nbDims - 2) {
            return false;
        }

        return std::equal(filter_dim.d, filter_dim.d + nbSpatialDims,
                          dims.d + dims.nbDims - nbSpatialDims);
    };
    ...
    if (inputs.at(1).is_tensor()) {
        ...
        if (needToExpandDims) {
            ...
            std::vector<int32_t> const axes{3};
            kernel_tensor_ptr =
                unsqueezeTensor(ctx, node, *kernel_tensor_ptr, axes);
            ASSERT(kernel_tensor_ptr && "Failed to unsqueeze tensor.",
                   ErrorCode::kUNSUPPORTED_NODE);
        }
        ASSERT(
            checkSpatialDims(kernel_tensor_ptr->getDimensions()) &&
        ...
        ASSERT(
            checkSpatialDims(kernel_weights.shape) &&
                "The input tensor shape misaligns with the input kernel shape.",
            ErrorCode::kUNSUPPORTED_NODE);
    }
    ...
    ASSERT((nChannel == -1 || C * ngroup == nChannel) &&
               "The attribute group and the kernel shape misalign with the "
               "channel size of the input tensor. ",
           ErrorCode::kINVALID_NODE);

    nvinfer1::IConvolutionLayer *layer = ctx->network()->addConvolutionNd(
        *input_tensor_ptr, K, filter_dim, kernel_weights, bias_weights);
    ASSERT(layer && "Failed to add the Convolution layer.",
           ErrorCode::kUNSUPPORTED_NODE);
    ...
    if (needToExpandDims) {
        ...
        ASSERT(output_tensor_ptr && "Failed to unsqueeze tensor.",
               ErrorCode::kUNSUPPORTED_NODE);
    }
...
nvinfer1::ITensor *unsqueezeTensor(IImporterContext *ctx,
                                   const onnx::NodeProto &node,
                                   nvinfer1::ITensor &tensor,
    ...
    const auto newDims = interlace(ctx, dims, shapeVector(1),
                                   ShapeTensor(1, std::move(subscripts)));
...
nvinfer1::ITensor *resizeShapeTensor(IImporterContext *ctx,
                                     nvinfer1::ITensor &input,
                                     TensorOrWeights &scales) {
    ...
    auto *prod = ctx->network()
                     ->addElementWise(scaleTensor, *inputShapeTensor,
                                      nvinfer1::ElementWiseOperation::kPROD)
                     ->getOutput(0);
    auto *floor = ctx->network()
                      ->addUnary(*prod, nvinfer1::UnaryOperation::kFLOOR)
                      ->getOutput(0);
...
int64_t volume(const nvinfer1::Dims &dims) {
    std::for_each(dims.d, dims.d + dims.nbDims, [](int32_t d) {
        assert(d >= 0 && "volume makes no sense for dynamic shapes");
    });
...
//! Return ShapeTensor representing x clamped to closed interval
//! [lowerBound,upperBound].
static ShapeTensor clamp(IImporterContext *ctx, const ShapeTensor &x,
                         const ShapeTensor &lowerBound,
...
static ShapeTensor bumpIfNegative(IImporterContext *ctx,
                                  const ShapeTensor &inputDims,
...
void decodeOnnxStartsAndEnds(IImporterContext *ctx,
                             const ShapeTensor &inputDims,
                             const ShapeTensor &steps, ShapeTensor &starts,
                             ShapeTensor &ends) {
    //! The ONNX specification is unreliable
    //! (https://github.com/onnx/onnx/issues/3063) thus the logic here is
    //! designed to match that in
...
ShapeTensor computeSliceSizes(IImporterContext *ctx, const ShapeTensor &starts,
                              const ShapeTensor &ends, const ShapeTensor &steps,
                              const ShapeTensor &dims) {
    if (steps.isAll(1)) {
    ...
    // Compute ceil((end-start)/step) using only operations available on
    // ShapeTensor, using the identity ceil(x) = -floor(-x).
    return sub(ctx, similar(ctx, dims, 0),
               floorDiv(ctx, sub(ctx, starts, ends), steps));
}

nvinfer1::ITensor *addSoftmax(IImporterContext *ctx,
                              const onnx::NodeProto &node,
                              nvinfer1::ITensor &input) {
    OnnxAttrs attrs(node, ctx);
    ...
    if (ctx->getOpsetVersion() >= 13) {
        ...
        // "The input does not need to explicitly be a 2D vector; rather, it
...
NodeImportResult addScatterLayer(IImporterContext *ctx,
                                 onnx::NodeProto const &node,
                                 std::vector<TensorOrWeights> &inputs,
                                 nvinfer1::ScatterMode mode, int32_t axis) {
    CHECK(notInvalidType(inputs.at(0), {"UINT8"}));
    ...
    if (mode == nvinfer1::ScatterMode::kELEMENT) {
        ...
        ASSERT(
            dataDims.nbDims == indicesDims.nbDims &&
                dataDims.nbDims == updatesDims.nbDims &&
                "Input dimensions to ScatterElements must have the same rank!",
            ErrorCode::kUNSUPPORTED_NODE);
            ...
            if (indicesDims.d[i] != -1 && dataDims.d[i] != -1) {
                ASSERT(
                    indicesDims.d[i] <= dataDims.d[i] &&
                        "Indices dimensions must be less than data dimensions!",
                    ErrorCode::kUNSUPPORTED_NODE);
            }
            if (updatesDims.d[i] != -1 && dataDims.d[i] != -1) {
                ASSERT(
                    updatesDims.d[i] <= dataDims.d[i] &&
                        "Updates dimensions must be less than data dimensions!",
                    ErrorCode::kUNSUPPORTED_NODE);
            }
        }
    }
...
nvinfer1::IElementWiseLayer *modWithIntegerInputs(IImporterContext *ctx,
                                                  nvinfer1::ITensor *input0,
    ...
    return ctx->network()->addElementWise(
        *input0,
        *ctx->network()
             ->addElementWise(*input1,
                              *ctx->network()
                                   ->addElementWise(*input0, *input1, divOp)
                                   ->getOutput(0),
                              eOp::kPROD)
             ->getOutput(0),
        eOp::kSUB);
...
nvinfer1::IElementWiseLayer *modWithFPInputs(IImporterContext *ctx,
                                             nvinfer1::ITensor *input0,
                                             nvinfer1::ITensor *input1,
                                             nvinfer1::ITensor *divResult,
                                             bool sameSign) {
    using eOp = nvinfer1::ElementWiseOperation;
...
float *convertFP16Data(void *weightValues, nvinfer1::Dims shape,
                       IImporterContext *ctx) {
    size_t const nbWeights = volume(shape);
    float *newWeights{static_cast<float *>(
        ctx->createTempWeights(onnx::TensorProto::FLOAT, shape)
            .values)};

    half_float::half *tempValues =
        static_cast<half_float::half *>(weightValues);
...
std::string filterDocString(std::string const &docString) {
    ...
    // The doc strings that PyTorch lib generates contain the literal
    // `site-packages` or `dist-packages`. We filter such lines out to keep only
    // the doc strings of the user-programmed codes.
    ...
    for (auto &line : lines) {
        ...
        if (writeLine) {
            ...
            // For that reason, we change it to a single-quote substring, if
            // any.
...
Status processMetadata(onnx::NodeProto const &node,
    ...
    ASSERT((layer != nullptr) && "The layer object does not exist.",
           ErrorCode::kUNSUPPORTED_NODE);
