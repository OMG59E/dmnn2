/***
 * @Author: xingwg
 * @Date: 2024-12-12 16:13:01
 * @LastEditTime: 2024-12-12 16:13:52
 * @FilePath: /dmnn2/src/parsers/onnx/ImporterContext.hpp
 * @Description:
 * @
...
public:
    ErrorRecorderWrapper(nvinfer1::INetworkDefinition *network,
                         nvinfer1::ILogger *logger)
        : mNetwork(network), mLogger(logger) {
        if (mNetwork) {
            mUserErrorRecorder = mNetwork->getErrorRecorder();
            mOnnxErrorRecorder =
                ONNXParserErrorRecorder::create(logger, mUserErrorRecorder);
    ...
    bool hasError() const {
        return mOnnxErrorRecorder != nullptr &&
               mOnnxErrorRecorder->getNbErrors() != 0;
    }
...
class ImporterContext final : public IImporterContext {
    ...
    StringMap<TensorOrWeights>
        mTensors;  // All tensors in the graph mapped to their names.
    ...
    std::set<std::string>
        mTensorNames;  // Keep track of how many times a tensor name shows up,
                       // to avoid duplicate naming in TRT.
    std::set<std::string>
        mLayerNames;  // Keep track of how many times a tensor name shows up, to
                      // avoid duplicate naming in TRT.
    int64_t mSuffixCounter{
        0};  // increasing suffix counter used to uniquify layer names.
    std::unordered_set<std::string>
        mUnsupportedShapeTensors;  // Container to hold output tensor names of
                                   // layers that produce shape tensor outputs
                                   // but do not natively support them.
    StringMap<std::string>
        mLoopTensors;  // Container to map subgraph tensors to their original
                       // outer graph names.
    std::string mOnnxFileLocation;  // Keep track of the directory of the parsed
                                    // ONNX file
    std::unique_ptr<ErrorRecorderWrapper>
        mErrorWrapper;  // error recorder to control TRT errors
    ...
    nvonnxparser::OnnxParserFlags
        mOnnxParserFlags;  // OnnxParserFlags specified by the parser

    // Logical library names for VC plugin libraries.  This gets translated to
    // library paths when getUsedVCPluginLibraries() is called.
    std::set<std::string> mLogicalVCPluginLibraries;

    //! Stack of names defined by nested ONNX graphs, with information about how
    //! to restore their associated values when popping back to the surrounding
    //! scope.
    //!
    ...
    //!
    std::vector<StringMap<std::pair<bool, TensorOrWeights>>>
        mBaseNameScopeStack;
...
public:
    ImporterContext(nvinfer1::INetworkDefinition *network,
        ...
        : mNetwork(network), mLogger(logger),
          mErrorWrapper(
              std::make_unique<ErrorRecorderWrapper>(mNetwork, logger)) {}
    nvinfer1::INetworkDefinition *network() override { return mNetwork; }
    StringMap<TensorOrWeights> &tensors() override { return mTensors; }
    StringMap<nvinfer1::TensorLocation> &tensorLocations() override {
        return mTensorLocations;
    }
    StringMap<float> &tensorRangeMins() override { return mTensorRangeMins; }
    StringMap<float> &tensorRangeMaxes() override { return mTensorRangeMaxes; }
    ...
    StringMap<std::string> &loopTensors() override { return mLoopTensors; }
    void setOnnxFileLocation(std::string location) override {
        mOnnxFileLocation = location;
    }
    std::string getOnnxFileLocation() override { return mOnnxFileLocation; }
    ...
    void registerTensor(TensorOrWeights tensor, std::string const &basename,
                        bool const checkUniqueName = false) override;

    void registerLayer(nvinfer1::ILayer *layer, std::string const &basename,
                       onnx::NodeProto const *node) override;
    void registerLayer(nvinfer1::ILayer *layer,
                       onnx::NodeProto const &node) override;

    nvinfer1::ILogger &logger() override { return *mLogger; }

    ShapedWeights createTempWeights(ShapedWeights::DataType type,
                                    nvinfer1::Dims shape,
                                    uint8_t value = 0) override {
        std::string const &name =
            generateUniqueName(mTensorNames, "tmp_weight");
    ...
    void clearOpsets() { mOpsets.clear(); }
    ...
    void
    setFlags(nvonnxparser::OnnxParserFlags const &onnxParserFlags) override {
    ...
    virtual void addUsedVCPluginLibrary(onnx::NodeProto const &node,
                                        ...
                                        char const *pluginLib) final;

    virtual std::vector<std::string> getUsedVCPluginLibraries() final;

    bool isConvertINT64Logged() { return mConvertINT64Logged; }
    void setConvertINT64Logged(bool logged) { mConvertINT64Logged = logged; }
    ...
    bool isConvertDoubleLogged() { return mConvertDoubleLogged; }
    void setConvertDoubleLogged(bool logged) { mConvertDoubleLogged = logged; }
...
private:
    std::string const &generateUniqueName(std::set<std::string> &namesSet,
        ...
        // Return reference to newly inserted string to avoid any c_str()'s
        // going out of scope
