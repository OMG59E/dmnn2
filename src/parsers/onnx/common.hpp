/***
 * @Author: xingwg
 * @Date: 2024-12-12 15:18:53
 * @LastEditTime: 2024-12-12 15:34:39
 * @FilePath: /dmnn2/src/parsers/common.hpp
 * @Description:
 * @
...
public:
    ...
    void log(Severity severity, const char *msg) noexcept override {
        if (severity <= _verbosity) {
            ...
            const char *sevstr =
                (severity == Severity::kINTERNAL_ERROR ? "    BUG"
                 : severity == Severity::kERROR        ? "  ERROR"
                 : severity == Severity::kWARNING      ? "WARNING"
                 : severity == Severity::kINFO         ? "   INFO"
                                                       : "UNKNOWN");
            (*_ostream) << "[" << buf << " " << sevstr << "] " << msg
                        << std::endl;
        }
    }
};
...
inline void print_version() {
    ...
    std::cout << "  ONNX IR version:  "
              << onnx_ir_version_string(onnx::IR_VERSION) << std::endl;
