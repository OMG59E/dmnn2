/***
 * @Author: xingwg
 * @Date: 2024-12-13 09:45:19
 * @LastEditTime: 2024-12-13 09:45:40
 * @FilePath: /dmnn2/src/parsers/onnx/NvOnnxParser.h
 * @Description:
 * @
...
enum class OnnxParserFlag : int32_t {
    //! Parse the ONNX model into the INetworkDefinition with the intention of
    //! using TensorRT's native layer implementation over the plugin
    //! implementation for InstanceNormalization nodes. This flag is planned to
    //! be deprecated in TensorRT 8.7 and removed in TensorRT 9.0. This flag is
    //! required when building version-compatible or hardware-compatible
    //! engines. There may be performance degradations when this flag is
    //! enabled.
...
public:
    ...
    //!         This method has very limited diagnostics. If parsing the
    //!         serialized model fails for any reason (e.g. unsupported IR
    //!         version, unsupported opset, etc.) it the user responsibility to
    //!         intercept and report the error. To obtain a better diagnostic,
    //!         use the parseFromFile method below.
    //!
    //! \param serialized_onnx_model Pointer to the serialized ONNX model
    //! \param serialized_onnx_model_size Size of the serialized ONNX model
    //!        in bytes
    //! \param model_path Absolute path to the model file for loading external
    //! weights if required
    ...
    //!
    //! \brief Parse an onnx model file, which can be a binary protobuf or a
    //! text onnx model
    ...
    //! 	       If the function returns True, one can proceed to engine
    //! building 	       without having to call \p parse or \p parseFromFile.
    //!
    //! \param serialized_onnx_model Pointer to the serialized ONNX model
    //! \param serialized_onnx_model_size Size of the serialized ONNX model
    //!        in bytes
    ...
    //!
    //! \brief Query the plugin libraries needed to implement operations used by
    //! the parser in a version-compatible engine.
    //!
    //! This provides a list of plugin libraries on the filesystem needed to
    //! implement operations in the parsed network.  If you are building a
    //! version-compatible engine using this network, provide this list to
    //! IBuilderConfig::setPluginsToSerialize to serialize these plugins along
    //! with the version-compatible engine, or, if you want to ship these plugin
    //! libraries externally to the engine, ensure that
    //! IPluginRegistry::loadLibrary is used to load these libraries in the
    //! appropriate runtime before deserializing the corresponding engine.
    //!
    //! \param[out] nbPluginLibs Returns the number of plugin libraries in the
    //! array, or -1 if there was an error.
    //! \return Array of `nbPluginLibs` C-strings describing plugin library
    //! paths on the filesystem if nbPluginLibs > 0, or nullptr otherwise.  This
    //! array is owned by the IParser, and the pointers in the array are only
    ...
    //!
    //! \note This function will override the previous set flags, rather than
    //! bitwise ORing the new flag.
    //!
    //! \see getFlags()
    //!
...
//! TensorRT, to a constant is okay if the constant is consistent with the
//! model. Each instance of the parser is designed to only parse one ONNX model
//! once.
//!
//! \see IParser
//!
