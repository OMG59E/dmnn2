/***
 * @Author: xingwg
 * @Date: 2024-12-12 17:45:54
 * @LastEditTime: 2024-12-14 19:56:21
 * @FilePath: /dmnn2/src/parsers/onnx/onnx_utils.hpp
 * @Description:
 * @
...
public:
    ...
    NamedDimension(int32_t index_, const std::string &dimParam_)
        : tensor(nullptr), index(index_), dimParam(dimParam_) {}
};
...
bool convertOnnxDims(OnnxDims const &onnxDims, nvinfer1::Dims &trtDims,
    ...
    for (const auto &onnxDim : onnxDims) {
        // For empty dimensions, the ONNX specification says it's a dynamic
        // dimension
        if (!onnxDim.has_dim_value() && !onnxDim.has_dim_param()) {
            onnxDimsVec.emplace_back(-1);
        } else {
            if (!onnxDim.dim_param().empty()) {
                namedDims.emplace_back(static_cast<int32_t>(onnxDimsVec.size()),
                                       onnxDim.dim_param());
            }
            const int32_t dim =
                onnxDim.dim_param() == ""
                    ? (onnxDim.dim_value() >= 0 ? onnxDim.dim_value() : -1)
                    : -1;
...
// Removes float_data, int32_data etc. from the text representation of an ONNX
// model
...
inline std::string
pretty_print_onnx_to_string(::google::protobuf::Message const &message) {
    std::string s;
...
inline std::ostream &operator<<(std::ostream &stream,
                                onnx::ModelProto const &message) {
    stream << pretty_print_onnx_to_string(message);
    return stream;
}

inline std::ostream &operator<<(std::ostream &stream,
                                onnx::NodeProto const &message) {
    stream << pretty_print_onnx_to_string(message);
    return stream;
}
...
inline std::string
onnx_ir_version_string(int64_t ir_version = onnx::IR_VERSION) {
