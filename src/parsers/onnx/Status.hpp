/***
 * @Author: xingwg
 * @Date: 2024-12-12 15:37:47
 * @LastEditTime: 2024-12-12 16:03:12
 * @FilePath: /dmnn2/src/parsers/Status.hpp
 * @Description:
 * @
...
#define MAKE_ERROR(desc, code)                                                 \
    onnx2trt::Status((code), (desc), __FILENAME__, __LINE__, __func__)
...
#define ASSERT_INPUT(condition, error_code, name)                              \
    do {                                                                       \
        if (!(condition)) {                                                    \
            return MAKE_INPUT_ERROR("Assertion failed: " #condition,           \
                                    (error_code), (name));                     \
        }                                                                      \
    } while (0)
...
template <typename T> class ValueOrStatus {
    bool _is_error;
    T _value;
    Status _error;
...
public:
    ValueOrStatus(T const &value)
        : _is_error(false), _value(value), _error(Status::success()) {}
