/***
 * @Author: xingwg
 * @Date: 2024-12-12 16:17:25
 * @LastEditTime: 2024-12-12 16:17:57
 * @FilePath: /dmnn2/src/parsers/onnx/onnx2trt_utils.hpp
 * @Description:
 * @
...
template <typename T>
static std::ostream &printSequence(std::ostream &stream, const T *begin,
                                   int count) {
    stream << "(";
    if (count > 0) {
...
static std::ostream &operator<<(std::ostream &stream,
                                const nvinfer1::Permutation &perm) {
...
inline nvinfer1::IConstantLayer *
addConstantScalar(IImporterContext *ctx, ScalarType scalar,
                  ShapedWeights::DataType type,
                  nvinfer1::Dims shape = nvinfer1::Dims{0}) {
    assert(volume(shape) == 1 &&
           "Cannot add constant scalar with a shape that has volume > 1");
    ShapedWeights scalarWeights = ctx->createTempWeights(type, shape);
    static_cast<ScalarType *>(scalarWeights.values)[0] =
        static_cast<ScalarType>(scalar);
    nvinfer1::IConstantLayer *l =
        ctx->network()->addConstant(scalarWeights.shape, scalarWeights);
...
template <typename ScalarType>
inline nvinfer1::IConstantLayer *
addConstant(IImporterContext *ctx, const std::vector<ScalarType> &values,
            ShapedWeights::DataType type, nvinfer1::Dims shape) {
    assert(volume(shape) == static_cast<int64_t>(values.size()) &&
           "Shape does not match number of values provided");
    assert(sizeof(ScalarType) == getDtypeSize(type) &&
           "ONNX dtype does not have the same size as the value type");
    ShapedWeights weights = ctx->createTempWeights(type, shape);
    std::memcpy(weights.values, values.data(),
                values.size() * sizeof(ScalarType));
    nvinfer1::IConstantLayer *l =
...
//! If t has rank less than nbDims, reshape it to have nbDims by prepending ones
//! to its dimensions. Assert failure if t has rank greater than nbDims.
...
// Helper function to calculate and return a vector representation of the
// pitches of a given shape
...
// Helper function for constantOfShape operator. Input shape must be a shape
// tensor
...
// Helper function to convert ONNX padding into TRT padding. Will update
// startTensor and totalPaddingTensor by reference
...
// Helper function to create a tensor of all zeros with the same shape as a data
// tensor
...
// Helper function to check that the input data types for an elementwise
...
nvinfer1::ITensor *flattenTensor(IImporterContext *ctx,
                                 onnx::NodeProto const &node,
                                 nvinfer1::ITensor &tensor, int axis = 0,
                                 bool regLayer = false);

// Gathers the specified dimension from a shape tensor. e.g.
// gatherDimension(shape=(7, 6, 5), dim=2) would return 5. shape specifies the
// shape of the returned Tensor. Must have a volume of 1.
...
// Helper function to calculate the output size of a convolution node given its
// attributes
...
void getKernelParams(IImporterContext *ctx, onnx::NodeProto const &onnx_node,
                     ...
                     const bool poolingCeilMode = false);

// Helper function to get the scaling mode for TRT's scale layer
nvinfer1::ScaleMode getScaleMode(nvinfer1::Dims const &weights_shape,
                                 nvinfer1::Dims const &tensor_shape);

// Helper function to get a float representation of a FLOAT or FLOAT16 ONNX
// type.
...
// Helper function to create a greaterOrEqual or lessOrEqual operation. Provide
// `greater=true` for greaterOrEqual, `greater=false` for lessOrEqual
...
NodeImportResult instanceNormPluginHelper(IImporterContext *ctx,
                                          onnx::NodeProto const &node,
                                          std::vector<TensorOrWeights> &inputs);
...
NodeImportResult reduceTensor(IImporterContext *ctx,
                              ...
                              TensorOrWeights inputAxes = TensorOrWeights());
...
NodeImportResult scaleHelper(IImporterContext *ctx, const onnx::NodeProto &node,
                             nvinfer1::ITensor &tensor_,
                             nvinfer1::ScaleMode mode,
                             const nvinfer1::Weights &shift,
                             const nvinfer1::Weights &scale,
                             const nvinfer1::Weights &power,
                             const char *shiftName, const char *scaleName);

// Helper function to set an ONNX attribute
void setAttr(nvinfer1::Dims *trtAttr, onnx::AttributeProto const *onnxAttr,
...
NodeImportResult unaryHelper(IImporterContext *ctx, const onnx::NodeProto &node,
                             TensorOrWeights &input,
...
// Helper function to calculate and return the expected output shape of a resize
// given the resize scale weights or scale tensor.
...
Status weightsToVector(TensorOrWeights weights,
                       std::vector<WeightType> *weightVector) {
    ...
    if (weights.weights().type == onnx::TensorProto::INT64) {
        ...
        std::copy(array_start, array_start + weights.weights().count(),
                  weightVector->begin());
...
template <typename T> ShapedWeights::DataType getShapeWeightsDataType() {
    static const std::unordered_map<std::type_index,
                                    onnx::TensorProto::DataType>
        tMap({{std::type_index(typeid(bool)), onnx::TensorProto::BOOL},
...
Status vectorToWeights(std::vector<T> &weightVector, TensorOrWeights *weights) {
    ...
    ASSERT(dtype != onnx::TensorProto::UNDEFINED && "Unsupported datatype",
           ErrorCode::kINVALID_VALUE);
    *weights = ShapedWeights(dtype, weightVector.data(), shape);
    return Status(ErrorCode::kSUCCESS);
}

// Helper function to convert ONNX node name. If no node name, using name of
// first output.
...
NodeImportResult addScatterLayer(IImporterContext *ctx,
                                 const onnx::NodeProto &node,
                                 std::vector<TensorOrWeights> &inputs,
                                 nvinfer1::ScatterMode mode, int32_t axis = 0);
...
//! RAII wrapper for IImporterContext::pushBaseNameScope() and
//! popBaseNameScope().
class NameScope {
public:
    NameScope(IImporterContext &context) : mContext(context) {
        mContext.pushBaseNameScope();
    }
    ~NameScope() { mContext.popBaseNameScope(); }
