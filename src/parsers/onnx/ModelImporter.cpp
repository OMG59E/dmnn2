/***
 * @Author: xingwg
 * @Date: 2024-12-13 09:49:52
 * @LastEditTime: 2024-12-13 14:35:16
 * @FilePath: /dmnn2/src/parsers/onnx/ModelImporter.cpp
 * @Description:
 * @
...
Status parseGraph(IImporterContext *ctx, onnx::GraphProto const &graph,
    ...
    for (auto const &nodeIndex : topoOrder) {
        ...
        for (auto const &inputName : node.input()) {
            // Empty input names indicate optional inputs which have not been
            // supplied.
            if (inputName.empty()) {
        ...
        for (auto const &output : outputs) {
            if (output.is_tensor()) {
                ...
                // in the future we may have a network/layer.validate() which
        ...
        for (int32_t i = 0; i < node.output().size(); ++i) {
            ...
            // mapped to an unused input
            if ((output || output.is_weights()) && !outputName.empty()) {
                ctx->registerTensor(std::move(output), outputName);
            }
            // UINT8 is only allowed as network inputs and outputs. Therefore
            // any node that produces an UINT8-typed output that is not also a
            // graph output is unsupported.
...
bool ModelImporter::supportsModel(void const *serialized_onnx_model,
    ...
    if (!allSupported) {
        ...
        for (int32_t i = 0; i < nerror; ++i) {
            ...
            // The node that we failed on is one of the input nodes (-1). Get
            // the name of the input node that we failed on and remove all nodes
            // that spawn out of it.
            else {
                // Node name is extracted through error->file as all errors
                // thrown on input nodes are wrapped around MAKE_INPUT_ERROR.
    ...
    for (int32_t node_idx : topological_order) {
        ...
        if (!unsupportedInput && !unsuccessfulParse) {
            if (newSubGraph) {
                // If it is the beginning of a new subGraph, we start a new
                // vector
...
Status ModelImporter::importModel(onnx::ModelProto const &model) {
    ASSERT(!mImporterCtx.network()->hasImplicitBatchDimension() &&
               "This version of the ONNX parser only supports TensorRT "
               "INetworkDefinitions with an explicit batch dimension. Please "
               "ensure the network was created using the EXPLICIT_BATCH "
    ...
    for (int32_t i = 0; i < model.opset_import().size(); ++i) {
        ...
        // TensorRT requires an ONNX graph to be generated with at least ai.onnx
    ...
    // Create a dummy tensors so that we can reserve output names. If the output
    // names are encountered elsewhere in the graph, the ctx will know to make
    // the names unique.
