 ...
 * @LastEditTime: 2024-12-13 14:33:53
 * @FilePath: /dmnn2/src/parsers/onnx/builtin_op_importers.cpp
 * @Description:
 * @
...
#define DEFINE_BUILTIN_OP_IMPORTER(op)                                         \
    NodeImportResult import##op(IImporterContext *ctx,                         \
                                onnx::NodeProto const &node,                   \
                                std::vector<TensorOrWeights> &inputs);         \
    static bool const op##_registered_builtin_op =                             \
...
#define RETURN_FIRST_OUTPUT(layer)                                             \
    do {                                                                       \
        ...
        ASSERT(layer_ptr && "Input layer is null.",                            \
...
#define RETURN_IDENTITY(input)                                                 \
    do {                                                                       \
        ...
        ASSERT(output && "Failed to add an identity layer.",                   \
               ErrorCode::kUNSUPPORTED_NODE);                                  \
        return {{output}};                                                     \
    } while (0)
...
#define RETURN_ALL_OUTPUTS(layer)                                              \
    do {                                                                       \
        nvinfer1::ILayer *layer_ptr = layer;                                   \
        ASSERT(layer_ptr && "The input layer is null.",                        \
               ErrorCode::kUNSUPPORTED_NODE);                                  \
...
void assertIsWeights(TensorOrWeights const &input,
                     std::string const &specificMsg) {
    if (!input.is_weights()) {
        std::ostringstream msg;
        msg << specificMsg;
        msg << " Try applying constant folding on the model using Polygraphy: "
               "https://github.com/NVIDIA/TensorRT/tree/master/tools/"
               "Polygraphy/examples/cli/surgeon/02_folding_constants";
...
DEFINE_BUILTIN_OP_IMPORTER(Acos) {
    return unaryHelper(ctx, node, inputs.at(0),
...
DEFINE_BUILTIN_OP_IMPORTER(And) {
    return elementwiseHelper(ctx, node, inputs,
...
NodeImportResult batchnormFallback(IImporterContext *ctx,
                                   onnx::NodeProto const &node,
                                   std::vector<TensorOrWeights> &inputs) {
    ...
    nvinfer1::ITensor *epsilon =
        addConstantScalar(ctx, eps, onnx::TensorProto::FLOAT, scalarShape)
            ->getOutput(0);

    // batchnorm = scale * (input - mean) / sqrt(variance + epsilon) + bias
    // The WAR is split the single c++ code line into 3 to avoid the sequence
    // swap by compiler.
    nvinfer1::ITensor *divisor =
        ctx->network()
            ->addUnary(*ctx->network()
                            ->addElementWise(*variance, *epsilon, eOp::kSUM)
                            ->getOutput(0),
                       uOp::kSQRT)
            ->getOutput(0);
    nvinfer1::ITensor *dividend =
    ...
    nvinfer1::IElementWiseLayer *layer = ctx->network()->addElementWise(
        *ctx->network()
             ->addElementWise(
                 *scale,
                 *ctx->network()
                      ->addElementWise(*dividend, *divisor, eOp::kDIV)
                      ->getOutput(0),
                 eOp::kPROD)
             ->getOutput(0),
        *bias, eOp::kSUM);
...
DEFINE_BUILTIN_OP_IMPORTER(BatchNormalization) {
    ASSERT((inputs.at(1).shape().nbDims == 1) &&
               "The shape of the scale input must be (C, )",
           ErrorCode::kINVALID_NODE);
    ASSERT((inputs.at(2).shape().nbDims == 1) &&
               "The shape of the bias input must be (C, )",
           ErrorCode::kINVALID_NODE);
    ASSERT((inputs.at(3).shape().nbDims == 1) &&
               "The shape of the mean input must be (C, )",
           ErrorCode::kINVALID_NODE);
    ASSERT((inputs.at(4).shape().nbDims == 1) &&
               "The shape of the var input must be (C, )",
           ErrorCode::kINVALID_NODE);

    OnnxAttrs attrs(node, ctx);
    auto const isTraining = attrs.get<int32_t>("training_mode", 0);
    ASSERT(!isTraining && "This version of TensorRT does not support "
                          "training_mode == 1 in BatchNormalization.",
           ErrorCode::kUNSUPPORTED_NODE);

    bool const allInputsWeights =
        inputs.at(1).is_weights() && inputs.at(2).is_weights() &&
        inputs.at(3).is_weights() && inputs.at(4).is_weights();
    ...
    auto const getValuesFP32 = [&](ShapedWeights const &w) -> float const * {
        return (w.type == onnx::TensorProto::FLOAT)
                   ? static_cast<float *>(w.values)
    ...
    auto combinedScale =
        ctx->createTempWeights(onnx::TensorProto::FLOAT, scale.shape);
    auto combinedBias =
        ctx->createTempWeights(onnx::TensorProto::FLOAT, bias.shape);
    for (int32_t i = 0; i < nbChannels; ++i) {
        combinedScale.at<float>(i) =
            scaleValues[i] / sqrtf(varianceValues[i] + eps);
        combinedBias.at<float>(i) =
            biasValues[i] - meanValues[i] * combinedScale.at<float>(i);
    }

    return scaleHelper(ctx, node, *tensorPtr, nvinfer1::ScaleMode::kCHANNEL,
                       combinedBias, combinedScale,
                       ShapedWeights::empty(onnx::TensorProto::FLOAT),
                       combinedBias.getName(), combinedScale.getName());
}
...
DEFINE_BUILTIN_OP_IMPORTER(Cast) {
    ...
    ASSERT(convertDtype(onnxType, &newType) &&
               "Unsupported data type for the Cast operator!",
           ErrorCode::kINVALID_NODE);
...
DEFINE_BUILTIN_OP_IMPORTER(Celu) {
    ...
    using uOp = nvinfer1::UnaryOperation;
    using eOpInstuctor =
        std::tuple<int, int, const nvinfer1::ElementWiseOperation>;

    ASSERT((!inputs.empty()) && "Inputs vector is empty.",
           ErrorCode::kINVALID_NODE);
    OnnxAttrs attrs(node, ctx);
    TensorOrWeights input = inputs.at(0);
    float alpha = attrs.get<float>("alpha", 1.0);

    TensorOrWeights weightsOfZero =
        ctx->createTempWeights(onnx::TensorProto::FLOAT, {0, {}});
    ShapedWeights weightsOfOnes =
        ctx->createTempWeights(onnx::TensorProto::FLOAT, {0, {}});
    std::vector<float> ones{1};
    std::memcpy(weightsOfOnes.values, ones.data(),
                weightsOfOnes.count() * sizeof(float));
    ShapedWeights weightsOfAlpha =
        ctx->createTempWeights(onnx::TensorProto::FLOAT, {0, {}});
    std::vector<float> alphas{alpha};
    std::memcpy(weightsOfAlpha.values, alphas.data(),
                weightsOfAlpha.count() * sizeof(float));
    ...
    // 1 -> 2
    // alpha -> 3
    std::vector<TensorOrWeights> newInputs{input, weightsOfZero, weightsOfOnes,
                                           weightsOfAlpha};
    ...
    for (auto i : newInputs) {
        ...
        ASSERT(tensor_ptr->getDimensions().nbDims == maxNbDims &&
    ...
    ASSERT(
        elementwiseCheck(tempInputs, eOp::kDIV) &&
            "Elementwise layer does not support the given inputs and operator.",
        ErrorCode::kUNSUPPORTED_NODE);
    nvinfer1::ITensor *combined = inputTensors.at(0);
    auto *divLayer = ctx->network()->addElementWise(
        *combined, *inputTensors.at(3), eOp::kDIV);
    ctx->registerLayer(divLayer, node);
    ASSERT(divLayer && "Failed to register layer.",
           ErrorCode::kUNSUPPORTED_NODE);
    combined = divLayer->getOutput(0);

    // Calculate exp(x/alpha) -> 4
    nvinfer1::IUnaryLayer *uLayer =
        ctx->network()->addUnary(*combined, uOp::kEXP);
    ...
    for (auto it : operations) {
        ...
        ASSERT((elementwiseCheck(tempInputs, op)) &&
                   "Elementwise layer does not support the given inputs and "
                   "operator.",
               ErrorCode::kUNSUPPORTED_NODE);
        ASSERT((firstTensor->getDimensions().nbDims ==
                secondTensor->getDimensions().nbDims) &&
                   "The number of dimensions should remain the same adding "
                   "inputs.",
               ErrorCode::kUNSUPPORTED_NODE);
        auto *layer =
            ctx->network()->addElementWise(*firstTensor, *secondTensor, op);
        ctx->registerLayer(layer, node);
        ASSERT(layer && "Failed to register layer.",
               ErrorCode::kUNSUPPORTED_NODE);
...
NodeImportResult elementwiseClipHelper(IImporterContext *ctx,
                                       onnx::NodeProto const &node,
                                       std::vector<TensorOrWeights> &inputs,
                                       size_t numInputs, int32_t onnxType) {
    OnnxAttrs attrs(node, ctx);
    ...
    if (ctx->getOpsetVersion() >= 11) {
        if (numInputs == 1) {
            ...
            // "min" can be optional if "max" is specified. Check for this case
            // here
    ...
    auto lowerClip = ctx->network()
                         ->addElementWise(*input, *alphaT, eOp::kMAX)
                         ->getOutput(0);
    auto upperClip = ctx->network()
                         ->addElementWise(*lowerClip, *betaT, eOp::kMIN)
                         ->getOutput(0);
    return {{upperClip}};
}
...
DEFINE_BUILTIN_OP_IMPORTER(Clip) {
    ...
    if (elementwiseClip) {
        ...
        ASSERT((type == DataType::kFLOAT || type == DataType::kHALF ||
                type == DataType::kINT32) &&
                   "This version of TensorRT only supports FLOAT or INT32 "
                   "inputs for Clip!",
               ErrorCode::kUNSUPPORTED_NODE);
        if (type == DataType::kFLOAT || type == DataType::kHALF) {
            return elementwiseClipHelper<float>(ctx, node, inputs, numInputs,
                                                onnx::TensorProto::FLOAT);
        } else {
            return elementwiseClipHelper<int32_t>(ctx, node, inputs, numInputs,
    ...
    if (ctx->getOpsetVersion() >= 11) {
        ...
        if (numInputs == 2) {
            ASSERT(inputs.at(1).is_weights() &&
                       "Clip min value must be an initializer!",
                   ErrorCode::kUNSUPPORTED_NODE);
        ...
        // Handle both "min" and "max" node inputs
            ...
            if (!inputs.at(2).isNullTensor()) {
                ASSERT(inputs.at(2).is_weights() &&
                           "Clip max value must be an initializer!",
                       ErrorCode::kUNSUPPORTED_NODE);
    ...
    nvinfer1::ITensor *clipOut =
        &activationHelper(ctx, node, inputs, nvinfer1::ActivationType::kCLIP,
                          &alpha, &beta)
             .value()
             .at(0)
             .tensor();
...
DEFINE_BUILTIN_OP_IMPORTER(Constant) {
    ...
    if (!attrs.get<std::vector<float>>("trt_outputs_range_min", {}).empty()) {
        // just create a constant layer here for 1-1 mapping during network
        // deserialization
    ...
    ASSERT((!attrs.count("sparse_value")) && (!attrs.count("value_string")) &&
               (!attrs.count("value_strings")) &&
               "This version of TensorRT does not support the sparse_value, "
               "value_string and value_strings attributes.",
           ErrorCode::kUNSUPPORTED_NODE);
    ...
    if (ctx->getOpsetVersion() >= 12) {
        if (attrs.count("value_float")) {
            ...
            std::memcpy(convertedWeights.values, &value,
                        convertedWeights.count() * sizeof(float));
            return {{convertedWeights}};
        }

        if (attrs.count("value_floats")) {
            std::vector<float> values =
                attrs.get<std::vector<float>>("value_floats");
            int32_t valueSize = values.size();
            ShapedWeights convertedWeights = ctx->createTempWeights(
                onnx::TensorProto::FLOAT, {1, {valueSize}});
            std::memcpy(convertedWeights.values, values.data(),
                        convertedWeights.count() * sizeof(float));
            return {{convertedWeights}};
        }
        ...
        if (attrs.count("value_int")) {
            ShapedWeights convertedWeights =
                ctx->createTempWeights(onnx::TensorProto::INT32, {0, {}});
            float value = attrs.get<float>("value_int");
            std::memcpy(convertedWeights.values, &value,
                        convertedWeights.count() * sizeof(int));
            return {{convertedWeights}};
        }

        if (attrs.count("value_ints")) {
            std::vector<float> values =
                attrs.get<std::vector<float>>("value_ints");
            int32_t valueSize = values.size();
            ShapedWeights convertedWeights = ctx->createTempWeights(
                onnx::TensorProto::INT32, {1, {valueSize}});
            std::memcpy(convertedWeights.values, values.data(),
                        convertedWeights.count() * sizeof(float));
            return {{convertedWeights}};
        }
...
DEFINE_BUILTIN_OP_IMPORTER(ConstantOfShape) {
    ...
    ShapedWeights zeroWeights = ctx->createTempWeights(
        onnx::TensorProto_DataType_FLOAT, nvinfer1::Dims{1, 1});
...
DEFINE_BUILTIN_OP_IMPORTER(Conv) {
    if (inputs.at(1).is_tensor() ||
        (inputs.size() > 2 && inputs.at(2).is_tensor())) {
    ...
    ASSERT(dims.nbDims >= 0 &&
               "TensorRT could not compute output dimensions of Conv",
           ErrorCode::kUNSUPPORTED_NODE);
    ...
    if (needToExpandDims) {
        ...
        ASSERT(tensorPtr && "Failed to unsqueeze tensor.",
               ErrorCode::kUNSUPPORTED_NODE);
    ...
    // Check that the number of spatial dimensions and the kernel shape matches
    // up.
    ASSERT((nbSpatialDims == kernelWeights.shape.nbDims - 2) &&
               "The number of spatial dimensions and the kernel shape doesn't "
               "match up for the Conv operator.",
           ErrorCode::kUNSUPPORTED_NODE);

    nvinfer1::Weights bias_weights;
    if (inputs.size() == 3) {
        assertIsWeights(inputs.at(2), "The bias tensor is required to be an "
                                      "initializer for the Conv operator.");
        ...
        ASSERT((shapedBiasWeights.shape.nbDims == 1) &&
                   "The bias tensor is required to be 1D.",
               ErrorCode::kINVALID_NODE);
        ASSERT((shapedBiasWeights.shape.d[0] == kernelWeights.shape.d[0]) &&
                   "The shape of the bias tensor misaligns with the weight "
                   "tensor.",
               ErrorCode::kINVALID_NODE);
    ...
    for (int32_t i = 1; i <= nbSpatialDims; ++i) {
        kernelSize.d[nbSpatialDims - i] =
            kernelWeights.shape.d[kernelWeights.shape.nbDims - i];
    }
    ...
    getKernelParams(ctx, node, &kernelSize, &strides, &begPadding, &endPadding,
                    paddingMode, exclude_padding, &dilations);

    for (int32_t i = 1; i <= nbSpatialDims; ++i) {
        ASSERT((kernelSize.d[nbSpatialDims - i] ==
                kernelWeights.shape.d[kernelWeights.shape.nbDims - i]) &&
                   "The size of spatial dimension and the size of kernel shape "
                   "are not equal for the Conv operator.",
               ErrorCode::kUNSUPPORTED_NODE);
    }
    ...
    nvinfer1::IConvolutionLayer *layer = ctx->network()->addConvolutionNd(
        *tensorPtr, noutput, kernelSize, kernelWeights, bias_weights);

    ASSERT(layer && "Failed to add a convolution layer.",
           ErrorCode::kUNSUPPORTED_NODE);
    ...
    ctx->registerLayer(layer, node);
    ctx->network()->setWeightsName(kernelWeights,
                                   inputs.at(1).weights().getName());
    if (inputs.size() == 3) {
        ctx->network()->setWeightsName(bias_weights,
                                       inputs.at(2).weights().getName());
    }
    ...
    LOG_VERBOSE("Using kernel: " << kernelSize << ", strides: " << strides
                                 << ", prepadding: " << begPadding
                                 << ", postpadding: " << endPadding
                                 << ", dilations: " << dilations
                                 << ", numOutputs: " << noutput);
...
DEFINE_BUILTIN_OP_IMPORTER(ConvTranspose) {
    ...
    auto const NCWtoNCHW = [&ctx, &node](nvinfer1::ITensor *&tensor,
                                         nvinfer1::Dims &tensorShape) {
    ...
    ASSERT(inputs.size() >= 2 && "deconvolution require at least 2 inputs.",
           ErrorCode::kUNSUPPORTED_NODE);

    nvinfer1::ITensor *tensorPtr = &convertToTensor(inputs.at(0), ctx);
    nvinfer1::ITensor *kernelTensorPtr =
        inputs.at(1).is_tensor() ? &convertToTensor(inputs.at(1), ctx)
                                 : nullptr;
    ...
    ASSERT(dims.nbDims >= 3 && dims.nbDims <= 5 &&
               "TensorRT only supports 1D, 2D or 3D deconvolutions!",
           ErrorCode::kUNSUPPORTED_NODE);
    ...
    if (inputs.size() > 2 && biasTensorPtr == nullptr) {
        ...
        ASSERT((shapedBiasWeights.shape.d[0] == noutput) &&
                   "The shape of the bias tensor does not align with the shape "
                   "of the output.",
               ErrorCode::kINVALID_NODE);
    ...
    // Kernel shape either comes from the attributes or extracted from the
    ...
    getKernelParams(ctx, node, &kernelSize, &strides, &begPadding, &endPadding,
                    paddingMode, exclude_padding, &dilations, &outputPadding);

    for (int32_t i = 1; i <= nbSpatialDims; ++i) {
        ASSERT((kernelSize.d[nbSpatialDims - i] ==
                kernelShape.d[kernelShape.nbDims - i]) &&
                   "Attribute kernel_shape misaligns with the dimensions of "
                   "the weight tensor.",
               ErrorCode::kUNSUPPORTED_NODE);
    }

    // Set padding. ONNX ConvTranspose supports many different padding modes.
    // Order of priority for padding:
    // 1. Output shape is specified - calculate expected pre and post padding.
    // 2. AUTO_PAD != NOTSET: ignore all other padding values and set padding
    // mode with layer->setPaddingMode.
    //    Pad the resulting output vector with values from output_padding
    // 3. Use specified "pads" values from the node. Pad the resulting output
    // vector with values from output_padding
    ...
    if (attrs.count("output_shape") && autoPadMode == std::string("NOTSET")) {
        ...
        // This function takes references to begPadding, endPadding and
        // outputPadding and will update them with correct values
        generatePadding(dims, outputShape, kernelSize, strides, dilations,
                        nbSpatialDims, begPadding, endPadding, outputPadding,
                        paddingMode);

        // NOTE: it is possible for generatePadding to produce negative values
        // for pre and post padding, which usually happens when output_shape is
        // provided but output_padding is not. Any negative values generated for
        // post-padding can be translated into outputPadding to pad the output
        // tensor post deconvolution. Any negative values for pre-padding are
        // unsupported.

        for (int32_t i = 0; i < nbSpatialDims; i++) {
            ASSERT(begPadding.d[i] >= 0 &&
                       "TensorRT does not support negative pre-padding in the "
                       "ConvTranspose operator!",
                   ErrorCode::kUNSUPPORTED_NODE);
            // Update outputPadding with any negative values in endPadding, and
    ...
    // When there is output_padding, if postPadding is larger than
    // outputPadding, just adjust postPadding Or reduce outputPadding as minimum
    // as possible.
    bool hasOutputPadding = false;
    if (outputPadding != makeDims(nbSpatialDims, 0) &&
        autoPadMode == std::string("NOTSET")) {
        for (int32_t i = 0; i < nbSpatialDims; ++i) {
    ...
    auto const emptyBiasWeights =
        ShapedWeights::empty(onnx::TensorProto::FLOAT);
    auto const kernelWeights =
        kernelTensorPtr ? nvinfer1::Weights{DataType::kFLOAT, nullptr, 0}
                        : inputs.at(1).weights();
    const auto biasWeights =
        biasTensorPtr ? nvinfer1::Weights{DataType::kFLOAT, nullptr, 0}
                      : staticBiasWeights;
    // Create a deconvolution layer and set known attributes - strides,ngroups,
    // and dilations If there is still output padding, remove the bias weights.
    // Bias will be added below.
    auto *layer = ctx->network()->addDeconvolutionNd(
        *tensorPtr, noutput, kernelSize, kernelWeights,
        hasOutputPadding ? emptyBiasWeights : biasWeights);
    layer->setStrideNd(strides);
    layer->setNbGroups(ngroup);
    layer->setDilationNd(dilations);
    if (kernelTensorPtr) {
    ...
    // Register layer, along with refittable kernel weights and bias weights (if
    // any)
    ctx->registerLayer(layer, node);
    tensorPtr = layer->getOutput(0);
    dims = tensorPtr->getDimensions();
    ...
    if (hasOutputPadding) {
        LOG_VERBOSE(
            "Padding output deconvolution tensor with: " << outputPadding);
        ...
        ASSERT(convertOnnxPadding(ctx, dims.nbDims, combinePadding, start,
                                  totalPadding) &&
                   "Failed to convert padding!",
               ErrorCode::kUNSUPPORTED_NODE);
        auto const size =
            ctx->network()
                ->addElementWise(shapeOf(*tensorPtr).tensor(ctx), *totalPadding,
                                 nvinfer1::ElementWiseOperation::kSUM)
                ->getOutput(0);
        auto const stride = makeDims(dims.nbDims, 1);
        auto const &dummy = stride;
        auto *sliceLayer =
            ctx->network()->addSlice(*tensorPtr, dummy, dummy, stride);
        ASSERT(sliceLayer && "Could not create padding layer",
               ErrorCode::kUNSUPPORTED_NODE);
        ...
        if (biasWeights.count != 0) {
            // Set C dimension to weights count and set other dimensions to 1 to
            // enable broadcast
            auto constantDims = makeDims(dims.nbDims, 1);
            constantDims.d[dims.nbDims - nbSpatialDims - 1] = biasWeights.count;
            auto biasConstant =
                ctx->network()->addConstant(constantDims, biasWeights);
            tensorPtr =
                ctx->network()
                    ->addElementWise(*tensorPtr, *biasConstant->getOutput(0),
                                     nvinfer1::ElementWiseOperation::kSUM)
                    ->getOutput(0);
        }
    }
    ...
    if (inputs.size() > 2 && biasTensorPtr == nullptr) {
        ctx->network()->setWeightsName(biasWeights,
                                       inputs.at(2).weights().getName());
    }
    ...
    if (needReshapeBack) {
        ...
        tensorPtr = squeezeTensor(ctx, node, *tensorPtr, axes);
        ASSERT(tensorPtr && "Failed to squeeze tensor.",
               ErrorCode::kUNSUPPORTED_NODE);
    }
...
DEFINE_BUILTIN_OP_IMPORTER(CumSum) {
    ...
    assertIsWeights(inputs.at(1),
                    "Axis input for CumSum must be an initializer!");
    ...
    /* For exclusive CumSums, it is equivalent as a non-exclusive CumSum on a
    ...
    */
    if (exclusive) {
        ...
        std::vector<nvinfer1::ITensor *> concatTensors =
            reverse == 1 ? std::vector<nvinfer1::ITensor *>{input, zero}
                         : std::vector<nvinfer1::ITensor *>{zero, input};

        auto concat = ctx->network()->addConcatenation(concatTensors.data(),
                                                       concatTensors.size());
        concat->setAxis(axis);
        ...
        if (reverse == 0) {
            ShapeTensor const subscripts{
                axesToInterlaceSubscripts(shapeVector(axis), dims.nbDims)};
            ShapeTensor starts =
                fillShapeVector(ctx, 0, shapeVector(dims.nbDims));
            ShapeTensor sizes = interlace(
                ctx, shapeOf(*input),
                sub(ctx, gather(ctx, shapeOf(*input), shapeVector(axis)),
                    shapeVector(1)),
                subscripts);
            ShapeTensor strides =
                fillShapeVector(ctx, 1, shapeVector(dims.nbDims));
            input = addSlice(ctx, *input, starts, sizes, strides)->getOutput(0);
        } else {
            ShapeTensor const subscripts{
                axesToInterlaceSubscripts(shapeVector(axis), dims.nbDims)};
            ShapeTensor starts = interlace(
                ctx, fillShapeVector(ctx, 0, shapeVector(dims.nbDims)),
                shapeVector(1), subscripts);
            ShapeTensor sizes = interlace(
                ctx, shapeOf(*input),
                sub(ctx, gather(ctx, shapeOf(*input), shapeVector(axis)),
                    shapeVector(1)),
    ...
    // Scan through each slice across summation axis and add it to the running
    // sum
    ...
    auto curSum = ctx->network()->addElementWise(
        *data, *runningSumTensor, nvinfer1::ElementWiseOperation::kSUM);
    runningSum->setInput(1, *curSum->getOutput(0));

    auto reverseFlag = reverse == 1 ? nvinfer1::LoopOutput::kREVERSE
...
DEFINE_BUILTIN_OP_IMPORTER(DepthToSpace) {
    ...
    ASSERT((inputs.at(0).shape().nbDims == 4) &&
    ...
    auto const C_2 =
        floorDiv(ctx, C, mul(ctx, blockSizeTensor, blockSizeTensor));
    ...
    if (mode == "DCR") {
        // First reshape to {N, blockSize, blockSize, C / (blockSize *
        // blockSize), H, W}
        firstShape =
            concat(ctx, N,
                   concat(ctx, blockSizeTensor,
                          concat(ctx, blockSizeTensor,
                                 concat(ctx, C_2, concat(ctx, H, W)))));
        std::copy(std::begin(DCRPerm), std::end(DCRPerm),
                  std::begin(perm.order));
    } else {
        // First reshape to {N, C / (blockSize * blockSize), blockSize,
        // blockSize, H, W}
        firstShape = concat(
            ctx, N,
            concat(ctx, C_2,
                   concat(ctx, blockSizeTensor,
                          concat(ctx, blockSizeTensor, concat(ctx, H, W)))));
        std::copy(std::begin(CRDPerm), std::end(CRDPerm),
                  std::begin(perm.order));
    }
    ...
    // Finally reshape to {N, C / (blockSize * blockSize), H * blockSize, W *
    // blockSize};
...
NodeImportResult QuantDequantLinearHelper(IImporterContext *ctx,
                                          onnx::NodeProto const &node,
                                          std::vector<TensorOrWeights> &inputs,
                                          bool isDQ, DataType datatype) {
    CHECK(notInvalidType(inputs.at(0), {"UINT8"}));

    // For QuantizeLinear, the output type (and thus quantization type) is
    // dependent on the second input (zero point).
    if (!isDQ && inputs.size() >= 3) {
        CHECK(notInvalidType(inputs.at(2), {"UINT8"}));
    }
    auto addConstantLayer =
        [ctx, node](nvinfer1::INetworkDefinition &network,
                    ShapedWeights const &weights) -> nvinfer1::ITensor * {
        nvinfer1::IConstantLayer *constLayer =
            network.addConstant(weights.shape, weights);
    ...
    auto newConstantInput = [&](int32_t i) {
        return inputs.at(i).is_weights() &&
               (ctx->getConstantLayer(inputs.at(i).weights().getName()) ==
                nullptr);
    };

    // Read the optional quantization axis attribute. Set it to the rank of the
    // input tensor if not provided
    ASSERT((inputs.size() >= 2) &&
               "This version of TensorRT requires at least 2 inputs for the "
               "QuantizeLinear/DequantizeLinear operator.",
    ...
    if (newConstantInput(1)) {
        ...
        ASSERT(scale.count() > 0 && "Cannot have scale with no coefficients.",
               nvonnxparser::ErrorCode::kINVALID_NODE);
        auto const *scaleVal = static_cast<float const *>(scale.values);
        auto scaleAllPositive = std::all_of(scaleVal, scaleVal + scale.count(),
                                            [](float x) { return x > 0; });
        ASSERT(scaleAllPositive && "Scale coefficients must all be positive",
               nvonnxparser::ErrorCode::kINVALID_NODE);

        // If the scale is concrete weights, then add a ConstantLayer that will
        // be an input which will initialize the scale weights.
    ...
    if (!isFP8) {
        if (newConstantInput(2)) {
            ASSERT((inputs.size() == 3) &&
                       "This version of TensorRT requires 3 inputs for the "
                       "INT8 QuantizeLinear/DequantizeLinear operator.",
                   nvonnxparser::ErrorCode::kINVALID_NODE);
            // Zero-point verification.
            auto zeroPoint = inputs.at(2).weights();
            ASSERT(shiftIsAllZeros(zeroPoint) &&
                       "TensorRT only supports symmetric quantization. The "
                       "zero point for the QuantizeLinear/DequantizeLinear "
                       "operator must be all zeros.",
                   nvonnxparser::ErrorCode::kINVALID_NODE);
            // Convert the zero-point to float because TRT uses float for
            // zero-point.
            auto fpZeroPoint =
                createZeroShifts(zeroPoint, onnx::TensorProto::FLOAT, ctx);
        ...
        // ONNX may represent a scalar using either 0-D or 1-D, so compare sizes
        // instead of shapes.
        ASSERT(zeroPointSize == scaleSize &&
                   "The scale and zero point must have the same volume.",
               nvonnxparser::ErrorCode::kINVALID_NODE);
    }
    ...
    if (scaleSize != 1) {
        ...
        // We assume this is weight-quantization with dimensions KCRS (K is #
        // output channels). Activations-quantization does not support per-axis
        // quantization.
        if (axis == nbDims) {
            axis = 0;
        }
        // Ensure that number of scale-coefficients is equal to the number of
        // output channels.
        int64_t K = dataInput.getDimensions().d[axis];
        ASSERT(K == scaleSize && "The number of scales is not equal to the "
                                 "number of output channels.",
               nvonnxparser::ErrorCode::kINVALID_NODE);
    } else {
        // Per-Tensor Quantization.
        // Currently axis is ignored by TRT, but it is required here by
        // addScaleNd (for computing nbSpatialDims). Set to a sane default
        // depending on rank the input tensor.
        axis = nbDims <= 1 ? 0 : 1;
    }
    ...
    if (isDQ) {
        ...
        nvinfer1::IDequantizeLayer *dq =
            ctx->network()->addDequantize(dataInput, *scaleInput);
        ASSERT(dq && "Failed to create Dequantize layer.",
               ErrorCode::kUNSUPPORTED_NODE);
        dq->setAxis(axis);
        layer = dq;
        if (isFP8) {
        ...
        nvinfer1::IQuantizeLayer *q =
            ctx->network()->addQuantize(dataInput, *scaleInput);
        ASSERT(q && "Failed to create Quantize layer.",
               ErrorCode::kUNSUPPORTED_NODE);
        q->setAxis(axis);
        layer = q;
        if (isFP8) {
...
DEFINE_BUILTIN_OP_IMPORTER(Dropout) {
    ...
    if (ctx->getOpsetVersion() <= 6) {
        ...
        ASSERT(isTestingMode && "TensorRT does not support the Droupout "
                                "operator with training mode.",
               ErrorCode::kUNSUPPORTED_NODE);
    } else if (ctx->getOpsetVersion() >= 12 && node.input().size() == 3) {
        ASSERT(inputs.at(2).is_weights() &&
                   "This Version of TensorRT only supports the training_mode "
                   "input as an initializer.",
               ErrorCode::kUNSUPPORTED_NODE);
        std::vector<int64_t> trainingMode;
        weightsToVector<int64_t>(inputs.at(2).weights(), &trainingMode);
        ASSERT(!trainingMode[0] && "TensorRT does not support the Dropout "
                                   "operator in training mode.",
               ErrorCode::kUNSUPPORTED_NODE);
    }
    ...
    if (noutputs == 1) {
        ...
        // Post opset 12 the mask tensor contains all 1s. Prior to opset 12 the
        // mask tensor contains all 0s.
        if (ctx->getOpsetVersion() >= 12) {
            maskTensor =
                ctx->network()
                    ->addElementWise(inputTensor, inputTensor,
...
DEFINE_BUILTIN_OP_IMPORTER(Einsum) {
    ...
    if (!invalidCharacters.empty()) {
        ...
        return MAKE_ERROR("Invalid character(s) in Einsum equation: " +
                              invalidCharacters,
                          ErrorCode::kINVALID_NODE);
    }
    ...
    nvinfer1::IEinsumLayer *einsumLayer = ctx->network()->addEinsum(
        inputTensors.data(), nbInputs, equation.c_str());
...
DEFINE_BUILTIN_OP_IMPORTER(Elu) {
    ...
    return activationHelper(ctx, node, inputs, nvinfer1::ActivationType::kELU,
                            &alpha);
}
...
DEFINE_BUILTIN_OP_IMPORTER(Expand) {
    ...
    // broadcast rule"
    ASSERT((inputs.at(1).shape().nbDims == 1) &&
               "The shape tensor is required to be 1D.",
    ...
    ShapeTensor const newDims = concat(
        ctx, fillShapeVector(ctx, 1, sub(ctx, newRank, inputRank)), inputDims);
    nvinfer1::ITensor &newInputTensor = reshape(ctx, inputTensor, newDims);

    // ", or the shape.ndim < input.shape.ndim"
    ShapeTensor newShape = concat(
        ctx, fillShapeVector(ctx, 1, sub(ctx, newRank, shapeLength)), shape);
    ...
    // Compute (x > 1 ? 1 : 0) for x in newDims, assuming positive x, using only
    // TensorRT operations.
    ShapeTensor const one = shapeVector(1);
    ShapeTensor const strides = min(ctx, one, sub(ctx, newDims, one));

    nvinfer1::ISliceLayer *sliceLayer =
        addSlice(ctx, newInputTensor, starts, sizes, strides);
...
DEFINE_BUILTIN_OP_IMPORTER(EyeLike) {
    ...
    ASSERT(dims.nbDims == 2 &&
               "Only 2D tensors are supported. Input must be of rank 2.",
           ErrorCode::kUNSUPPORTED_NODE);
    ...
    if (attrs.count("dtype")) {
        ...
        ASSERT(convertDtype(onnxType, &dtype) && "Unsupported cast!",
               ErrorCode::kINVALID_NODE);
        LOG_VERBOSE("Casting to type: " << dtype);
    }

    // Create weights and constant layer
    ASSERT(!isDynamic(dims) &&
               "Eyelike does not work for dynamically shaped tensors.",
           ErrorCode::kUNSUPPORTED_NODE);
    ...
    ShapedWeights tempWeights =
        ctx->createTempWeights(onnx::TensorProto::INT32, dims);
...
DEFINE_BUILTIN_OP_IMPORTER(Flatten) {
    ...
    ASSERT(tensorPtr && "Failed to flatten the tensor.",
           ErrorCode::kUNSUPPORTED_NODE);
    return {{tensorPtr}};
}
...
DEFINE_BUILTIN_OP_IMPORTER(GatherElements) {
    ...
    auto *layer = ctx->network()->addGatherV2(data, indices,
...
DEFINE_BUILTIN_OP_IMPORTER(GatherND) {
    ...
    auto *layer =
        ctx->network()->addGatherV2(data, indices, nvinfer1::GatherMode::kND);
...
DEFINE_BUILTIN_OP_IMPORTER(Gemm) {
    ...
    // Validate inputs
    ASSERT(inputA.getDimensions().nbDims == 2 &&
               inputB.getDimensions().nbDims == 2 &&
               "GEMM must have 2D inputs!",
           ErrorCode::kINVALID_NODE);
    ...
    LOG_VERBOSE("Using opA: " << static_cast<int>(opA)
                              << " opB: " << static_cast<int>(opB));

    nvinfer1::IMatrixMultiplyLayer *matmul =
        ctx->network()->addMatrixMultiply(inputA, opA, inputB, opB);
    ...
    if (alpha != 1.f) {
        ...
        nvinfer1::IElementWiseLayer *scaledMatmul =
            ctx->network()->addElementWise(
                *alphaConstantTensor, *matmulTensor,
    ...
    if (inputs.size() > 2) {
        ...
        if (beta != 1.f) {
            ...
            nvinfer1::IElementWiseLayer *scaledBias =
                ctx->network()->addElementWise(
                    *betaConstantTensor, *biasTensor,
                    nvinfer1::ElementWiseOperation::kPROD);
            biasTensor = scaledBias->getOutput(0);
        }
        CHECK(broadcastTensors(ctx, matmulTensor, biasTensor));
        nvinfer1::IElementWiseLayer *biasAdd = ctx->network()->addElementWise(
            *matmulTensor, *biasTensor, nvinfer1::ElementWiseOperation::kSUM);
...
DEFINE_BUILTIN_OP_IMPORTER(GlobalAveragePool) {
    LOG_VERBOSE("GlobalAveragePool operators are implemented via Reduce layers "
                "rather than Pooling layers");
    return {{globalPoolingHelper(ctx, node, convertToTensor(inputs.at(0), ctx),
...
DEFINE_BUILTIN_OP_IMPORTER(GlobalLpPool) {
    ...
    auto &pTensor =
        *addConstantScalar(ctx, p, onnx::TensorProto::FLOAT, scalarDims)
             ->getOutput(0);
    auto &pInvTensor =
        *addConstantScalar(ctx, 1.F / p, onnx::TensorProto::FLOAT, scalarDims)
             ->getOutput(0);

    // firstPow = pow(x, p)
    auto *firstPow = ctx->network()
                         ->addElementWise(tensor, pTensor,
                                          nvinfer1::ElementWiseOperation::kPOW)
                         ->getOutput(0);
    // reduced = reduce_sum(firstPow)
    auto *reduced = globalPoolingHelper(ctx, node, *firstPow,
                                        nvinfer1::ReduceOperation::kSUM);
    // finalPow = pow(reduced, 1./p)
    auto *finalPow = ctx->network()
                         ->addElementWise(*reduced, pInvTensor,
                                          nvinfer1::ElementWiseOperation::kPOW)
                         ->getOutput(0);
    return {{finalPow}};
}
...
DEFINE_BUILTIN_OP_IMPORTER(GlobalMaxPool) {
    LOG_VERBOSE("GlobalMaxPool operators are implemented via Reduce layers "
                "rather than Pooling layers");
    return {{globalPoolingHelper(ctx, node, convertToTensor(inputs.at(0), ctx),
...
DEFINE_BUILTIN_OP_IMPORTER(GreaterOrEqual) {
    return {{greaterLessOrEqual(ctx, node, &convertToTensor(inputs.at(0), ctx),
...
nvinfer1::ITensor *concatenateRNNOutputs(
    IImporterContext *ctx, onnx::NodeProto const &node, nvinfer1::ILoop *loop,
    nvinfer1::ITensor *singlePassShape, nvinfer1::ITensor *sequenceLength,
    nvinfer1::ITensor *concatenatedOutput, int numDirections,
    std::vector<TensorOrWeights> &inputs, bool reverse = false) {
    nvinfer1::ITensor *yOutput{nullptr};
    if (numDirections == 2) {
        nvinfer1::ITensor *forwardStart =
            addConstant(ctx, std::vector<int32_t>{0, 0, 0},
                        onnx::TensorProto::INT32, nvinfer1::Dims{1, 3})
                ->getOutput(0);
        nvinfer1::ITensor *reverseStart =
            addConstant(ctx, std::vector<int32_t>{1, 0, 0},
                        onnx::TensorProto::INT32, nvinfer1::Dims{1, 3})
                ->getOutput(0);

        LOG_VERBOSE("Concatenated output shape: "
                    << concatenatedOutput->getDimensions());
        nvinfer1::ISliceLayer *HtForwardLayer = ctx->network()->addSlice(
            *concatenatedOutput, nvinfer1::Dims3{0, 0, 0},
            nvinfer1::Dims3{0, 0, 0}, nvinfer1::Dims3{1, 1, 1});
        LOG_VERBOSE("Forward pass shape: "
                    << HtForwardLayer->getOutput(0)->getDimensions());
        ...
        nvinfer1::ISliceLayer *HtBackwardLayer = ctx->network()->addSlice(
            *concatenatedOutput, nvinfer1::Dims3{0, 0, 0},
            nvinfer1::Dims3{0, 0, 0}, nvinfer1::Dims3{1, 1, 1});
        LOG_VERBOSE("Reverse pass shape: "
                    << HtBackwardLayer->getOutput(0)->getDimensions());
        ...
        if (inputs.size() > 4 && inputs.at(4)) {
            ...
            forwardHt = clearMissingSequenceElements(ctx, node, loop, seqLens,
                                                     forwardHt, sequenceLength);
            backwardHt = clearMissingSequenceElements(
                ctx, node, loop, seqLens, backwardHt, sequenceLength,
                /*reverse=*/true);
        }

        nvinfer1::ILoopOutputLayer *forwardOutput = loop->addLoopOutput(
            *forwardHt, nvinfer1::LoopOutput::kCONCATENATE, 0);
        ...
        std::array<nvinfer1::ITensor *, 2> passes{
            {forwardOutput->getOutput(0), reverseOutput->getOutput(0)}};
        nvinfer1::IConcatenationLayer *concat =
            ctx->network()->addConcatenation(passes.data(), passes.size());
        concat->setAxis(1);
        yOutput = concat->getOutput(0);
    } else {
        if (inputs.size() > 4 && inputs.at(4)) {
            nvinfer1::ITensor *seqLens = &convertToTensor(inputs.at(4), ctx);
            concatenatedOutput = clearMissingSequenceElements(
                ctx, node, loop, seqLens, concatenatedOutput, sequenceLength,
                reverse);
        }
        nvinfer1::ILoopOutputLayer *scanOut =
            loop->addLoopOutput(*concatenatedOutput,
                                (reverse ? nvinfer1::LoopOutput::kREVERSE
                                         : nvinfer1::LoopOutput::kCONCATENATE),
                                0);
...
DEFINE_BUILTIN_OP_IMPORTER(GRU) {
    ...
    std::string const direction =
        attrs.get<std::string>("direction", "forward");
    int32_t const numDirections = (direction == "bidirectional") ? 2 : 1;
    int32_t const hiddenSize = attrs.get<int32_t>("hidden_size");
    int32_t const linearBeforeReset =
        attrs.get<int32_t>("linear_before_reset", 0);
    const float clip =
        attrs.get("clip", -1.f);  // Clipping cannot be negative, so -1.0 is a
                                  // good sentinel value.
    ...
    if (numDirections == 2) {
        defaultActs.insert(defaultActs.end(),
                           {trtAct::kSIGMOID, trtAct::kTANH});
    }
    std::vector<trtAct> activations =
        attrs.get<std::vector<trtAct>>("activations", defaultActs);

    std::vector<float> activationAlphas =
        attrs.get<std::vector<float>>("activation_alpha", std::vector<float>{});
    std::transform(activations.begin() + activationAlphas.size(),
                   activations.end(), std::back_inserter(activationAlphas),
                   &getActivationDefaultAlpha);

    std::vector<float> activationBetas =
        attrs.get<std::vector<float>>("activation_beta", std::vector<float>{});
    std::transform(activations.begin() + activationBetas.size(),
                   activations.end(), std::back_inserter(activationBetas),
                   &getActivationDefaultBeta);

    // TODO: Support cases where in bidirectional GRUs, activations of reverse
    // iteration do not match forward pass.
    // TODO: This will require splitting the input tensor in the loop when
    // applying activations.
    if (numDirections == 2) {
        ASSERT(std::equal(activations.begin(),
                          activations.begin() + NUM_ACTIVATIONS,
                          activations.begin() + NUM_ACTIVATIONS) &&
                   "The parser does not currently support cases where "
                   "activations for the reverse pass of the GRU do not match "
                   "the forward pass.",
               ErrorCode::kUNSUPPORTED_NODE);
        ASSERT(std::equal(activationAlphas.begin(),
                          activationAlphas.begin() + NUM_ACTIVATIONS,
                          activationAlphas.begin() + NUM_ACTIVATIONS) &&
                   "The parser does not currently support cases where "
                   "activations for the reverse pass of the GRU do not match "
                   "the forward pass.",
               ErrorCode::kUNSUPPORTED_NODE);
        ASSERT(std::equal(activationBetas.begin(),
                          activationBetas.begin() + NUM_ACTIVATIONS,
                          activationBetas.begin() + NUM_ACTIVATIONS) &&
                   "The parser does not currently support cases where "
                   "activations for the reverse pass of the GRU do not match "
                   "the forward pass.",
    ...
    // Need to split weights/biases into ZR gates and H gate, because h(t)
    // computations depend on z(t) and r(t).
    nvinfer1::ITensor *numDirectionsTensor =
        addConstantScalar(ctx, numDirections, onnx::TensorProto::INT32,
                          Dims{1, 1})
            ->getOutput(0);
    nvinfer1::ITensor *hiddenSizeTensor =
        addConstantScalar(ctx, hiddenSize, onnx::TensorProto::INT32, Dims{1, 1})
            ->getOutput(0);
    nvinfer1::ITensor *hiddenSizeDoubledTensor =
        addConstantScalar(ctx, 2 * hiddenSize, onnx::TensorProto::INT32,
                          Dims{1, 1})
            ->getOutput(0);
    ...
    nvinfer1::ITensor *weightsZRStart =
        addConstant(ctx, std::vector<int32_t>{0, 0, 0},
                    onnx::TensorProto::INT32, Dims{1, 3})
            ->getOutput(0);
    nvinfer1::ITensor *weightsZRSize =
        net->addConcatenation(
               ...
               3)
            ->getOutput(0);
    nvinfer1::ISliceLayer *weightsZRLayer =
        net->addSlice(weights, Dims{3}, Dims{3}, Dims3{1, 1, 1});
    ...
    LOG_VERBOSE(
        "Weights for ZR gates shape is: " << weightsZR->getDimensions());

    nvinfer1::ITensor *weightsHStart =
        addConstant(ctx, std::vector<int32_t>{0, 2 * hiddenSize, 0},
                    onnx::TensorProto::INT32, Dims{1, 3})
            ->getOutput(0);
    nvinfer1::ITensor *weightsHSize =
        net->addConcatenation(
               ...
               3)
            ->getOutput(0);
    nvinfer1::ISliceLayer *weightsHLayer =
        net->addSlice(weights, Dims{3}, Dims{3}, Dims3{1, 1, 1});
    ...
    nvinfer1::ITensor *recurrenceWeightsZR =
        net->addSlice(recurrenceWeights, Dims3{0, 0, 0},
                      Dims3{numDirections, 2 * hiddenSize, hiddenSize},
                      Dims3{1, 1, 1})
            ->getOutput(0);
    LOG_VERBOSE("Recurrence weights for ZR gates shape is: "
                << recurrenceWeightsZR->getDimensions());
    nvinfer1::ITensor *recurrenceWeightsH =
        net->addSlice(recurrenceWeights, Dims3{0, 2 * hiddenSize, 0},
                      Dims3{numDirections, hiddenSize, hiddenSize},
                      Dims3{1, 1, 1})
            ->getOutput(0);
    LOG_VERBOSE("Recurrence weights for H gate shape is: "
                << recurrenceWeightsH->getDimensions());

    // bias/recurrenceBias will have shape (numDirections, NUM_GATES *
    // hiddenSize)
    ...
    if (inputs.size() > 3 && inputs.at(3)) {
        // ONNX bias is a concatenation of Wb and Rb on the second axis, so has
        // shape (numDirections, 2 * NUM_GATES * hiddenSize) Unsqueeze to
        // (numDirections, 1, 2 * NUM_GATES * hiddenSize) so we can broadcast
        // later
        nvinfer1::ITensor *concatenatedBias =
            &convertToTensor(inputs.at(3), ctx);
        nvinfer1::IShuffleLayer *unsqueeze = net->addShuffle(*concatenatedBias);
        unsqueeze->setReshapeDimensions(
            Dims3{numDirections, 1, 2 * NUM_GATES * hiddenSize});
        unsqueeze->setZeroIsPlaceholder(false);
        concatenatedBias = unsqueeze->getOutput(0);

        biasZR = net->addSlice(*concatenatedBias, Dims3{0, 0, 0},
                               Dims3{numDirections, 1, 2 * hiddenSize},
                               Dims3{1, 1, 1})
                     ->getOutput(0);
        LOG_VERBOSE("Bias for ZR gates shape is: " << biasZR->getDimensions());
        biasH =
            net->addSlice(*concatenatedBias, Dims3{0, 0, 2 * hiddenSize},
        ...
        recurrenceBiasZR =
            net->addSlice(
                   *concatenatedBias, Dims3{0, 0, NUM_GATES * hiddenSize},
                   Dims3{numDirections, 1, 2 * hiddenSize}, Dims3{1, 1, 1})
                ->getOutput(0);
        LOG_VERBOSE("Recurrence bias for ZR gates shape is: "
                    << recurrenceBiasZR->getDimensions());
        recurrenceBiasH =
            net->addSlice(*concatenatedBias,
                          Dims3{0, 0, (NUM_GATES + 2) * hiddenSize},
                          Dims3{numDirections, 1, hiddenSize}, Dims3{1, 1, 1})
                ->getOutput(0);
        LOG_VERBOSE("Recurrence bias for H gate shape is: "
                    << recurrenceBiasH->getDimensions());
    }

    // Get a shape tensor containing: (numDirections, batchSize, hiddenSize)
    auto const initialStateShape = [&ctx, &numDirections, &hiddenSize, &input,
                                    &net]() -> nvinfer1::ITensor * {
        // Get batchSize from input shape
        nvinfer1::ITensor *numDirectionsTensor =
            addConstantScalar(ctx, numDirections,
                              onnx::TensorProto_DataType_INT32, Dims{1, 1})
                ->getOutput(0);
        LOG_VERBOSE("numDirections is: "
                    << numDirections << ", numDirections Tensor shape: "
                    << numDirectionsTensor->getDimensions());
        nvinfer1::ITensor *hiddenSizeTensor =
            addConstantScalar(ctx, hiddenSize, onnx::TensorProto_DataType_INT32,
                              Dims{1, 1})
                ->getOutput(0);
        LOG_VERBOSE("hiddenSize is: " << hiddenSize
                                      << ", hiddenSizeTensor shape: "
                                      << hiddenSizeTensor->getDimensions());
        nvinfer1::ITensor *batchSizeTensor =
            getAxisLength(ctx, input, 1, Dims{1, 1});
        LOG_VERBOSE(
            "batchSizeTensor shape: " << batchSizeTensor->getDimensions());

        nvinfer1::IConcatenationLayer *concatenatedShape =
            net->addConcatenation(
                std::array<nvinfer1::ITensor *, 3>{
    ...
    LOG_VERBOSE("Gate output rank (equal to initial hidden/cell state rank): "
                << gateOutputShape->getDimensions());
    ...
    // Add X(t)
    nvinfer1::ITensor *iterationInput =
        addRNNInput(ctx, node, loop, inputs, direction);
    ASSERT(iterationInput && "Failed to add RNN input.",
           ErrorCode::kINVALID_NODE);

    // H(t-1)
    auto const getInitialInputValue =
        [&ctx, &gateOutputShape, &inputs,
         &node](size_t inputIdx) -> nvinfer1::ITensor * {
        if (inputs.size() > inputIdx && inputs.at(inputIdx)) {
            return &convertToTensor(inputs.at(inputIdx), ctx);
        }
        return constantOfShape(
            ctx, node,
            addConstantScalar(ctx, 0.f, onnx::TensorProto_DataType_FLOAT,
                              Dims{1, 1})
                ->getOutput(0),
            gateOutputShape);
    };
    ...
    LOG_VERBOSE(
        "Initial hidden state shape: " << initialHidden->getDimensions());
    ...
    // Compute stackedZR(t) = f(X(t) * W[zr]^T + H(t-1) * R[zr]^T + (Wb[zr] +
    // Rb[zr])). stackedZR(t) has shape (numDirections, batchSize, 2 *
    // hiddenSize)
    nvinfer1::ITensor *xtWTZR =
        net->addMatrixMultiply(*iterationInput, mOp::kNONE, *weightsZR,
                               mOp::kTRANSPOSE)
            ->getOutput(0);
    LOG_VERBOSE("X(t) * W[zr]^T -> " << xtWTZR->getDimensions());

    nvinfer1::ITensor *ht1RT =
        net->addMatrixMultiply(*Ht1->getOutput(0), mOp::kNONE,
                               *recurrenceWeightsZR, mOp::kTRANSPOSE)
            ->getOutput(0);
    LOG_VERBOSE("H(t-1) * R[zr]^T -> " << ht1RT->getDimensions());

    nvinfer1::ITensor *stackedZRt =
        net->addElementWise(*xtWTZR, *ht1RT, eOp::kSUM)->getOutput(0);
    if (biasZR && recurrenceBiasZR) {
        stackedZRt =
            net->addElementWise(*stackedZRt, *biasZR, eOp::kSUM)->getOutput(0);
        stackedZRt =
            net->addElementWise(*stackedZRt, *recurrenceBiasZR, eOp::kSUM)
                ->getOutput(0);
    }
    ...
    auto const isolateGate = [&ctx, &hiddenSize, &gateOutputShape,
                              &net](nvinfer1::ITensor *gates,
                                    int32_t gateIndex) -> nvinfer1::ITensor * {
        nvinfer1::ISliceLayer *isolateGate = net->addSlice(
            *gates, Dims3{0, 0, 0}, Dims3{0, 0, 0}, Dims3{1, 1, 1});
        isolateGate->setInput(
            1, *addConstant(ctx,
                            std::vector<int32_t>{0, 0, gateIndex * hiddenSize},
    ...
    nvinfer1::ITensor *xtWTH =
        net->addMatrixMultiply(*iterationInput, mOp::kNONE, *weightsH,
                               mOp::kTRANSPOSE)
            ->getOutput(0);
    if (linearBeforeReset == 0) {
        // h(t) = g(xtWTH + (r(t) . H(t-1)) * (R[h]^T) + Rb[h] + Wb[h])
        // rtHt1 = (r(t) . H(t-1))
        nvinfer1::ITensor *rtHt1 =
            net->addElementWise(*rt, *Ht1->getOutput(0), eOp::kPROD)
                ->getOutput(0);
        // rtHt1Rh = (r(t) . H(t-1)) * (R[h]^T)
        nvinfer1::ITensor *rtHt1Rh =
            net->addMatrixMultiply(*rtHt1, mOp::kNONE, *recurrenceWeightsH,
                                   mOp::kTRANSPOSE)
                ->getOutput(0);

        // (xtWTH + rtHt1Rh) + (Rb[h] + Wb[h])
        nvinfer1::ITensor *actInput =
            net->addElementWise(*xtWTH, *rtHt1Rh, eOp::kSUM)->getOutput(0);

        // If bias is defines, both recurrence and normal bias must be present
        if (recurrenceBiasH && biasH) {
            nvinfer1::ITensor *secondSum =
                net->addElementWise(*recurrenceBiasH, *biasH, eOp::kSUM)
                    ->getOutput(0);
            actInput = net->addElementWise(*actInput, *secondSum, eOp::kSUM)
                           ->getOutput(0);
        }

        nvinfer1::IActivationLayer *htLayer = net->addActivation(
            *addClip(ctx, actInput, clip), activations.at(1));
        ...
        nvinfer1::ITensor *ht1Rh =
            net->addMatrixMultiply(*Ht1->getOutput(0), mOp::kNONE,
                                   *recurrenceWeightsH, mOp::kTRANSPOSE)
                ->getOutput(0);

        // rtHtRhRbh = r(t) . (ht1Rh + Rb[h])
        if (recurrenceBiasH) {
            ht1Rh = net->addElementWise(*ht1Rh, *recurrenceBiasH, eOp::kSUM)
                        ->getOutput(0);
        }
        nvinfer1::ITensor *rtHtRhRbh =
            net->addElementWise(*rt, *ht1Rh, eOp::kPROD)->getOutput(0);

        // h(t) = g(xtWTH + rtHtRhRbh + Wb[h])
        if (biasH) {
            rtHtRhRbh = net->addElementWise(*rtHtRhRbh, *biasH, eOp::kSUM)
                            ->getOutput(0);
        }
        nvinfer1::IActivationLayer *htLayer = net->addActivation(
            *addClip(ctx,
                     net->addElementWise(*xtWTH, *rtHtRhRbh, eOp::kSUM)
                         ->getOutput(0),
                     clip),
            activations.at(1));
        htLayer->setAlpha(activationAlphas.at(1));
    ...
    nvinfer1::ITensor *Ht =
        net->addElementWise(
               *net->addElementWise(
                       ...
                       *ht, eOp::kPROD)
                    ->getOutput(0),
               *net->addElementWise(*zt, *Ht1->getOutput(0), eOp::kPROD)
                    ->getOutput(0),
               eOp::kSUM)
            ->getOutput(0);
    ...
    if (inputs.size() > 4 && inputs.at(4)) {
        ...
        Ht = numDirections == 2
                 ? maskBidirRNNHidden(ctx, node, loop, seqLens, maxLen,
                                      Ht1->getOutput(0), Ht, singlePassShape)
                 : maskRNNHidden(ctx, node, loop, seqLens, Ht1->getOutput(0),
                                 Ht, maxLen, direction == "reverse");
    }
    Ht1->setInput(1, *Ht);
    LOG_VERBOSE("H(t) -> " << Ht->getDimensions());
    ...
    outputs.emplace_back(concatenateRNNOutputs(
        ctx, node, loop, singlePassShape, getAxisLength(ctx, input, 0), Ht,
        numDirections, inputs, direction == "reverse"));
    // Yh = last value of H(t)
    outputs.emplace_back(loop->addLoopOutput(*Ht1->getOutput(0),
...
DEFINE_BUILTIN_OP_IMPORTER(HardSigmoid) {
    ...
    return activationHelper(ctx, node, inputs,
                            nvinfer1::ActivationType::kHARD_SIGMOID, &alpha,
                            &beta);
}

DEFINE_BUILTIN_OP_IMPORTER(Identity) {
    auto *layer =
        ctx->network()->addIdentity(convertToTensor(inputs.at(0), ctx));
    ctx->registerLayer(layer, node);
    RETURN_FIRST_OUTPUT(layer);
}
...
DEFINE_BUILTIN_OP_IMPORTER(If) {
    ...
    onnx::GraphProto const &thenGraph =
        attrs.get<onnx::GraphProto const &>("then_branch");
    onnx::GraphProto const &elseGraph =
        attrs.get<onnx::GraphProto const &>("else_branch");
    ...
    //
    // The condition is not a build-time constant. Construct an if-conditional
    // construct.
    //

    // The `condition` tensor must be a scalar boolean.
    auto *condTensor = convertToScalar(ctx, &convertToTensor(cond, ctx));
    ASSERT(condTensor && "Failed to convert the input cond to a scalar.",
           ErrorCode::kINVALID_NODE);
    ...
    CHECK(addIfOutputLayers(ctx, conditional, thenGraph, thenLayers,
                            thenSubgraphTensors, elseGraph, elseLayers,
...
DEFINE_BUILTIN_OP_IMPORTER(ImageScaler) {
    ...
    ShapedWeights shiftWeights =
        ctx->createTempWeights(onnx::TensorProto_DataType_FLOAT, dims);
    std::copy(biases.begin(), biases.end(),
              static_cast<float *>(shiftWeights.values));
    // Scale is applied to every element of the input, but we need to duplicate
    // it over every channel.
    float scale = attrs.get<float>("scale", 1.0f);
    ShapedWeights scaleWeights =
        ctx->createTempWeights(onnx::TensorProto_DataType_FLOAT, dims);
    std::fill(static_cast<float *>(scaleWeights.values),
              static_cast<float *>(scaleWeights.values) + scaleWeights.count(),
              scale);
    // Finally add the scale layer.
    auto layer = ctx->network()->addScale(tensor, nvinfer1::ScaleMode::kCHANNEL,
                                          shiftWeights, scaleWeights,
                                          nvinfer1::Weights{});
    ctx->registerLayer(layer, node);
    RETURN_FIRST_OUTPUT(layer);
}
...
DEFINE_BUILTIN_OP_IMPORTER(InstanceNormalization) {
    // Choose plugin implementation for non-VC and non-HC engines, and native
    // implementation for VC and HC engines.
    auto flags = ctx->getFlags();
    uint32_t nativeInstanceNormFlag =
...
DEFINE_BUILTIN_OP_IMPORTER(IsInf) {
    ...
    std::fill(scalarDims.d, scalarDims.d + scalarDims.nbDims, 1);
    auto &zeroTensor =
        *addConstantScalar(ctx, 0.F, onnx::TensorProto::FLOAT, scalarDims)
             ->getOutput(0);

    if (detectNegative) {
        auto *isNeg =
            ctx->network()
                ->addElementWise(input, zeroTensor,
                                 nvinfer1::ElementWiseOperation::kLESS)
                ->getOutput(0);
        auto *isInf = ctx->network()
                          ->addUnary(input, nvinfer1::UnaryOperation::kISINF)
                          ->getOutput(0);
        RETURN_FIRST_OUTPUT(ctx->network()->addElementWise(
            *isNeg, *isInf, nvinfer1::ElementWiseOperation::kAND));
    }
    if (detectPositive) {
        auto *isPos =
            ctx->network()
        ...
        auto *isInf = ctx->network()
                          ...
                          ->getOutput(0);
        RETURN_FIRST_OUTPUT(ctx->network()->addElementWise(
            *isPos, *isInf, nvinfer1::ElementWiseOperation::kAND));
    }
    // In this case, always return false.
    auto *isPos = ctx->network()
                      ->addElementWise(input, zeroTensor,
                                       nvinfer1::ElementWiseOperation::kGREATER)
                      ->getOutput(0);
    auto *isNeg = ctx->network()
                      ->addElementWise(input, zeroTensor,
                                       nvinfer1::ElementWiseOperation::kLESS)
                      ->getOutput(0);
    RETURN_FIRST_OUTPUT(ctx->network()->addElementWise(
        *isPos, *isNeg, nvinfer1::ElementWiseOperation::kAND));
}

DEFINE_BUILTIN_OP_IMPORTER(IsNaN) {
    // IEEE arithmetic guarantees that x == x is false if x is a NaN, and true
    // otherwise.
    std::vector<TensorOrWeights> const newInputs{inputs[0], inputs[0]};
    auto equalResult = elementwiseHelper(
        ctx, node, newInputs, nvinfer1::ElementWiseOperation::kEQUAL);
...
DEFINE_BUILTIN_OP_IMPORTER(LayerNormalization) {
    ...
    auto biasType = input->getType() == nvinfer1::DataType::kFLOAT
                        ? onnx::TensorProto::FLOAT
                        : onnx::TensorProto::FLOAT16;
    auto *bias = inputs.size() == 3
                     ? &convertToTensor(inputs.at(2), ctx)
                     : addConstantScalar(ctx, 0, biasType)->getOutput(0);
    ...
    auto *layer =
        ctx->network()->addNormalization(*input, *scale, *bias, axesMask);
...
DEFINE_BUILTIN_OP_IMPORTER(LeakyRelu) {
    ...
    return activationHelper(ctx, node, inputs,
                            nvinfer1::ActivationType::kLEAKY_RELU, &alpha);
}
...
DEFINE_BUILTIN_OP_IMPORTER(LogSoftmax) {
    ...
    nvinfer1::IUnaryLayer *unaryLayer =
        ctx->network()->addUnary(*softmax, nvinfer1::UnaryOperation::kLOG);
    // Reshape back to original shape
    auto *reshapeLayer =
        addShuffle(ctx, *unaryLayer->getOutput(0), shapeOf(input));
    RETURN_FIRST_OUTPUT(reshapeLayer);
}

DEFINE_BUILTIN_OP_IMPORTER(Loop) {
    constexpr int32_t NB_NON_STATE_INPUTS =
        2;  // First 2 inputs are trip count and condition respectively.
    constexpr int32_t NB_DISCARDED_OUTPUTS =
        1;  // First output is the updated value of the condition, and is
            // ignored by the outer loop node.
    constexpr int32_t DUMMY_SCAN_OUTPUT_LENGTH = 1024;
    ASSERT((inputs.size() >= 2) &&
               "The Loop operator requires at least 2 inputs.",
           ErrorCode::kINVALID_NODE);
    OnnxAttrs attrs(node, ctx);
    ...
    if (inputs[0]) {
        // Some convertors will use INT_MAX to signify "use cond input as loop
        // termination". From TRT's perspective, we can just treat these cases
        // as an empty tripLimit.
        bool const isMaxTripCount =
            inputs[0].is_weights() &&
            static_cast<int32_t *>(inputs[0].weights().values)[0] ==
                std::numeric_limits<int32_t>::max();
        if (!isMaxTripCount) {
            tripLimit = convertToScalar(ctx, &convertToTensor(inputs[0], ctx));
            ASSERT(tripLimit &&
                       "Failed to convert the trip-count input to a scalar.",
                   ErrorCode::kINVALID_NODE);
    ...
    if (inputs[1]) {
        ...
        ASSERT(cond && "Failed to convert the input cond to a scalar.",
               ErrorCode::kINVALID_NODE);
    ...
    for (size_t i = 2; i < inputs.size(); ++i) {
        stateVars.emplace_back(
            loop->addRecurrence(convertToTensor(inputs[i], ctx)));
        ctx->loopTensors()[body.input(i).name()] = node.input(i);
        ctx->registerTensor(TensorOrWeights{stateVars.back()->getOutput(0)},
                            body.input(i).name());
        LOG_VERBOSE("Mapped Loop node input " << node.input(i)
                                              << " to loop body input "
                                              << body.input(i).name());
    }
    ...
    if (cond) {
        ...
        auto condOutput = convertToScalar(
            ctx, &convertToTensor(ctx->tensors().at(bodyOutputName), ctx));
        recurrence->setInput(1, *condOutput);
        loop->addTripLimit(*recurrence->getOutput(0),
    ...
    for (int32_t i = 0; i < nbStateVars; ++i) {
        // The first output of the body graph is the updated condition, which is
        // ignored by the Loop node.
        int32_t const index = i + NB_DISCARDED_OUTPUTS;
        auto const &bodyOutputName = body.output(index).name();
        auto &stateOutput =
            convertToTensor(ctx->tensors().at(bodyOutputName), ctx);
        LOG_VERBOSE("For state variable output: "
                    << bodyOutputName
        ...
        nodeOutputs.emplace_back(
            loop->addLoopOutput(*stateVars.at(i)->getOutput(0),
                                nvinfer1::LoopOutput::kLAST_VALUE)
                ->getOutput(0));
    }
    ...
    for (int32_t i = nbStateVars + NB_DISCARDED_OUTPUTS; i < nbOutputs; ++i) {
        ...
        auto &scanOutput =
            convertToTensor(ctx->tensors().at(bodyOutputName), ctx);
        LOG_VERBOSE("For scan output: "
                    << bodyOutputName
                    << ", found matching tensor: " << scanOutput.getName()
                    << ", with shape: " << scanOutput.getDimensions());
        nvinfer1::ILoopOutputLayer *trtScanOut = loop->addLoopOutput(
            scanOutput, nvinfer1::LoopOutput::kCONCATENATE, 0);
        // If trip limit is set, we can set the loop output to the tripLimit,
        // otherwise, set to some dummy constant value. In the latter case, the
        // scan outputs must not be used in the rest of the model.
        if (tripLimit) {
            trtScanOut->setInput(1, *tripLimit);
        } else {
            trtScanOut->setInput(
                1, *addConstantScalar(ctx, DUMMY_SCAN_OUTPUT_LENGTH,
                                      onnx::TensorProto_DataType_INT32)
                        ->getOutput(0));
        }
...
DEFINE_BUILTIN_OP_IMPORTER(LSTM) {
    ...
    ASSERT(inputForget == 0 &&
               "Coupled input/forget is unsupported in the LSTM converter",
           ErrorCode::kUNSUPPORTED_NODE);
    ...
    if (numDirections == 2) {
        defaultActs.insert(defaultActs.end(),
                           {trtAct::kSIGMOID, trtAct::kTANH, trtAct::kTANH});
    }
    std::vector<trtAct> activations =
        attrs.get<std::vector<trtAct>>("activations", defaultActs);
    ...
    std::vector<float> activationAlphas =
        attrs.get<std::vector<float>>("activation_alpha", std::vector<float>{});
    activationAlphas =
        parseLSTMActivationValues(activations, activationAlphas, true);

    std::vector<float> activationBetas =
        attrs.get<std::vector<float>>("activation_beta", std::vector<float>{});
    activationBetas =
        parseLSTMActivationValues(activations, activationBetas, false);

    // TODO: Support cases where in bidirectional LSTMs, activations of reverse
    // iteration do not match forward pass.
    // TODO: This will require splitting the input tensor in the loop when
    // applying activations.
    if (numDirections == 2) {
        ASSERT(std::equal(activations.begin(),
                          ...
                          activations.begin() + NUM_ACTIVATIONS) &&
                   "The parser does not currently support cases where "
                   "activations for the reverse pass of the LSTM do not match "
                   "the forward pass.",
               ErrorCode::kUNSUPPORTED_NODE);
        ASSERT(std::equal(activationAlphas.begin(),
                          activationAlphas.begin() + NUM_ACTIVATIONS,
                          activationAlphas.begin() + NUM_ACTIVATIONS) &&
                   "The parser does not currently support cases where "
                   "activation alphas for the reverse pass of the LSTM do not "
        ...
        ASSERT(std::equal(activationBetas.begin(),
                          ...
                          activationBetas.begin() + NUM_ACTIVATIONS) &&
                   "The parser does not currently support cases where "
                   "activation betas for the reverse pass of the LSTM do not "
                   "match the forward pass.",
               ErrorCode::kUNSUPPORTED_NODE);
    }

    // Roll Rb into Wb (and RBb into WBb). Bias is in the form  [Wb[iofc],
    // Rb[iofc], WBb[iofc], RBb[iofc]]. So reshape such that we can perform a
    // reduction to add Wb and Rb.
    ...
    if (inputs.size() > 3 && inputs.at(3)) {
        ...
        nvinfer1::IShuffleLayer *reshapeBias =
            ctx->network()->addShuffle(*bias);
        reshapeBias->setReshapeDimensions(
            nvinfer1::Dims3{numDirections, 2, NUM_GATES * hiddenSize});
        reshapeBias->setZeroIsPlaceholder(false);
        LOG_VERBOSE("Reshaping bias to: "
                    << reshapeBias->getOutput(0)->getDimensions());
        combinedBias =
            ctx->network()
                ->addReduce(*reshapeBias->getOutput(0),
                            nvinfer1::ReduceOperation::kSUM, /*axis=*/0b010,
                            /*keepDimensions=*/true)
                ->getOutput(0);
        LOG_VERBOSE("After reduction, bias shape is: "
                    << combinedBias->getDimensions());
    }

    // Get a shape tensor containing: (numDirections, batchSize, hiddenSize)
    auto const initialStateShape = [&ctx, &numDirections, &hiddenSize,
                                    &input]() -> nvinfer1::ITensor * {
        // Get batchSize from input shape
        nvinfer1::ITensor *numDirectionsTensor =
            addConstantScalar(ctx, numDirections,
                              onnx::TensorProto_DataType_INT32,
                              nvinfer1::Dims{1, 1})
                ->getOutput(0);
        LOG_VERBOSE("numDirectionsTensor shape: "
                    << numDirectionsTensor->getDimensions());
        nvinfer1::ITensor *hiddenSizeTensor =
            addConstantScalar(ctx, hiddenSize, onnx::TensorProto_DataType_INT32,
                              nvinfer1::Dims{1, 1})
                ->getOutput(0);
        LOG_VERBOSE(
            "hiddenSizeTensor shape: " << hiddenSizeTensor->getDimensions());
        nvinfer1::ITensor *batchSizeTensor =
            getAxisLength(ctx, input, 1, nvinfer1::Dims{1, 1});
        LOG_VERBOSE(
            "batchSizeTensor shape: " << batchSizeTensor->getDimensions());

        std::array<nvinfer1::ITensor *, 3> tensors{
            {numDirectionsTensor, batchSizeTensor, hiddenSizeTensor}};
        nvinfer1::IConcatenationLayer *concatenatedShape =
            ctx->network()->addConcatenation(tensors.data(), 3);
        return concatenatedShape->getOutput(0);
    };
    nvinfer1::ITensor *gateOutputShape = initialStateShape();
    LOG_VERBOSE("Gate output rank (equal to initial hidden/cell state rank): "
    ...
    LOG_VERBOSE(
        "Initial cell state shape: " << initialCellState->getDimensions());
    ...
    // intermediate(t) has shape (numDirections, batchSize, 4 * hiddenSize)
    nvinfer1::ITensor *xtWT =
        ctx->network()
            ->addMatrixMultiply(*iterationInput,
                                nvinfer1::MatrixOperation::kNONE, *weights,
    ...
    nvinfer1::ITensor *ht1RT =
        ctx->network()
            ->addMatrixMultiply(
                *Ht1->getOutput(0), nvinfer1::MatrixOperation::kNONE,
    ...
    nvinfer1::ITensor *intermediatet =
        ctx->network()->addElementWise(*xtWT, *ht1RT, eOp::kSUM)->getOutput(0);
    if (combinedBias) {
        intermediatet =
            ctx->network()
                ->addElementWise(*intermediatet, *combinedBias, eOp::kSUM)
                ->getOutput(0);
    }
    ...
    auto const isolateGate = [&ctx, &hiddenSize, &gateOutputShape](
        ...
        nvinfer1::ISliceLayer *isolate = ctx->network()->addSlice(
            *gates, nvinfer1::Dims3{0, 0, 0}, nvinfer1::Dims3{0, 0, 0},
            nvinfer1::Dims3{1, 1, 1});
        isolate->setInput(
            1, *addConstant(
                    ctx, std::vector<int32_t>{0, 0, gateIndex * hiddenSize},
    ...
    auto const addPeephole =
        [&ctx, &node, &hiddenSize, &numDirections,
         &peephole](nvinfer1::ITensor *gate, nvinfer1::ITensor *cellState,
                    int32_t gateIndex) -> nvinfer1::ITensor * {
        nvinfer1::ISliceLayer *isolatePeephole = ctx->network()->addSlice(
            *peephole, nvinfer1::Dims2{0, gateIndex * hiddenSize},
            nvinfer1::Dims2{numDirections, hiddenSize}, nvinfer1::Dims2{1, 1});
        auto *peepholeWeights = unsqueezeTensor(
            ctx, node, *isolatePeephole->getOutput(0), std::vector<int32_t>{1});
        LOG_VERBOSE("Peephole weight for gate: "
                    << gateIndex
                    << " shape: " << peepholeWeights->getDimensions());

        return ctx->network()
            ->addElementWise(
                *gate,
                *ctx->network()
                     ->addElementWise(*peepholeWeights, *cellState, eOp::kPROD)
                     ->getOutput(0),
                eOp::kSUM)
            ->getOutput(0);
    };
    ...
    nvinfer1::IActivationLayer *itGateAct = ctx->network()->addActivation(
        *addClip(ctx, itGate, clip), activations.at(0));
    ...
    nvinfer1::IActivationLayer *ftGateAct = ctx->network()->addActivation(
        *addClip(ctx, ftGate, clip), activations.at(0));
    ...
    nvinfer1::IActivationLayer *ctAct = ctx->network()->addActivation(
        *addClip(ctx, isolateGate(intermediatet, 3), clip), activations.at(1));
    ...
    nvinfer1::ITensor *operandIC =
        ctx->network()
            ->addElementWise(*itGate, *ctGate, eOp::kPROD)
            ->getOutput(0);
    nvinfer1::ITensor *operandFC =
        ctx->network()
            ->addElementWise(*ftGate, *Ct1->getOutput(0), eOp::kPROD)
            ->getOutput(0);
    nvinfer1::ITensor *Ct =
        ctx->network()
    ...
    nvinfer1::ITensor *singlePassShape =
        ctx->network()
            ->addElementWise(*gateOutputShape,
                             *addConstant(ctx,
                                          std::vector<int>{numDirections, 1, 1},
                                          onnx::TensorProto_DataType_INT32,
                                          nvinfer1::Dims{1, 3})
                                  ->getOutput(0),
                             eOp::kDIV)
    ...
    if (inputs.size() > 4 && inputs.at(4)) {
        ...
        Ct = numDirections == 2
                 ? maskBidirRNNHidden(ctx, node, loop, seqLens, maxLen,
                                      Ct1->getOutput(0), Ct, singlePassShape)
                 : maskRNNHidden(ctx, node, loop, seqLens, Ct1->getOutput(0),
                                 Ct, maxLen, direction == "reverse");
    }
    ...
    nvinfer1::IActivationLayer *otGateAct = ctx->network()->addActivation(
        *addClip(ctx, otGate, clip), activations.at(0));
    otGateAct->setAlpha(activationAlphas.at(0));
    otGateAct->setBeta(activationBetas.at(0));
    otGate = otGateAct->getOutput(0);

    // H(t) = o(t) . h(C(t))
    nvinfer1::IActivationLayer *hAct = ctx->network()->addActivation(
        *addClip(ctx, Ct, clip), activations.at(2));
    hAct->setAlpha(activationAlphas.at(2));
    hAct->setBeta(activationBetas.at(2));

    nvinfer1::ITensor *Ht =
        ctx->network()
            ->addElementWise(*otGate, *hAct->getOutput(0), eOp::kPROD)
            ->getOutput(0);
    if (inputs.size() > 4 && inputs.at(4)) {
        nvinfer1::ITensor *seqLens = &convertToTensor(inputs.at(4), ctx);
        auto maxLen = getAxisLength(ctx, input, 0);
    ...
    outputs.emplace_back(loop->addLoopOutput(*Ct1->getOutput(0),
                                             nvinfer1::LoopOutput::kLAST_VALUE)
                             ->getOutput(0));
...
DEFINE_BUILTIN_OP_IMPORTER(LpNormalization) {
    ...
    ASSERT((dt == DataType::kFLOAT || dt == DataType::kHALF) &&
               "Only float inputs/outputs supported in LpNormalization.",
           ErrorCode::kINVALID_NODE);

    CHECK(convertAxis(axis, nbDims));

    ASSERT((p == 1 || p == 2) && "Only L1 and L2 normalization are supported.",
           ErrorCode::kINVALID_NODE);
    nvinfer1::ITensor *norm{nullptr};
    TensorOrWeights zeros =
        ctx->createTempWeights(onnx::TensorProto::FLOAT, {0, {}});
    ...
    if (p == 1) {
        // abs(x)
        nvinfer1::IUnaryLayer *absLayer =
            ctx->network()->addUnary(*input, uOp::kABS);
        ...
        nvinfer1::IReduceLayer *reduceLayer =
            ctx->network()->addReduce(*norm, rOp::kSUM, 1 << axis, true);
        ctx->registerLayer(reduceLayer, node);
        norm = reduceLayer->getOutput(0);
    } else if (p == 2) {
        // x^2
        auto *sqrLayer =
            ctx->network()->addElementWise(*input, *input, eOp::kPROD);
...
DEFINE_BUILTIN_OP_IMPORTER(LpPool) {
    ...
    nvinfer1::PaddingMode paddingMode;
    bool exclude_padding(false);
    getKernelParams(ctx, node, &kernelShape, &strides, &begPadding, &endPadding,
                    paddingMode, exclude_padding);
    ...
    nvinfer1::ITensor *kernelSzTensor =
        addConstantScalar(ctx, kernelSz, onnx::TensorProto::FLOAT, scalarDims)
            ->getOutput(0);
    ...
    nvinfer1::IPoolingLayer *poolLayer =
        ctx->network()->addPoolingNd(*output, pType::kAVERAGE, kernelShape);
    ...
    auto *correctedSumLayer =
        ctx->network()->addElementWise(*output, *kernelSzTensor, eOp::kPROD);
    ...
    if (p == 2) {
        nvinfer1::IUnaryLayer *sqrtLayer =
            ctx->network()->addUnary(*output, uOp::kSQRT);
        ctx->registerLayer(sqrtLayer, node);
...
DEFINE_BUILTIN_OP_IMPORTER(MatMul) {
    ...
    if (t1Dims > t2Dims && t2Dims == 1) {
        // The second input is 1-D vector, promote to matrix by appending 1 in
        // shape.
        std::vector<int32_t> axes{1};
        inputB = unsqueezeTensor(ctx, node, *inputB, axes);
        needSqueezeTail = true;
    } else if (t1Dims < t2Dims && t1Dims == 1) {
        // The first argument is 1-D, promote to matrix by prepending a 1 in
        // shape. This is done in broadcast extra dimensions.
...
DEFINE_BUILTIN_OP_IMPORTER(MaxPool) {
    ASSERT(node.output().size() == 1 &&
               "TensorRT does not support the indices output in MaxPool!",
           ErrorCode::kUNSUPPORTED_NODE);
    return poolingHelper(ctx, node, inputs, nvinfer1::PoolingType::kMAX);
}

DEFINE_BUILTIN_OP_IMPORTER(Mean) {
    auto sum_result = elementwiseHelper(ctx, node, inputs,
                                        nvinfer1::ElementWiseOperation::kSUM);
    ...
    ASSERT(constant_layer && "Failed to create the scalar tensor.",
           ErrorCode::kUNSUPPORTED_NODE);
...
DEFINE_BUILTIN_OP_IMPORTER(MeanVarianceNormalization) {
    ...
    // The current formula avoids (E(x^2) - E(x)^2) < 0 caused by float point
    // precision errors
    using eOp = nvinfer1::ElementWiseOperation;
    using uOp = nvinfer1::UnaryOperation;
    using rOp = nvinfer1::ReduceOperation;
    ...
    ASSERT((dt == DataType::kFLOAT || dt == DataType::kHALF) &&
               "Only float/half inputs/outputs supported in "
    ...
    auto *reduceLayer =
        ctx->network()->addReduce(*input, rOp::kAVG, axesMask, true);
    ctx->registerLayer(reduceLayer, node);
    auto *meanX = reduceLayer->getOutput(0);

    // numerator: x-mean(x)
    auto *numSubLayer =
        ctx->network()->addElementWise(*input, *meanX, eOp::kSUB);
    ctx->registerLayer(numSubLayer, node);
    auto *numerator = numSubLayer->getOutput(0);

    // (x-mean(x))^2
    auto *sqrLayer =
        ctx->network()->addElementWise(*numerator, *numerator, eOp::kPROD);
    ctx->registerLayer(sqrLayer, node);
    auto *sqrNumerator = sqrLayer->getOutput(0);

    // mean((x-mean(x))^2)
    auto *meanLayer =
        ctx->network()->addReduce(*sqrNumerator, rOp::kAVG, axesMask, true);
    ctx->registerLayer(meanLayer, node);
    auto *variance = meanLayer->getOutput(0);

    // sqrt(mean((x-mean(x))^2))
    nvinfer1::IUnaryLayer *sqrtLayer =
        ctx->network()->addUnary(*variance, uOp::kSQRT);
    ctx->registerLayer(sqrtLayer, node);
    ...
    std::fill(scalarShape.d, scalarShape.d + scalarShape.nbDims, 1);
    auto *epsilonTensor =
        addConstantScalar(ctx, 1e-9f, onnx::TensorProto_DataType_FLOAT,
                          scalarShape)
            ->getOutput(0);
    auto *addEpsLayer =
        ctx->network()->addElementWise(*stdDev, *epsilonTensor, eOp::kSUM);
    ctx->registerLayer(addEpsLayer, node);
    stdDev = addEpsLayer->getOutput(0);

    // division numerator/standard-deviation
    auto *divLayer =
        ctx->network()->addElementWise(*numerator, *stdDev, eOp::kDIV);
    ctx->registerLayer(divLayer, node);
...
DEFINE_BUILTIN_OP_IMPORTER(Mod) {
    ...
    if (fmod == 0) {
        ...
        ASSERT((input0->getType() == DataType::kINT32) &&
                   "The fmod attribute is set to 0. Inputs cannot be of "
                   "floating point types.",
               ErrorCode::kINVALID_NODE);
        // Result = input0 - (input1 * floorDiv(input0, input1))
        nvinfer1::IElementWiseLayer *resultLayer =
            modWithIntegerInputs(ctx, input0, input1, false);
    ...
    // Fmod with integer inputs
        ...
        nvinfer1::IElementWiseLayer *resultLayer =
            modWithIntegerInputs(ctx, input0, input1, true);
        ctx->registerLayer(resultLayer, node);
        RETURN_FIRST_OUTPUT(resultLayer);
    }
    // Fmod with floating point inputs
        ...
        TensorOrWeights divResult =
            elementwiseHelper(ctx, node, {input0, input1}, eOp::kDIV)
                .value()
                .at(0);
        ...
        auto *condition =
            greaterLessOrEqual(ctx, node, &divResult.tensor(), zero, true);
        auto *outputWithDivFloor = layerWithDivFloor->getOutput(0);
        auto *outputWithDivCeil = layerWithDivCeil->getOutput(0);

        // If (input0 / input1) >= 0, result = input0 - (input1 * floor(input0 /
        // input1)) Else result = input0 - (input1 * ceil(input0 / input1))
        auto *result = ctx->network()->addSelect(
            *condition, *outputWithDivFloor, *outputWithDivCeil);
...
DEFINE_BUILTIN_OP_IMPORTER(NonMaxSuppression) {
    ...
    ASSERT(inputs.size() >= 2 && inputs.size() <= 5 &&
               "The node requires between 2-5 inputs",
           ErrorCode::kUNSUPPORTED_NODE);

    // Input: boxes
    nvinfer1::ITensor *boxesTensorPtr = &convertToTensor(inputs.at(0), ctx);
    ASSERT(boxesTensorPtr->getDimensions().nbDims == 3 &&
               "The boxes tensor must be 3D",
           ErrorCode::kUNSUPPORTED_NODE);

    // Input: scores
    nvinfer1::ITensor *scoresTensorPtr = &convertToTensor(inputs.at(1), ctx);
    ASSERT(scoresTensorPtr->getDimensions().nbDims == 3 &&
               "The scores tensor must be 3D",
           ErrorCode::kUNSUPPORTED_NODE);
    ...
    if (inputs.size() >= 3 && !inputs.at(2).isNullTensor()) {
        ...
        ASSERT(maxOutputBoxesPerClassTensorPtr != nullptr &&
                   "The max_output_boxes_per_class tensor must be 0D",
               ErrorCode::kUNSUPPORTED_NODE);
    } else {
        auto *constantLayer = ctx->network()->addConstant(
            nvinfer1::Dims{0, {}},
            nvinfer1::Weights{DataType::kINT32, &maxOutputBoxesPerClassDefault,
                              1});
        ASSERT(constantLayer != nullptr && "Failed to add in constant for "
    ...
    if (inputs.size() >= 4 && !inputs.at(3).isNullTensor()) {
        ...
        ASSERT(iouThresholdTensorPtr != nullptr &&
                   "The iou_threshold tensor must be 0D",
               ErrorCode::kUNSUPPORTED_NODE);
    }
    ...
    if (inputs.size() >= 5 && !inputs.at(4).isNullTensor()) {
        ...
        ASSERT(scoreThresholdTensorPtr != nullptr &&
                   "The score_threshold tensor must be 0D",
               ErrorCode::kUNSUPPORTED_NODE);
    }

    // Transpose scores tensor from [batch, classes, bounding_boxes] to [batch,
    // bounding_boxes, classes]
    nvinfer1::Permutation perm{0, 2, 1};
    nvinfer1::ITensor *transposedScoresTensorPtr =
        transposeTensor(ctx, node, *scoresTensorPtr, perm);
    ASSERT(transposedScoresTensorPtr && "Failed to transpose the scores input.",
           ErrorCode::kUNSUPPORTED_NODE);

    // Create the NMS layer
    auto *layer =
        ctx->network()->addNMS(*boxesTensorPtr, *transposedScoresTensorPtr,
                               *maxOutputBoxesPerClassTensorPtr);
    ASSERT(layer != nullptr && "Failed to create NMS layer.",
           ErrorCode::kUNSUPPORTED_NODE);
    ctx->registerLayer(layer, node);
    ...
    int32_t const centerPointBox =
        OnnxAttrs{node, ctx}.get("center_point_box", 0);
    nvinfer1::BoundingBoxFormat fmt;
    switch (centerPointBox) {
    case 0:
        fmt = nvinfer1::BoundingBoxFormat::kCORNER_PAIRS;
        break;
    case 1:
        fmt = nvinfer1::BoundingBoxFormat::kCENTER_SIZES;
        break;
    default:
        ASSERT(0 && "Invalid value provided for the center_point_box attribute",
               ErrorCode::kUNSUPPORTED_NODE);
    }
...
DEFINE_BUILTIN_OP_IMPORTER(OneHot) {
    ...
    CHECK(notInvalidType(inputs.at(2), {"UINT8"}));
    ASSERT(node.input_size() && "OneHot must have exactly 3 inputs",
           ErrorCode::kINVALID_NODE);
    ...
    nvinfer1::ITensor *depth =
        &convertToTensor(inputs.at(1), ctx);  // tensor #1 in ONNX
    if (!inputs.at(1).isInt32()) {
        depth = castHelper(ctx, depth, DataType::kINT32);
    }
    depth = convertToScalar(ctx, depth);
    ASSERT(depth && "Failed to convert the depth to a scalar.",
           ErrorCode::kINVALID_NODE);
...
DEFINE_BUILTIN_OP_IMPORTER(Pad) {
    ...
    ASSERT(tensorPtr->getType() != DataType::kUINT8,
           ErrorCode::kUNSUPPORTED_NODE);
    ...
    if (ctx->getOpsetVersion() < 11) {
        ...
        // In opset >= 11, padding indicies and values moved from attributes to
        // inputs
        ...
        if (inputs.size() >= 3 && !inputs.at(2).isNullTensor()) {
            ...
            if (inputs.at(2).is_weights()) {
                ...
                ASSERT(
                    (padWeight.count() == 1) &&
                        "The input constant_value is required to be a scalar.",
                    ErrorCode::kINVALID_NODE);
                ...
                case onnx::TensorProto::FLOAT16:
                    value = float(reinterpret_cast<half_float::half const *>(
                        padWeight.values)[0]);
                    isValueSet = true;
                    break;
                default:
        ...
        if (inputs.size() == 4 && !inputs.at(3).isNullTensor()) {
            ASSERT(false && "TensorRT does not support dynamic axes for pad!",
                   ErrorCode::kUNSUPPORTED_NODE);
        }
    }
    ...
    if (onnxPadding.empty()) {
        ...
        ASSERT((onnxPaddingPtr->getDimensions().nbDims == 1) &&
                   "The pads input must be 1D.",
               ErrorCode::kUNSUPPORTED_NODE);
        ...
        auto pre =
            ctx->network()
                ->addSlice(*onnxPaddingPtr, nvinfer1::Dims{1, {0}},
                           nvinfer1::Dims{1, {nbDims}}, nvinfer1::Dims{1, {1}})
                ->getOutput(0);
        auto post =
            ctx->network()
                ->addSlice(*onnxPaddingPtr, nvinfer1::Dims{1, {nbDims}},
                           nvinfer1::Dims{1, {nbDims}}, nvinfer1::Dims{1, {1}})
                ->getOutput(0);

        std::vector<int32_t> const zerosVal(nbDims, 0);
        auto const zeros = addConstant(ctx, zerosVal, onnx::TensorProto::INT32,
                                       nvinfer1::Dims{1, {nbDims}})
                               ->getOutput(0);
        start = ctx->network()
                    ->addElementWise(*zeros, *pre,
                                     nvinfer1::ElementWiseOperation::kSUB)
                    ->getOutput(0);
        auto const totalPadding =
            ctx->network()
                ->addElementWise(*pre, *post,
                                 nvinfer1::ElementWiseOperation::kSUM)
                ->getOutput(0);
        size =
            ctx->network()
        ...
        if (std::all_of(onnxPadding.begin(), onnxPadding.end(),
        ...
        ASSERT(
            convertOnnxPadding(ctx, nbDims, onnxPadding, start, totalPadding) &&
                "Failed to convert padding!",
            ErrorCode::kUNSUPPORTED_NODE);
        size =
            ctx->network()
    ...
    ASSERT(layer && "Could not create padding layer",
           ErrorCode::kUNSUPPORTED_NODE);
    ...
    if (mode == "constant") {
        ...
        if (valuePtr) {
            ...
            case DataType::kHALF:
            case DataType::kINT8:
                fillValue = addConstant(ctx, std::vector<float>{value},
                                        onnx::TensorProto::FLOAT,
                                        nvinfer1::Dims{0, {0}})
                                ->getOutput(0);
                break;
            default:
                fillValue =
                    addConstant(
                        ctx, std::vector<int32_t>{static_cast<int32_t>(value)},
                        onnx::TensorProto::INT32, nvinfer1::Dims{0, {0}})
                        ->getOutput(0);
                break;
            }
            ASSERT(fillValue && "Could not create layer for constant_value",
                   ErrorCode::kUNSUPPORTED_NODE);
...
DEFINE_BUILTIN_OP_IMPORTER(ParametricSoftplus) {
    ...
    return activationHelper(ctx, node, inputs,
                            nvinfer1::ActivationType::kSOFTPLUS, &alpha, &beta);
}
...
DEFINE_BUILTIN_OP_IMPORTER(PRelu) {
    ...
    ASSERT((inputs.size() == 2) &&
               "The PRelu operator requires exactly 2 inputs.",
           ErrorCode::kINVALID_NODE);
    nvinfer1::ITensor *input = &convertToTensor(inputs.at(0), ctx);
...
NodeImportResult randomUniformHelper(IImporterContext *ctx,
                                     onnx::NodeProto const &node,
                                     ShapeTensor const &inputShape,
                                     OnnxAttrs const &attrs,
                                     DataType const &inputDType) {
    auto *fillLayer =
        addFill(ctx, inputShape, nvinfer1::FillOperation::kRANDOM_UNIFORM);
    ctx->registerLayer(fillLayer, node);

    // Set datatype of output:
    //      RandomUniform: dype is required and defaults to 1
    //      RandomUniformLike: dtype is optional and defaults to the same type
    //      as the input
    if (attrs.count("dtype")) {
        auto dtype = attrs.get<int32_t>("dtype", 1);
        switch (dtype) {
        case onnx::TensorProto::FLOAT:
            fillLayer->setOutputType(0, DataType::kFLOAT);
            break;
        case onnx::TensorProto::FLOAT16:
            fillLayer->setOutputType(0, DataType::kHALF);
            break;
        default:
            return MAKE_ERROR("Unsupported data type",
                              ErrorCode::kINVALID_VALUE);
        }
    } else {
    ...
    // TensorRT does not support "seed" field now. The support will be added in
    // future versions.
    if (attrs.count("seed")) {
        LOG_WARNING("TensorRT currently ignores the \"seed\" field in "
                    "RandomUniform op. Random seeds will be used.");
    }
...
DEFINE_BUILTIN_OP_IMPORTER(RandomUniform) {
    ...
    ShapeTensor const inputShape{
        1, std::vector<int64_t>(shapeAsIntList.begin(), shapeAsIntList.end())};
...
DEFINE_BUILTIN_OP_IMPORTER(RandomUniformLike) {
    ASSERT((inputs.size() == 1) &&
               "The RandomUniformLike operator requires exactly 1 input.",
           ErrorCode::kINTERNAL_ERROR);
    ASSERT((inputs.at(0).is_tensor()) &&
               "The input tensor cannot be an initializer.",
...
NodeImportResult randomNormalHelper(IImporterContext *ctx,
                                    onnx::NodeProto const &node,
                                    ShapeTensor const &inputShape,
                                    OnnxAttrs const &attrs,
                                    DataType const &inputDType) {
    auto *fillLayer =
        addFill(ctx, inputShape, nvinfer1::FillOperation::kRANDOM_NORMAL);
    ctx->registerLayer(fillLayer, node);

    // Set datatype of output:
    //      RandomNormal: dype is required and defaults to 1
    //      RandomNormalLike: dtype is optional and defaults to the same type as
    //      the input
    if (attrs.count("dtype")) {
        auto dtype = attrs.get<int32_t>("dtype", 1);
        switch (dtype) {
    ...
    if (attrs.count("seed")) {
        LOG_WARNING("TensorRT currently ignores the \"seed\" field in "
                    "RandomNormal op. Random seeds will be used.");
    }
...
DEFINE_BUILTIN_OP_IMPORTER(RandomNormalLike) {
    ASSERT((inputs.size() == 1) &&
               "The RandomNormalLike operator requires exactly 1 input.",
           ErrorCode::kINTERNAL_ERROR);
    ASSERT((inputs.at(0).is_tensor()) &&
               "The input tensor cannot be an initializer.",
           nvonnxparser::ErrorCode::kUNSUPPORTED_NODE);
...
DEFINE_BUILTIN_OP_IMPORTER(Range) {
    ASSERT((inputs.at(0).isInt32() || inputs.at(0).isFp32()) &&
               "This version of TensorRT only supports int32 and float input "
               "types for Range!",
           ErrorCode::kUNSUPPORTED_NODE);
    ...
    ASSERT((start.isFloat() == limit.isFloat() &&
            start.isFloat() == delta.isFloat()) &&
               "For range operator types for start, limit, and delta must be "
               "identical.",
           ErrorCode::kUNSUPPORTED_NODE);
    ...
    ShapeTensor const quotient =
        start.isFloat() ? castToInt32(ctx, fQuotient) : fQuotient;
    ShapeTensor const numberOfElements =
        max(ctx, sub(ctx, zero, quotient), zero);

    nvinfer1::IFillLayer *layer =
        addFill(ctx, convertTo1D(ctx, numberOfElements),
    ...
    if (start.allValuesKnown() && delta.allValuesKnown()) {
        ...
        if (!start.isFloat()) {
            // Set output type to INT32 for ranges that should be INT32, since
            // TRT only accepts double type for setAlpha and setBeta
            layer->setOutputType(0, DataType::kINT32);
        }
    } else {
...
DEFINE_BUILTIN_OP_IMPORTER(ReduceL1) {
    NodeImportResult abs_result =
        unaryHelper(ctx, node, inputs.at(0), nvinfer1::UnaryOperation::kABS);
...
DEFINE_BUILTIN_OP_IMPORTER(ReduceLogSumExp) {
    ...
    auto exp_result =
        unaryHelper(ctx, node, inputs.at(0), nvinfer1::UnaryOperation::kEXP);
...
DEFINE_BUILTIN_OP_IMPORTER(ReduceMax) {
    return reduceTensor(ctx, node, inputs.at(0),
                        nvinfer1::ReduceOperation::kMAX);
}
...
DEFINE_BUILTIN_OP_IMPORTER(ReduceSum) {
    if (ctx->getOpsetVersion() >= 13 && inputs.size() >= 2) {
        return reduceTensor(ctx, node, inputs.at(0),
                            nvinfer1::ReduceOperation::kSUM, inputs.at(1));
    } else {
        return reduceTensor(ctx, node, inputs.at(0),
                            nvinfer1::ReduceOperation::kSUM);
    }
...
DEFINE_BUILTIN_OP_IMPORTER(ReduceSumSquare) {
    ...
    ASSERT(sqr_layer && "Failed to add an ElementWise layer.",
           ErrorCode::kUNSUPPORTED_NODE);
    nvinfer1::ITensor *sqr_tensorPtr = sqr_layer->getOutput(0);
    return reduceTensor(ctx, node, sqr_tensorPtr,
                        nvinfer1::ReduceOperation::kSUM);
}
...
DEFINE_BUILTIN_OP_IMPORTER(Resize) {
    ...
    ASSERT((inputRank > 0) && "The input tensor cannot be a scalar.",
           ErrorCode::kUNSUPPORTED_NODE);
    // Add resize layer
    ...
    if (mode == "cubic") {
        ...
        ASSERT((mode == "nearest") && "Invalid Resize mode",
               ErrorCode::kUNSUPPORTED_NODE);
    }
    ...
    if (ctx->getOpsetVersion() >= 11) {
        ...
        transformationMode = attrs.get<std::string>(
            "coordinate_transformation_mode", "half_pixel");
        auto nearest_mode =
            attrs.get<std::string>("nearest_mode", "round_prefer_floor");

        ASSERT((transformationMode != "tf_half_pixel_for_nn" ||
                nearest_mode == "round_prefer_floor") &&
                   "This version of TensorRT only support round_prefer_floor "
                   "nearest mode in tf_half_pixel_for_nn!",
               ErrorCode::kUNSUPPORTED_NODE);
        ...
        if (transformationMode == "align_corners") {
            ...
            ASSERT(!"TensorRT only supports half_pixel, pytorch_half_pixel, "
                    "tf_half_pixel_for_nn, asymmetric and "
        ...
        if (transformationMode != "tf_half_pixel_for_nn") {
            ...
            auto excludeOutside =
                static_cast<bool>(attrs.get<int32_t>("exclude_outside", 0));
    ...
    // For opset 10 resize, the only supported mode is asymmetric resize with
    // scales. Nearest resizes use floor rounding.
    else {
    ...
    if (scales.is_weights()) {
        // TRT-15340: Remove this and use else path when safety support nbDims
        // == 1.
        ShapedWeights scales_weights = scales.weights();
        ASSERT((scales_weights.shape.nbDims == 1) &&
                   "The scales input must be 1D.",
               ErrorCode::kUNSUPPORTED_NODE);
        int32_t scaleSize = scales_weights.shape.d[0];
        ASSERT((scaleSize == inputRank) &&
                   "The shape of input scales must align with the input rank.",
               ErrorCode::kINVALID_NODE);
        float const *scaleValues =
            static_cast<float const *>(scales_weights.values);

        // check resize dims
        if (resizeMode == nvinfer1::ResizeMode::kLINEAR) {
            ASSERT(canUseNDResize(scaleSize, scaleValues, 3) &&
                       "This version of TensorRT only supports linear resizing "
                       "on the outermost 3 dimensions.",
                   ErrorCode::kUNSUPPORTED_NODE);
        } else if (resizeMode == nvinfer1::ResizeMode::kCUBIC) {
            ASSERT(canUseNDResize(scaleSize, scaleValues, 2) &&
                       "This version of TensorRT only supports cubic resizing "
                       "on the outermost 2 dimensions.",
                   ErrorCode::kUNSUPPORTED_NODE);
        }
...
DEFINE_BUILTIN_OP_IMPORTER(Reshape) {
    ...
    if (ctx->getOpsetVersion() >= 5) {
        ...
        if (attrs.count("allowzero")) {
            ...
            if (ctx->getOpsetVersion() < 14) {
                LOG_WARNING(getNodeName(node)
                            << ": Using attribute allowzero with opset < 14 is "
                               "a TensorRT extension.");
            }
        }
    }
    ...
    if (ctx->getOpsetVersion() >= 5) {
        ...
        shape = ShapeTensor(1, std::vector<int64_t>(shapeAsIntList.begin(),
                                                    shapeAsIntList.end()));
    }
    ...
    nvinfer1::IShuffleLayer *layer =
        addShuffle(ctx, data, shape, /*zeroIsPlaceholder=*/!allowZero);
    ctx->registerLayer(layer, node);
    RETURN_FIRST_OUTPUT(layer);
}

DEFINE_BUILTIN_OP_IMPORTER(ReverseSequence) {
    ASSERT((inputs.size() == 2) && "ReverseSequence expects two input tensors: "
                                   "input and sequence_lens",
           ErrorCode::kINVALID_NODE);
    CHECK(notInvalidType(inputs.at(0), {"UINT8"}));
    nvinfer1::ITensor *input = &convertToTensor(inputs.at(0), ctx);
    ...
    ASSERT((inputDims.nbDims >= 2) && "Rank of input must be at least two",
           ErrorCode::kUNSUPPORTED_NODE);
    ASSERT((sequenceLensDims.nbDims == 1) &&
               "Rank of sequence_lens must be one",
           ErrorCode::kUNSUPPORTED_NODE);

    OnnxAttrs attrs{node, ctx};
    int32_t const batchAxis = attrs.get<int32_t>("batch_axis", 1);
    int32_t const sequenceAxis = attrs.get<int32_t>("time_axis", 0);
    ASSERT((batchAxis >= 0 && batchAxis <= inputDims.nbDims) &&
               "Invalid batch_axis",
           ErrorCode::kUNSUPPORTED_NODE);
    ASSERT((sequenceAxis >= 0 && sequenceAxis <= inputDims.nbDims) &&
               "Invalid time_axis",
           ErrorCode::kUNSUPPORTED_NODE);
    ASSERT((batchAxis != sequenceAxis) &&
               "batch_axis and time_axis cannot be the same",
           ErrorCode::kUNSUPPORTED_NODE);

    auto layer = ctx->network()->addReverseSequence(*input, *sequenceLens);
    ctx->registerLayer(layer, node);
    ASSERT(layer && "Failed to add ReverseSequence layer.",
           ErrorCode::kUNSUPPORTED_NODE);
...
DEFINE_BUILTIN_OP_IMPORTER(RNN) {
    ...
    std::vector<nvinfer1::ActivationType> defaultActs{
    ...
    std::vector<nvinfer1::ActivationType> activations =
        attrs.get<std::vector<nvinfer1::ActivationType>>("activations",
                                                         defaultActs);
    ...
    // TODO: Support cases where in bidirectional RNNs, activations of reverse
    // iteration do not match forward pass.
    // TODO: This will require splitting the input tensor in the loop when
    // applying activations.
    if (numDirections == 2) {
        ASSERT(std::equal(activations.begin(),
                          ...
                          activations.begin() + NUM_ACTIVATIONS) &&
                   "The parser does not currently support cases where "
                   "activations for the reverse pass of the RNN do not match "
                   "the forward pass.",
               ErrorCode::kUNSUPPORTED_NODE);
        ASSERT(std::equal(activationAlphas.begin(),
                          activationAlphas.begin() + NUM_ACTIVATIONS,
    ...
    if (inputs.size() > 3 && inputs.at(3)) {
        ...
        nvinfer1::IShuffleLayer *reshapeBias =
            ctx->network()->addShuffle(*bias);
        reshapeBias->setReshapeDimensions(
            nvinfer1::Dims3{numDirections, 2, hiddenSize});
        reshapeBias->setZeroIsPlaceholder(false);
        LOG_VERBOSE("Reshaping bias to: "
                    << reshapeBias->getOutput(0)->getDimensions());
        combinedBias =
    ...
    // H(t-1)
    nvinfer1::IRecurrenceLayer *hiddenState =
        loop->addRecurrence(*initialHidden);
    ctx->registerLayer(hiddenState, node);
    LOG_VERBOSE(
        "Hidden state shape: " << hiddenState->getOutput(0)->getDimensions());
    ...
    nvinfer1::ITensor *ht1RT =
        ctx->network()
            ->addMatrixMultiply(
                *hiddenState->getOutput(0), nvinfer1::MatrixOperation::kNONE,
                *recurrenceWeights, nvinfer1::MatrixOperation::kTRANSPOSE)
            ->getOutput(0);
    LOG_VERBOSE("H(t-1) * R^T -> " << ht1RT->getDimensions());
    ...
    nvinfer1::ITensor *intermediatet =
        ctx->network()
            ->addElementWise(*xtWT, *ht1RT,
                             nvinfer1::ElementWiseOperation::kSUM)
            ->getOutput(0);
    if (combinedBias) {
        intermediatet =
            ctx->network()
                ->addElementWise(*intermediatet, *combinedBias,
                                 nvinfer1::ElementWiseOperation::kSUM)
                ->getOutput(0);
    }
    ...
    nvinfer1::IActivationLayer *hAct = ctx->network()->addActivation(
        *addClip(ctx, intermediatet, clip), activations.at(0));
    ...
    if (inputs.size() > 4 && inputs.at(4)) {
        ...
        Ht = numDirections == 2
                 ? maskBidirRNNHidden(ctx, node, loop, seqLens, maxLen,
                                      hiddenState->getOutput(0), Ht,
                                      singlePassShape)
                 : maskRNNHidden(ctx, node, loop, seqLens,
                                 hiddenState->getOutput(0), Ht, maxLen,
                                 direction == "reverse");
    }
    ...
    outputs.emplace_back(concatenateRNNOutputs(
        ...
        numDirections, inputs, direction == "reverse"));
    // Yh = last value of H(t)
    outputs.emplace_back(loop->addLoopOutput(*hiddenState->getOutput(0),
                                             nvinfer1::LoopOutput::kLAST_VALUE)
                             ->getOutput(0));
...
DEFINE_BUILTIN_OP_IMPORTER(RoiAlign) {
    ...
    ASSERT(roiDims.nbDims == 2 && roiDims.d[1] == 4 &&
               "Found incorrect dimensions for ROIs input!",
           ErrorCode::kUNSUPPORTED_NODE);
    ...
    if (ctx->getOpsetVersion() >= 16) {
        coordinateTransformationMode =
            attrs.get("coordinate_transformation_mode",
                      std::string("half_pixel")) == "half_pixel"
                ? 1
                : 0;
    } else {
    ...
    f.emplace_back("coordinate_transformation_mode",
                   &coordinateTransformationMode,
                   nvinfer1::PluginFieldType::kINT32, 1);
    f.emplace_back("mode", &mode, nvinfer1::PluginFieldType::kINT32, 1);
    f.emplace_back("output_height", &outputHeight,
                   nvinfer1::PluginFieldType::kINT32, 1);
    f.emplace_back("output_width", &outputWidth,
                   nvinfer1::PluginFieldType::kINT32, 1);
    f.emplace_back("sampling_ratio", &samplingRatio,
                   nvinfer1::PluginFieldType::kINT32, 1);
    f.emplace_back("spatial_scale", &spatialScale,
                   nvinfer1::PluginFieldType::kFLOAT32, 1);

    // Create plugin from registry
    auto const plugin =
        createPlugin(getNodeName(node),
                     importPluginCreator(ctx, pluginName, pluginVersion), f);

    ASSERT(plugin != nullptr &&
               "ROIAlign plugin was not found in the plugin registry!",
           ErrorCode::kUNSUPPORTED_NODE);

    nvinfer1::ITensor *const inputTensorsPtr[3] = {tensorPtr, roisPtr,
                                                   batchIndicesPtr};
...
DEFINE_BUILTIN_OP_IMPORTER(ScaledTanh) {
    ...
    float beta = attrs.get<float>("beta");
    return activationHelper(ctx, node, inputs,
                            nvinfer1::ActivationType::kSCALED_TANH, &alpha,
                            &beta);
}
...
DEFINE_BUILTIN_OP_IMPORTER(Scan) {
    ...
    if (opset8Offset == 1) {
        ASSERT(
            inputs.at(0).isNullTensor() &&
                "TensorRT doesn't support sequence_lens input for this node!",
            ErrorCode::kUNSUPPORTED_NODE);
    }
    ...
    std::fill(defaultScanInputArgs.begin(), defaultScanInputArgs.end(),
              opset8Offset);
    std::vector<int32_t> scanInputAxes(
        attrs.get("scan_input_axes", defaultScanInputArgs));

    // Populate scan input directions
    std::vector<int32_t> defaultInputScanDirection(nbScanInputs);
    std::fill(defaultInputScanDirection.begin(),
              defaultInputScanDirection.end(), 0);
    std::vector<int32_t> const scanInputDirections(
        attrs.get("scan_input_directions", defaultInputScanDirection));

    // Populate scan output axis
    std::vector<int32_t> defaultScanOutputArgs(nbScanOutputs);
    std::fill(defaultScanOutputArgs.begin(), defaultScanOutputArgs.end(),
              opset8Offset);
    std::vector<int32_t> scanOutputAxes(
        attrs.get("scan_output_axes", defaultScanOutputArgs));

    // Populate scan ouput directions
    std::vector<int32_t> defaultOutputScanDirection(nbScanOutputs);
    std::fill(defaultOutputScanDirection.begin(),
              defaultOutputScanDirection.end(), 0);
    std::vector<int32_t> const scanOutputDirections(
        attrs.get("scan_output_directions", defaultOutputScanDirection));
    ...
    auto loop = ctx->network()->addLoop();
    // When multiple scan inputs are present, scan behaves like zip, so it is
    // sufficient to use only one scan input to determine trip limit.
    nvinfer1::ITensor *tripLimit = getAxisLength(
        ctx, &convertToTensor(inputs.back(), ctx), scanInputAxes.back());
    loop->addTripLimit(*tripLimit, nvinfer1::TripLimit::kCOUNT);

    // Establish scope for names local to the subgraph.
    NameScope nameScope(*ctx);

    // Add initial state inputs using recurrent layers, and scan inputs using
    // iterators.
    std::vector<nvinfer1::IRecurrenceLayer *> stateVars{};
    for (int32_t i = 0; i < nbStateVars; ++i) {
        stateVars.emplace_back(loop->addRecurrence(
            convertToTensor(inputs.at(i + opset8Offset), ctx)));
        ctx->registerTensor(TensorOrWeights{stateVars.back()->getOutput(0)},
                            body.input(i).name());
    }
    ...
    for (int32_t i = 0; i < nbScanInputs; ++i) {
        const int32_t index =
            nbStateVars + i;  // Scan Inputs are after the state variables.
        nvinfer1::IIteratorLayer *scanInput = loop->addIterator(
            convertToTensor(inputs.at(index + opset8Offset), ctx));
        scanInput->setAxis(scanInputAxes.at(i));
        scanInput->setReverse(scanInputDirections.at(i) == 1);
        ctx->registerTensor(TensorOrWeights{scanInput->getOutput(0)},
                            body.input(index).name());
    }
    ...
    for (int32_t i = 0; i < nbScanOutputs; ++i) {
        ...
        auto const scanDirection = (scanOutputDirections.at(i) == 0)
                                       ? nvinfer1::LoopOutput::kCONCATENATE
        ...
        LOG_VERBOSE("For scan output: "
                    ...
                    << ", found matching tensor: " << scanOutput.getName()
                    << ", with shape: " << scanOutput.getDimensions()
                    << ". Using scan direction: "
                    << static_cast<int32_t>(scanDirection)
                    << ", and scan axis: " << scanAxis);
        nvinfer1::ILoopOutputLayer *trtScanOut =
            loop->addLoopOutput(scanOutput, scanDirection, scanAxis);
        trtScanOut->setInput(1, *tripLimit);
        nodeOutputs.emplace_back(trtScanOut->getOutput(0));
    }
...
DEFINE_BUILTIN_OP_IMPORTER(GridSample) {
    ...
    ASSERT((inputs.size() == 2) &&
               "TRT expects two input tensors: grid and input",
           ErrorCode::kINVALID_NODE);
    nvinfer1::ITensor &input = convertToTensor(inputs.at(0), ctx);
    ...
    ASSERT((gridRank > 0) && "The grid tensor cannot be a scalar.",
           ErrorCode::kUNSUPPORTED_NODE);
    ASSERT((gridRank == inputRank) &&
               "The input tensor and the grid tensor must have the same rank.",
           ErrorCode::kUNSUPPORTED_NODE);

    // Add grid sample layer
    nvinfer1::IGridSampleLayer *layer =
        ctx->network()->addGridSample(input, grid);
    ctx->registerLayer(layer, node);
    OnnxAttrs attrs(node, ctx);
...
DEFINE_BUILTIN_OP_IMPORTER(ScatterElements) {
    ...
    int32_t nbDims = inputs.at(0).shape().nbDims;
    CHECK(convertAxis(axis, nbDims));
    return addScatterLayer(ctx, node, inputs, nvinfer1::ScatterMode::kELEMENT,
                           axis);
}
...
DEFINE_BUILTIN_OP_IMPORTER(Scatter) {
    ...
    if (ctx->getOpsetVersion() >= 11) {
        LOG_WARNING("Scatter was deprecated in Opset 11. Node: \""
                    << getNodeName(node)
...
DEFINE_BUILTIN_OP_IMPORTER(Selu) {
    ...
    return activationHelper(ctx, node, inputs, nvinfer1::ActivationType::kSELU,
                            &alpha, &beta);
}
...
DEFINE_BUILTIN_OP_IMPORTER(Size) {
    ...
    // "outputs a int64 scalar that equals to the total number of elements of
    // the input tensor."
...
DEFINE_BUILTIN_OP_IMPORTER(Slice) {
    ...
    // If opset version >= 10 slice parameters are weights instead of
    // attributes.
    if (ctx->getOpsetVersion() >= 10) {
        ASSERT((nbInputs >= 3 && nbInputs <= 5) &&
                   "Post-opset 10 Slice operator requires 3 - 5 inputs.",
               ErrorCode::kUNSUPPORTED_NODE);
        starts = ShapeTensor{ctx, inputs.at(1)};
        ends = ShapeTensor{ctx, inputs.at(2)};
        // "If axes are omitted, they are set to [0, ..., ndim-1]."
        axes = nbInputs > 3 ? ShapeTensor(ctx, inputs.at(3))
                            : iotaShapeVector(dims.size());
        // Doesn't support dynamic axes currently.
        ASSERT((axes.allValuesKnown()) &&
                   "This version of TensorRT does not support dynamic axes.",
               ErrorCode::kUNSUPPORTED_NODE);
        ASSERT((starts.size() == axes.size()) &&
                   "The shape of input starts misaligns with the shape of "
                   "input axes.",
               ErrorCode::kUNSUPPORTED_NODE);
        ASSERT(ends.size() == axes.size() &&
                   "The shape of input ends misaligns with the shape of input "
                   "axes.",
               ErrorCode::kUNSUPPORTED_NODE);
        // "If steps are omitted, they are set to [1, ..., 1] of length
        // len(starts)."
        steps = inputs.size() > 4 ? ShapeTensor(ctx, inputs.at(4))
                                  : similar(ctx, starts, 1);
    } else {
        OnnxAttrs attrs(node, ctx);
        starts = ShapeTensor(1, attrs.get<std::vector<int64_t>>("starts"));
        ends = ShapeTensor(1, attrs.get<std::vector<int64_t>>("ends"));
        // "It's optional. If not present, will be treated as [0, 1, ...,
        // len(starts) - 1]."
        axes = attrs.count("axes")
                   ? ShapeTensor(1, attrs.get<std::vector<int64_t>>("axes"))
    ...
    for (int64_t axis : axes) {
        ...
        ASSERT((-r <= axis && axis < r) &&
                   "The range of axis must be in [-r, r-1], where r is the "
                   "rank of input data.",
               ErrorCode::kINVALID_VALUE);
    ...
    ASSERT((std::unordered_set<int64_t>(axes.begin(), axes.end()).size() ==
            static_cast<size_t>(axes.size())) &&
    ...
    if (axes.size() < dims.size() || !isIota) {
        ...
        ShapeTensor const subscripts{
            axesToInterlaceSubscripts(axes, dims.size())};
...
DEFINE_BUILTIN_OP_IMPORTER(SpaceToDepth) {
    ...
    // Input tensor is in NCHW format
    ASSERT((inputs.at(0).shape().nbDims == 4) &&
               "The input tensor must be in the NCHW format.",
           ErrorCode::kUNSUPPORTED_NODE);
    nvinfer1::ITensor *tensorPtr = &convertToTensor(inputs.at(0), ctx);
    ...
    // First reshape to {N, C, H / blockSize, blockSize, W / blockSize,
    // blockSize}

    auto const firstShapeDims =
        concat(ctx, N,
               concat(ctx, C,
                      concat(ctx, H_2,
                             concat(ctx, blockSizeTensor,
    ...
    auto secondShapeDims =
        concat(ctx, N, concat(ctx, C_2, concat(ctx, H_2, W_2)));
...
DEFINE_BUILTIN_OP_IMPORTER(Split) {
    ...
    bool const hasSplitList = (ctx->getOpsetVersion() >= 13)
                                  ? (inputs.size() == 2)
                                  : attrs.count("split");
    if (hasSplitList) {
        ...
        if (ctx->getOpsetVersion() >= 13) {
            if (inputs.at(1).is_weights()) {
                ...
                int32_t const *splitValues =
                    static_cast<int32_t const *>(splitWeights.values);
        ...
        ASSERT((splitList.empty() || (splitList.size() == numOutputs)) &&
                   "The shape of the split attribute misaligns with the number "
                   "of outputs.",
               ErrorCode::kINVALID_NODE);
    } else {
        // "Otherwise, the tensor is split to equal sized parts."
        sizeSliceAxis = floorDiv(ctx, gather(ctx, inputDims, shapeVector(axis)),
                                 shapeVector(numOutputs));
    ...
    for (int32_t i = 0; i < (int32_t)numOutputs; ++i) {
        ...
        if (hasSplitList) {
            if (splitList.empty()) {
                sizeSliceAxis = gather(ctx, splitSizesTensor,
                                       ShapeTensor(1, std::vector<int64_t>{i}));
            } else {
        ...
        nvinfer1::ISliceLayer *slice =
            addSlice(ctx, inputTensor, starts, sizes, ones);
        ctx->registerLayer(slice, node);
...
DEFINE_BUILTIN_OP_IMPORTER(Squeeze) {
    ...
    if (ctx->getOpsetVersion() >= 13) {
        if (inputs.size() == 2) {
            assertIsWeights(inputs.at(1),
                            "Squeeze axes input must be an initializer!");
    ...
    if (axes.size() == 0) {
        ...
        ASSERT(
            !isDynamic(shape) &&
                "Cannot infer squeeze dimensions from a dynamic shape! Please "
                "re-export your model with the Squeeze axes input set.",
            ErrorCode::kUNSUPPORTED_NODE);
    ...
    ASSERT(squeezed && "Failed to squeeze tensor!",
           ErrorCode::kUNSUPPORTED_NODE);
...
DEFINE_BUILTIN_OP_IMPORTER(ThresholdedRelu) {
    ...
    return activationHelper(
        ctx, node, inputs, nvinfer1::ActivationType::kTHRESHOLDED_RELU, &alpha);
}
...
DEFINE_BUILTIN_OP_IMPORTER(Tile) {
    ...
    nvinfer1::ISliceLayer *tile =
        addSlice(ctx, input, similar(ctx, inputDims, 0), outputShape,
                 similar(ctx, inputDims, 1));
...
DEFINE_BUILTIN_OP_IMPORTER(TopK) {
    ...
    int32_t k{1};
    if (ctx->getOpsetVersion() < 10) {
        ASSERT((attrs.count("k")) && "Attribute k is missing.",
               ErrorCode::kINVALID_NODE);
        k = attrs.get<int>("k");
    }
    int32_t nbDims = tensorPtr->getDimensions().nbDims;
    ...
    if (needToExpandDims) {
        ...
        tensorPtr = unsqueezeTensor(ctx, node, *tensorPtr, axes);
        ASSERT(tensorPtr && "Failed to unsqueeze input x.",
               ErrorCode::kUNSUPPORTED_NODE);
    }
    bool needCast = tensorPtr->getType() == nvinfer1::DataType::kINT32;
    if (needCast) {
        LOG_WARNING("TensorRT is using FLOAT32 precision to run an INT32 TopK. "
                    "Rounding errors may occur for large integer "
                    "values");
    ...
    nvinfer1::ITopKLayer *layer =
        ctx->network()->addTopK(*tensorPtr, operation, k, axisMask);
    if (ctx->getOpsetVersion() >= 10) {
        ASSERT((inputs.size() == 2) &&
                   "Expects two input tensors for opset >= 10: X and K",
               ErrorCode::kINVALID_NODE);
    ...
    if (needToExpandDims) {
        ...
        ASSERT(values && "Failed to squeeze the input values.",
               ErrorCode::kUNSUPPORTED_NODE);
        indices = squeezeTensor(ctx, node, *indices, axes);
        ASSERT(indices && "Failed to squeeze the input indices.",
               ErrorCode::kUNSUPPORTED_NODE);
    }
...
DEFINE_BUILTIN_OP_IMPORTER(Transpose) {
    ...
    ASSERT((ndim <= nvinfer1::Dims::MAX_DIMS) &&
               "The rank of the input tensor exceeds the maximum supported by "
    ...
    nvinfer1::ITensor &itensor =
        input.is_tensor() ? input.tensor() : convertToTensor(input, ctx);
    nvinfer1::ITensor *output_tensor =
        transposeTensor(ctx, node, itensor, perm);
    ASSERT(output_tensor && "Failed to transpose the input.",
           ErrorCode::kUNSUPPORTED_NODE);
...
DEFINE_BUILTIN_OP_IMPORTER(Trilu) {
    ...
    ASSERT((nbDims >= 2) && "Trilu input must have at least 2 dimensions!",
           ErrorCode::kINVALID_NODE);
    OnnxAttrs attrs(node, ctx);
    ...
    const ShapeTensor iotadims =
        concat(ctx, gather(ctx, shapeOf(*data), shapeVector(N)),
               gather(ctx, shapeOf(*data), shapeVector(M)));

    // Trilu can be represented via trl(A) = select(R >= C, A, 0) for keeping
    // the lower diagonals Simiarly, trl(A) = select(R <= C, A, 0) represents
    // keeping the upper diagonals

    auto *rows = iota(ctx, iotadims, 0);
    auto *cols = iota(ctx, iotadims, 1);
    auto *zero = createZeroTensor(ctx, data);

    // k tensor shifts the number of diagonals we accept. Positive means to
    // include k number of diagonals above the main, while a negative k means to
    // exclude the main and k number of diagonals below the main. Adjust column
    // tensor accordingly.
    if (inputs.size() == 2) {
        auto *k = &convertToTensor(inputs.at(1), ctx);
        cols = &elementwiseHelper(ctx, node, {cols, k}, eOp::kSUB)
                    .value()
                    .at(0)
                    .tensor();
    }

    // Unsqueeze to broadcast rows/cols if necessary during next elementwise
    // operation.
    if (nbDims > 2) {
...
DEFINE_BUILTIN_OP_IMPORTER(Unsqueeze) {
    ...
    if (ctx->getOpsetVersion() >= 13) {
        ...
        if (inputs.size() == 2) {
            ...
            ASSERT(axesInput.allValuesKnown() &&
                       "Axes input for unsqueeze operation should be a "
                       "constant tensor.",
                   ErrorCode::kUNSUPPORTED_NODE);
    ...
    ASSERT(expanded && "Failed to unsqueeze tensor!",
           ErrorCode::kUNSUPPORTED_NODE);
...
DEFINE_BUILTIN_OP_IMPORTER(Upsample) {
    ...
    ASSERT((nbDims > 0) && "The input tensor cannot be a scalar.",
           ErrorCode::kUNSUPPORTED_NODE);
    OnnxAttrs attrs(node, ctx);
    ...
    // resize.
    nvinfer1::ResizeMode resizeMode = (mode == "linear" || mode == "bilinear")
    ...
    if (ctx->getOpsetVersion() >= 9) {
        ...
        ASSERT((inputs.size() == 2) &&
                   "Operator Upsample requires exactly 2 inputs.",
               ErrorCode::kINVALID_NODE);
        auto scales_input = inputs.at(1);
        if (scales_input.is_weights()) {
            // TRT-15340: Remove this and use else path when safety support
            // nbDims == 1.
            ...
            ASSERT((scales_weights.count() == static_cast<size_t>(nbDims)) &&
                       "The shape of the scales input must align with the "
                       "dimensions of the input.",
                   ErrorCode::kUNSUPPORTED_NODE);
            ASSERT((scales_weights.type == onnx::TensorProto::FLOAT) &&
                       "This version of TensorRT only supports FLOAT scales "
                       "input.",
                   ErrorCode::kINVALID_NODE);
            float const *scales_ptr =
                static_cast<float const *>(scales_weights.values);
            ...
            if (mode == "linear" || mode == "bilinear") {
                ASSERT(canUseNDResize(scale_factors.size(),
                                      &scale_factors.front(), 3) &&
                           "This version of TensorRT only supports linear "
                           "resizing on the outermost 3 dimensions",
                       ErrorCode::kUNSUPPORTED_NODE);
            }
            layer->setScales(scale_factors.data(), nbDims);
        } else {
            nvinfer1::ITensor *resizeShape =
                resizeShapeTensor(ctx, tensor, scales_input);
            nvinfer1::Dims const outDims = resizeShape->getDimensions();
            ASSERT((outDims.nbDims == 1) && "The scales input must be 1D.",
                   ErrorCode::kUNSUPPORTED_NODE);
            // Scale factors has batch dimension.
            ASSERT((outDims.d[0] == nbDims) &&
                       ...
                       "dimensions of the input.",
                   ErrorCode::kUNSUPPORTED_NODE);
            ASSERT((resizeShape->getType() == DataType::kINT32) &&
                       "Resize output shape type must be integral.",
                   ErrorCode::kINVALID_NODE);
            layer->setInput(1, *resizeShape);
        }
    } else {
        // TRT-15340: Adapt to use resizeShapeTensor instead when safety support
        // nbDims == 1.
        ASSERT(attrs.count("scales") && "Attribute scales is missing.",
               ErrorCode::kUNSUPPORTED_NODE);
        ...
        if (mode == "linear" || mode == "bilinear") {
            ASSERT(canUseNDResize(scale_factors.size(), &scale_factors.front(),
                                  3) &&
                       "This version of TensorRT only supports linear resizing "
                       "on the outermost 3 dimensions",
                   ErrorCode::kUNSUPPORTED_NODE);
...
DEFINE_BUILTIN_OP_IMPORTER(Where) {
    ...
    ASSERT((cDims.nbDims == xDims.nbDims) &&
               "The shape of the condition input tensor must be the same of "
               "the input x tensor.",
           ErrorCode::kUNSUPPORTED_NODE);
...
// Copies the given field into the fieldData map, returns data and number of T
// elements in the vector in which the data was copied into.
template <typename T>
std::tuple<void const *, size_t>
copyField(T const &field, std::string const &fieldName,
    ...
    return std::make_tuple(fieldData[fieldName].data(),
                           fieldData[fieldName].size() / nbBytes);
}

template <typename T>
std::tuple<void const *, size_t>
copyField(std::vector<T> const &repeatedField, std::string const &fieldName,
          string_map<std::vector<uint8_t>> &fieldData) {
    size_t const nbBytes{sizeof(T) * repeatedField.size()};
    fieldData[fieldName].resize(nbBytes);
    std::memcpy(fieldData[fieldName].data(), repeatedField.data(), nbBytes);
    return std::make_tuple(fieldData[fieldName].data(),
                           fieldData[fieldName].size() / sizeof(T));
}

std::tuple<void const *, size_t>
copyField(std::string const &field, std::string const &fieldName,
          string_map<std::vector<uint8_t>> &fieldData) {
    static_assert(sizeof(std::string::value_type) == sizeof(uint8_t),
                  "String type does not have 1 byte elements");
    std::copy(field.begin(), field.end(),
              std::back_inserter(fieldData[fieldName]));
    // Append \0 as end of C style string.
    fieldData[fieldName].push_back('\0');
    return std::make_tuple(fieldData[fieldName].data(),
                           fieldData[fieldName].size());
}

std::tuple<void const *, size_t>
copyField(std::vector<std::string> const &repeatedField,
...
std::tuple<void const *, size_t>
copyField(ShapedWeights const &field, std::string const &fieldName,
          string_map<std::vector<uint8_t>> &fieldData) {
    // Weights do not require a copy
    return std::make_tuple(field.values, field.count());
}

// Load plugin fields from an ONNX node, using fieldData for temporary
// allocations.
std::vector<nvinfer1::PluginField>
loadFields(string_map<std::vector<uint8_t>> &fieldData, OnnxAttrs const &attrs,
           nvinfer1::PluginFieldCollection const *fieldNames,
           IImporterContext *ctx) {
    std::vector<nvinfer1::PluginField> fields{};
    for (int32_t i = 0; i < fieldNames->nbFields; ++i) {
        // Some plugins may have default values for fields that map to optional
        // attributes in an ONNX graph.
        if (!attrs.count(fieldNames->fields[i].name)) {
            LOG_WARNING("Attribute "
                        << fieldNames->fields[i].name
                        << " not found in plugin node! Ensure that the plugin "
                           "creator has a default value "
                           "defined or the engine may fail to build.");
            continue;
        }

        // Name must be retrieved from the map so that it is alive for long
        // enough.
        std::string const &fieldName =
            fieldData
                .emplace(fieldNames->fields[i].name, std::vector<uint8_t>{})
                .first->first;
        ...
        case onnx::AttributeProto::FLOAT:
            std::tie(data, length) =
                copyField(attrs.get<float>(fieldName), fieldName, fieldData);
            type = nvinfer1::PluginFieldType::kFLOAT32;
            break;
        case onnx::AttributeProto::INT:
            std::tie(data, length) =
                copyField(attrs.get<int32_t>(fieldName), fieldName, fieldData);
            type = nvinfer1::PluginFieldType::kINT32;
            break;
        case onnx::AttributeProto::STRING:
            std::tie(data, length) = copyField(
                attrs.get<std::string>(fieldName), fieldName, fieldData);
            type = nvinfer1::PluginFieldType::kCHAR;
            break;
        case onnx::AttributeProto::FLOATS:
            std::tie(data, length) = copyField(
                attrs.get<std::vector<float>>(fieldName), fieldName, fieldData);
            type = nvinfer1::PluginFieldType::kFLOAT32;
            break;
        case onnx::AttributeProto::INTS:
            std::tie(data, length) = copyField(
                attrs.get<std::vector<int>>(fieldName), fieldName, fieldData);
            type = nvinfer1::PluginFieldType::kINT32;
            break;
        case onnx::AttributeProto::STRINGS:
            std::tie(data, length) =
                copyField(attrs.get<std::vector<std::string>>(fieldName),
                          fieldName, fieldData);
            type = nvinfer1::PluginFieldType::kCHAR;
            break;
        case onnx::AttributeProto::TENSOR: {
            ...
            case onnx::TensorProto::INT8:
                type = nvinfer1::PluginFieldType::kINT8;
                break;
            case onnx::TensorProto::INT16:
                type = nvinfer1::PluginFieldType::kINT16;
                break;
            ...
            case onnx::TensorProto::STRING:
                ...
                break;
            case onnx::TensorProto::FLOAT16:
                type = nvinfer1::PluginFieldType::kFLOAT16;
                break;
            case onnx::TensorProto::DOUBLE:
                type = nvinfer1::PluginFieldType::kFLOAT64;
                break;
...
DEFINE_BUILTIN_OP_IMPORTER(Shrink) {
    ...
    ASSERT(
        (originalType == DataType::kFLOAT || originalType == DataType::kHALF ||
         originalType == DataType::kINT8 || originalType == DataType::kINT32) &&
            "Only FLOAT, HALF, INT8 and INT32 are supported in Shrink.",
        ErrorCode::kUNSUPPORTED_NODE);
    ...
    nvinfer1::ITensor *lambdTensor =
        addConstant(ctx, std::vector<float>{lambd}, onnx::TensorProto::FLOAT,
                    {0, {1}})
            ->getOutput(0);
    ...
    nvinfer1::ITensor *biasTensor =
        addConstant(ctx, std::vector<float>{bias}, onnx::TensorProto::FLOAT,
                    {0, {1}})
            ->getOutput(0);
    CHECK(broadcastTensors(ctx, biasTensor, x));

    nvinfer1::ITensor *zeroTensor =
        addConstant(ctx, std::vector<float>{0.}, onnx::TensorProto::FLOAT,
                    {0, {1}})
            ->getOutput(0);
    ...
    nvinfer1::ITensor *xGreaterThanLambd =
        &elementwiseHelper(ctx, node, {x, lambdTensor},
                           nvinfer1::ElementWiseOperation::kGREATER)
             .value()
             .at(0)
             .tensor();
    nvinfer1::ITensor *xMinusBias =
        &elementwiseHelper(ctx, node, {x, biasTensor},
                           nvinfer1::ElementWiseOperation::kSUB)
             .value()
             .at(0)
             .tensor();
    nvinfer1::ITensor *output =
        ctx->network()
            ->addSelect(*xGreaterThanLambd, *xMinusBias, *zeroTensor)
            ->getOutput(0);

    // If x < -lambd, y = x + bias;
    nvinfer1::ITensor *xLessThanMinusLambd =
        &elementwiseHelper(ctx, node, {x, negLambdTensor},
                           nvinfer1::ElementWiseOperation::kLESS)
             .value()
             .at(0)
             .tensor();
    ...
    auto *layer =
        ctx->network()->addSelect(*xLessThanMinusLambd, *xAddBias, *output);
    ctx->registerLayer(layer, node);
...
DEFINE_BUILTIN_OP_IMPORTER(HardSwish) {
    nvinfer1::ITensor *x = &convertToTensor(inputs.at(0), ctx);
    ASSERT((x->getType() == DataType::kFLOAT ||
            x->getType() == DataType::kHALF ||
            x->getType() == DataType::kINT8) &&
               "Only FLOAT, HALF or INT8 input is supported for the HardSwish "
               "operator in this version of TensorRT",
           ErrorCode::kUNSUPPORTED_NODE);
    ...
    nvinfer1::ITensor *hardSigmoid =
        &activationHelper(ctx, node, inputs,
                          nvinfer1::ActivationType::kHARD_SIGMOID, &kALPHA,
                          &kBETA)
             .value()
             .at(0)
             .tensor();

    return elementwiseHelper(ctx, node, {x, hardSigmoid},
                             nvinfer1::ElementWiseOperation::kPROD);
}
...
DEFINE_BUILTIN_OP_IMPORTER(NonZero) {
    nvinfer1::ITensor *x = &convertToTensor(inputs.at(0), ctx);
    ASSERT(
        (x->getType() == DataType::kFLOAT || x->getType() == DataType::kHALF ||
         x->getType() == DataType::kINT32 || x->getType() == DataType::kINT8 ||
         x->getType() == DataType::kBOOL) &&
            "Only FLOAT, HALF, INT32, INT8 or BOOL input is supported for the "
            "NonZero operator in this version of TensorRT",
        ErrorCode::kUNSUPPORTED_NODE);
...
DEFINE_BUILTIN_OP_IMPORTER(FallbackPluginImporter) {
    ...
    std::string const pluginVersion{
        attrs.get<std::string>("plugin_version", "1")};
    std::string const pluginNamespace{
        attrs.get<std::string>("plugin_namespace", "")};

    LOG_INFO("Searching for plugin: "
             << pluginName << ", plugin_version: " << pluginVersion
             << ", plugin_namespace: " << pluginNamespace);
    nvinfer1::IPluginCreator *creator =
        importPluginCreator(ctx, pluginName, pluginVersion, pluginNamespace);
    ASSERT(creator && "Plugin not found, are the plugin name, version, and "
                      "namespace correct?",
           ErrorCode::kUNSUPPORTED_NODE);

    nvinfer1::PluginFieldCollection const *fieldNames =
        creator->getFieldNames();
    // Field data needs to be type erased, we use fieldData for temporary
    // allocations.
    string_map<std::vector<uint8_t>> fieldData{};
    std::vector<nvinfer1::PluginField> fields =
        loadFields(fieldData, attrs, fieldNames, ctx);
    ...
    auto *layer = ctx->network()->addPluginV2(pluginInputs.data(),
                                              pluginInputs.size(), *plugin);
    ctx->registerLayer(layer, node);
...
DEFINE_BUILTIN_OP_IMPORTER(TRT_Scale) {
    ASSERT((inputs.size() >= 1) && "Input is required.",
           nvonnxparser::ErrorCode::kINVALID_NODE);
    ASSERT((inputs.at(0).is_tensor()) && "The first input must be a tensor.",
           nvonnxparser::ErrorCode::kINVALID_NODE);
    if (inputs.size() >= 2) {
        ASSERT((inputs.at(1).is_weights()) &&
                   "The second input must be an initializer.",
               nvonnxparser::ErrorCode::kINVALID_NODE);
    }
    auto &input = inputs.at(0).tensor();
    ...
    auto type = inputs.size() > 1
                    ? inputs.at(1).weights().type
                    : trtDataTypeToONNX(inputs.at(0).tensor().getType());
    ...
    if (attrs.get<bool>("scale")) {
        ASSERT((inputs.at(counter).is_weights()) &&
                   "The scale input must be an initializer.",
               nvonnxparser::ErrorCode::kUNSUPPORTED_NODE);
        scale = inputs.at(counter++).weights();
    }
    if (attrs.get<bool>("shift")) {
        ASSERT((inputs.at(counter).is_weights()) &&
                   "The shift input must be an initializer.",
               nvonnxparser::ErrorCode::kUNSUPPORTED_NODE);
        shift = inputs.at(counter++).weights();
    }
    if (attrs.get<bool>("power")) {
        ASSERT((inputs.at(counter).is_weights()) &&
                   "The power input must be an initializer.",
               nvonnxparser::ErrorCode::kUNSUPPORTED_NODE);
        power = inputs.at(counter++).weights();
    }

    nvinfer1::IScaleLayer *layer =
        ctx->network()->addScale(input, mode, shift, scale, power);
    ctx->registerLayer(layer, node);
    RETURN_FIRST_OUTPUT(layer);
}
...
DEFINE_BUILTIN_OP_IMPORTER(TRT_Shuffle) {
    ...
    OnnxAttrs attrs(node, ctx);
    nvinfer1::Permutation perm1 =
        attrs.get<nvinfer1::Permutation>("first_perm");
    nvinfer1::Permutation perm2 =
        attrs.get<nvinfer1::Permutation>("second_perm");
    ...
    if (inputs.size() == 1) {
        if (attrs.count("reshape_dims")) {
            nvinfer1::Dims reshapeDims =
                attrs.get<nvinfer1::Dims>("reshape_dims");
            layer->setReshapeDimensions(reshapeDims);
        }
    } else {
        ASSERT(inputs.at(1).is_tensor() && "The second input must be a tensor.",
               nvonnxparser::ErrorCode::kINVALID_NODE);
...
DEFINE_BUILTIN_OP_IMPORTER(TRT_TopK_Min) {
    ...
    nvinfer1::ITopKLayer *layer =
        ctx->network()->addTopK(input, nvinfer1::TopKOperation::kMIN, k, axes);
    ctx->registerLayer(layer, node);
...
DEFINE_BUILTIN_OP_IMPORTER(TRT_MatMul) {
    ...
    nvinfer1::MatrixOperation op0 =
        attrs.get<nvinfer1::MatrixOperation>("op_0");
    nvinfer1::MatrixOperation op1 =
        attrs.get<nvinfer1::MatrixOperation>("op_1");

    nvinfer1::IMatrixMultiplyLayer *layer =
        ctx->network()->addMatrixMultiply(input0, op0, input1, op1);
    ctx->registerLayer(layer, node);
    RETURN_FIRST_OUTPUT(layer);
}

typedef std::function<void(int32_t, nvinfer1::RNNGateType, nvinfer1::Weights)>
    RNNWeightsAdder;

bool addRNNv2Weights(RNNWeightsAdder adder, int32_t layerNb,
                     std::vector<nvinfer1::RNNGateType> const &gates,
...
DEFINE_BUILTIN_OP_IMPORTER(TRT_RNNv2) {
    ...
    nvinfer1::RNNInputMode inputMode =
        attrs.get<nvinfer1::RNNInputMode>("input_mode");
    nvinfer1::RNNDirection direction =
        attrs.get<nvinfer1::RNNDirection>("direction");
    ...
    nvinfer1::IRNNv2Layer *layer =
        ctx->network()->addRNNv2(input, layerCount, hiddenSize, maxSeqLen, op);
    ctx->registerLayer(layer, node);
    layer->setInputMode(inputMode);
    layer->setDirection(direction);

    if (attrs.get<bool>("has_hidden_state")) {
        ASSERT(inputs.at(counter).is_tensor() &&
                   "The input hidden_state must be a tensor.",
               nvonnxparser::ErrorCode::kINVALID_NODE);
        layer->setHiddenState(inputs.at(counter++).tensor());
    }
    if (op == nvinfer1::RNNOperation::kLSTM &&
        attrs.get<bool>("has_cell_state", false)) {
        ASSERT(inputs.at(counter).is_tensor() &&
                   "The input cell_state must be a tensor.",
               nvonnxparser::ErrorCode::kINVALID_NODE);
        layer->setCellState(inputs.at(counter++).tensor());
    }
    if (attrs.get<bool>("has_seq_lengths")) {
        ASSERT(inputs.at(counter).is_tensor() &&
                   "The input seq_lengths must be a tensor.",
               nvonnxparser::ErrorCode::kINVALID_NODE);
        layer->setSequenceLengths(inputs.at(counter++).tensor());
    }

    int32_t nbWeights =
        (direction == nvinfer1::RNNDirection::kBIDIRECTION ? 2 : 1) *
        layerCount;
    const int32_t K =
        direction == nvinfer1::RNNDirection::kUNIDIRECTION ? 1 : 2;
    std::vector<nvinfer1::RNNGateType> gates;
    switch (op) {
    case nvinfer1::RNNOperation::kRELU:
    case nvinfer1::RNNOperation::kTANH:
        gates =
            std::vector<nvinfer1::RNNGateType>({nvinfer1::RNNGateType::kINPUT});
        break;
    ...
    case nvinfer1::RNNOperation::kGRU:
        gates = std::vector<nvinfer1::RNNGateType>(
            {nvinfer1::RNNGateType::kUPDATE, nvinfer1::RNNGateType::kRESET,
             nvinfer1::RNNGateType::kHIDDEN});
        break;
    }

    RNNWeightsAdder weightsAdder = [&layer](int32_t n,
                                            nvinfer1::RNNGateType gate,
                                            nvinfer1::Weights weights) {
        layer->setWeightsForGate(n, gate, true, weights);
    };
    RNNWeightsAdder recurrentWeightsAdder =
        [&layer](int32_t n, nvinfer1::RNNGateType gate,
                 nvinfer1::Weights weights) {
            layer->setWeightsForGate(n, gate, false, weights);
        };
    RNNWeightsAdder biasAdder = [&layer](int32_t n, nvinfer1::RNNGateType gate,
                                         nvinfer1::Weights weights) {
        layer->setBiasForGate(n, gate, true, weights);
    };
    RNNWeightsAdder recurrentBiasAdder = [&layer](int32_t n,
                                                  nvinfer1::RNNGateType gate,
                                                  nvinfer1::Weights weights) {
    ...
    for (int32_t n = 0; n < nbWeights; ++n) {
        if (n >= K || inputMode == nvinfer1::RNNInputMode::kLINEAR) {
            ASSERT(addRNNv2Weights(weightsAdder, n, gates, inputs, counter) &&
                       "Failed to add weights to the RNN layer.",
                   nvonnxparser::ErrorCode::kINVALID_NODE);
        }
        ASSERT(
            addRNNv2Weights(recurrentWeightsAdder, n, gates, inputs, counter) &&
                "Failed to add recurrent weights to the RNN layer.",
            nvonnxparser::ErrorCode::kINVALID_NODE);
        ASSERT(addRNNv2Weights(biasAdder, n, gates, inputs, counter) &&
                   "Failed to add bias to the RNN layer.",
               nvonnxparser::ErrorCode::kINVALID_NODE);
...
DEFINE_BUILTIN_OP_IMPORTER(TRT_RaggedSoftmax) {
    ...
    nvinfer1::IRaggedSoftMaxLayer *layer =
        ctx->network()->addRaggedSoftMax(input, bounds);
    ctx->registerLayer(layer, node);
    RETURN_FIRST_OUTPUT(layer);
}
...
DEFINE_BUILTIN_OP_IMPORTER(TRT_FullyConnected) {
    ...
    ASSERT(inputs.at(1).is_weights() &&
               "The input kernel must be an initializer.",
           nvonnxparser::ErrorCode::kINVALID_NODE);
    auto &kernelWeights = inputs.at(1).weights();
...
DEFINE_BUILTIN_OP_IMPORTER(TRT_MaxAverageBlendPool) {
    ...
    nvinfer1::IPoolingLayer *layer = ctx->network()->addPoolingNd(
        input, nvinfer1::PoolingType::kMAX_AVERAGE_BLEND, kernelSize);
    ASSERT(layer && "Failed to create a Pooling layer.",
           ErrorCode::kUNSUPPORTED_NODE);
    ctx->registerLayer(layer, node);
    layer->setStrideNd(strides);
...
DEFINE_BUILTIN_OP_IMPORTER(TRT_PluginV2) {
    std::vector<nvinfer1::ITensor *> tensors;
    for (auto &input : inputs) {
        ASSERT(input.is_tensor() && "The input must be a tensor.",
               nvonnxparser::ErrorCode::kUNSUPPORTED_NODE);
    ...
    nvinfer1::IPluginCreator *creator = registry->getPluginCreator(
        name.c_str(), version.c_str(), nspace.c_str());
    ASSERT(creator && "Plugin not found, are the plugin name, version, and "
                      "namespace correct?",
           nvonnxparser::ErrorCode::kINVALID_NODE);

    auto const plugin =
        creator->deserializePlugin("", buffer.data(), buffer.size());

    nvinfer1::IPluginV2Layer *layer =
        ctx->network()->addPluginV2(tensors.data(), tensors.size(), *plugin);
    ctx->registerLayer(layer, node);
    RETURN_ALL_OUTPUTS(layer);
}
...
DEFINE_BUILTIN_OP_IMPORTER(TRT_Gather) {
    ...
    ASSERT((indices.getType() == DataType::kINT32) &&
           ...
           nvonnxparser::ErrorCode::kINVALID_NODE);
    ASSERT((r >= 1) && "0D input data is not allowed.",
           nvonnxparser::ErrorCode::kINVALID_NODE);
    ASSERT((-r <= axis && axis < r) &&
               "The attribute axis should be in range [-r, r-1], where r is "
    ...
    nvinfer1::IGatherLayer *layer =
        ctx->network()->addGather(data, indices, axis);
    ctx->registerLayer(layer, node);
    layer->setNbElementWiseDims(nbElementWiseDims);
    RETURN_FIRST_OUTPUT(layer);
}
...
DEFINE_BUILTIN_OP_IMPORTER(TRT_Slice) {
    ...
    if (inputs.size() == 1) {
        ...
        ASSERT((inputs.size() == 4) &&
                   "Exactly 4 inputs are required by TRT_Slice.",
               nvonnxparser::ErrorCode::kUNSUPPORTED_NODE);
...
DEFINE_BUILTIN_OP_IMPORTER(TRT_Resize) {
    ...
    auto const transformation =
        attrs.get<nvinfer1::ResizeCoordinateTransformation>("coordTransform");
    ...
    if (inputs.size() == 1) {
        auto outputDims =
            attrs.get<nvinfer1::Dims>("output_dims", nvinfer1::Dims{-1, {}});
        if (outputDims.nbDims > 0) {
            layer->setOutputDimensions(outputDims);
        } else {
            // TRT-15340: Adapt to use resizeShapeTensor instead when safety
            // support nbDims == 1.
            auto scales = attrs.get<std::vector<float>>("scales");
            ASSERT((scales.size() > 0) && "Attribute scales is missing.",
                   nvonnxparser::ErrorCode::kINVALID_NODE);
            layer->setScales(&scales[0], scales.size());
        }
    } else {
        ASSERT((inputs.at(1).is_tensor()) &&
                   "The output dimension input must be a tensor.",
               nvonnxparser::ErrorCode::kUNSUPPORTED_NODE);
        layer->setInput(1, inputs.at(1).tensor());
    }
    RETURN_FIRST_OUTPUT(layer);
...
DEFINE_BUILTIN_OP_IMPORTER(TRT_Conv) { return importConv(ctx, node, inputs); }
