...
#define DECLARE_BUILTIN_OP_IMPORTER(op)                                             \
    NodeImportResult import##op(IImporterContext *ctx, onnx::NodeProto const &node, \
...
NodeImportResult batchnormFallback(IImporterContext *ctx, onnx::NodeProto const &node,
                                   std::vector<TensorOrWeights> &inputs) {
    ...
    nvinfer1::ITensor *epsilon = addConstantScalar(ctx, eps, onnx::TensorProto::FLOAT, scalarShape)->getOutput(0);
    ...
    nvinfer1::IElementWiseLayer *layer = ctx->network()->addElementWise(
        *ctx->network()
             ->addElementWise(*scale, *ctx->network()->addElementWise(*dividend, *divisor, eOp::kDIV)->getOutput(0),
                              eOp::kPROD)
             ->getOutput(0),
        *bias, eOp::kSUM);
...
DEFINE_BUILTIN_OP_IMPORTER(BatchNormalization) {
    ASSERT((inputs.at(1).shape().nbDims == 1) && "The shape of the scale input must be (C, )",
    ...
    ASSERT(!isTraining &&
               "This version of TensorRT does not support "
...
DEFINE_BUILTIN_OP_IMPORTER(Cast) {
    ...
    ASSERT(convertDtype(onnxType, &newType) && "Unsupported data type for the Cast operator!",
           ErrorCode::kINVALID_NODE);
...
DEFINE_BUILTIN_OP_IMPORTER(ConstantOfShape) {
    ...
    ShapedWeights zeroWeights = ctx->createTempWeights(onnx::TensorProto_DataType_FLOAT, nvinfer1::Dims{1, 1});
...
DEFINE_BUILTIN_OP_IMPORTER(Conv) {
    ...
    if (inputs.size() == 3) {
        assertIsWeights(inputs.at(2),
                        "The bias tensor is required to be an "
        ...
        ASSERT((shapedBiasWeights.shape.nbDims == 1) && "The bias tensor is required to be 1D.",
               ErrorCode::kINVALID_NODE);
...
DEFINE_BUILTIN_OP_IMPORTER(ConvTranspose) {
    ...
    auto *layer = ctx->network()->addDeconvolutionNd(*tensorPtr, noutput, kernelSize, kernelWeights,
...
DEFINE_BUILTIN_OP_IMPORTER(DepthToSpace) {
    ...
    ASSERT((inputs.at(0).shape().nbDims == 4) && "The input tensor must be in NCHW format.",
           ErrorCode::kUNSUPPORTED_NODE);
    nvinfer1::ITensor *tensorPtr = &convertToTensor(inputs.at(0), ctx);
...
NodeImportResult QuantDequantLinearHelper(IImporterContext *ctx, onnx::NodeProto const &node,
                                          std::vector<TensorOrWeights> &inputs, bool isDQ, DataType datatype) {
    CHECK(notInvalidType(inputs.at(0), {"UINT8"}));
    ...
    auto addConstantLayer = [ctx, node](nvinfer1::INetworkDefinition &network,
    ...
    if (scaleSize != 1) {
        ...
        ASSERT(K == scaleSize &&
                   "The number of scales is not equal to the "
...
DEFINE_BUILTIN_OP_IMPORTER(Dropout) {
    ...
    if (ctx->getOpsetVersion() <= 6) {
        ...
        ASSERT(isTestingMode &&
                   "TensorRT does not support the Droupout "
        ...
        ASSERT(!trainingMode[0] &&
                   "TensorRT does not support the Dropout "
...
DEFINE_BUILTIN_OP_IMPORTER(Gemm) {
    ...
    // Scale A*B if needed.
    if (alpha != 1.f) {
        nvinfer1::IConstantLayer *alphaConstant = addConstantScalar(ctx, alpha, onnx::TensorProto_DataType_FLOAT);
    ...
    if (inputs.size() > 2) {
        ...
        // Scale C if needed
        if (beta != 1.f) {
            nvinfer1::IConstantLayer *betaConstant = addConstantScalar(ctx, beta, onnx::TensorProto_DataType_FLOAT);
...
DEFINE_BUILTIN_OP_IMPORTER(GlobalAveragePool) {
    LOG_VERBOSE(
        "GlobalAveragePool operators are implemented via Reduce layers "
...
DEFINE_BUILTIN_OP_IMPORTER(GlobalMaxPool) {
    LOG_VERBOSE(
        "GlobalMaxPool operators are implemented via Reduce layers "
        "rather than Pooling layers");
...
nvinfer1::ITensor *concatenateRNNOutputs(IImporterContext *ctx, onnx::NodeProto const &node, nvinfer1::ILoop *loop,
    ...
    if (numDirections == 2) {
        ...
        if (inputs.size() > 4 && inputs.at(4)) {
            ...
            backwardHt = clearMissingSequenceElements(ctx, node, loop, seqLens, backwardHt, sequenceLength,
                                                      /*reverse=*/true);
        }
...
DEFINE_BUILTIN_OP_IMPORTER(GRU) {
    ...
    const float clip = attrs.get("clip", -1.f);  // Clipping cannot be negative, so -1.0 is a
                                                 // good sentinel value.
    ...
    if (numDirections == 2) {
        ASSERT(std::equal(activations.begin(), activations.begin() + NUM_ACTIVATIONS,
                   ...
                   "the forward pass.",
               ErrorCode::kUNSUPPORTED_NODE);
        ASSERT(std::equal(activationAlphas.begin(), activationAlphas.begin() + NUM_ACTIVATIONS,
                          activationAlphas.begin() + NUM_ACTIVATIONS) &&
                   "The parser does not currently support cases where "
                   "activations for the reverse pass of the GRU do not match "
                   "the forward pass.",
               ErrorCode::kUNSUPPORTED_NODE);
        ASSERT(std::equal(activationBetas.begin(), activationBetas.begin() + NUM_ACTIVATIONS,
    ...
    nvinfer1::ITensor *weightsZRStart =
        addConstant(ctx, std::vector<int32_t>{0, 0, 0}, onnx::TensorProto::INT32, Dims{1, 3})->getOutput(0);
    ...
    nvinfer1::ITensor *recurrenceWeightsZR =
        net->addSlice(recurrenceWeights, Dims3{0, 0, 0}, Dims3{numDirections, 2 * hiddenSize, hiddenSize},
                      Dims3{1, 1, 1})
            ->getOutput(0);
    ...
    auto const initialStateShape = [&ctx, &numDirections, &hiddenSize, &input, &net]() -> nvinfer1::ITensor * {
        ...
        nvinfer1::ITensor *numDirectionsTensor =
            addConstantScalar(ctx, numDirections, onnx::TensorProto_DataType_INT32, Dims{1, 1})->getOutput(0);
        LOG_VERBOSE("numDirections is: " << numDirections
                                         << ", numDirections Tensor shape: " << numDirectionsTensor->getDimensions());
        nvinfer1::ITensor *hiddenSizeTensor =
            addConstantScalar(ctx, hiddenSize, onnx::TensorProto_DataType_INT32, Dims{1, 1})->getOutput(0);
    ...
    auto const isolateGate = [&ctx, &hiddenSize, &gateOutputShape, &net](nvinfer1::ITensor *gates,
        ...
        isolateGate->setInput(1, *addConstant(ctx, std::vector<int32_t>{0, 0, gateIndex * hiddenSize},
    ...
    if (linearBeforeReset == 0) {
        ...
        nvinfer1::ITensor *ht1Rh =
            net->addMatrixMultiply(*Ht1->getOutput(0), mOp::kNONE, *recurrenceWeightsH, mOp::kTRANSPOSE)->getOutput(0);
    ...
    nvinfer1::ITensor *Ht =
        net->addElementWise(
               *net->addElementWise(
                       *net->addElementWise(
                               *addConstantScalar(ctx, 1.f, onnx::TensorProto::FLOAT, Dims3{1, 1, 1})->getOutput(0),
                               *zt, eOp::kSUB)
                            ->getOutput(0),
                       *ht, eOp::kPROD)
                    ->getOutput(0),
...
DEFINE_BUILTIN_OP_IMPORTER(ImageScaler) {
    ...
    std::fill(static_cast<float *>(scaleWeights.values),
              static_cast<float *>(scaleWeights.values) + scaleWeights.count(), scale);
    // Finally add the scale layer.
    auto layer = ctx->network()->addScale(tensor, nvinfer1::ScaleMode::kCHANNEL, shiftWeights, scaleWeights,
                                          nvinfer1::Weights{});
    ctx->registerLayer(layer, node);
    RETURN_FIRST_OUTPUT(layer);
}
...
DEFINE_BUILTIN_OP_IMPORTER(LayerNormalization) {
    ...
    auto biasType =
        input->getType() == nvinfer1::DataType::kFLOAT ? onnx::TensorProto::FLOAT : onnx::TensorProto::FLOAT16;
    auto *bias =
...
DEFINE_BUILTIN_OP_IMPORTER(Loop) {
    ...
    constexpr int32_t NB_DISCARDED_OUTPUTS = 1;  // First output is the updated value of the condition, and is
    ...
    if (inputs[0]) {
        ...
        bool const isMaxTripCount = inputs[0].is_weights() && static_cast<int32_t *>(inputs[0].weights().values)[0] ==
    ...
    for (int32_t i = nbStateVars + NB_DISCARDED_OUTPUTS; i < nbOutputs; ++i) {
        ...
        if (tripLimit) {
            ...
            trtScanOut->setInput(
                1, *addConstantScalar(ctx, DUMMY_SCAN_OUTPUT_LENGTH, onnx::TensorProto_DataType_INT32)->getOutput(0));
        }
...
DEFINE_BUILTIN_OP_IMPORTER(LSTM) {
    ...
    float const clip = attrs.get("clip", -1.f);  // Clipping cannot be negative, so -1.0 is a
                                                 // good sentinel value.

    ASSERT(inputForget == 0 && "Coupled input/forget is unsupported in the LSTM converter",
           ErrorCode::kUNSUPPORTED_NODE);
    ...
    auto const initialStateShape = [&ctx, &numDirections, &hiddenSize, &input]() -> nvinfer1::ITensor * {
        // Get batchSize from input shape
        nvinfer1::ITensor *numDirectionsTensor =
            addConstantScalar(ctx, numDirections, onnx::TensorProto_DataType_INT32, nvinfer1::Dims{1, 1})->getOutput(0);
        LOG_VERBOSE("numDirectionsTensor shape: " << numDirectionsTensor->getDimensions());
        nvinfer1::ITensor *hiddenSizeTensor =
            addConstantScalar(ctx, hiddenSize, onnx::TensorProto_DataType_INT32, nvinfer1::Dims{1, 1})->getOutput(0);
    ...
    auto const getInitialInputValue = [&ctx, &gateOutputShape, &inputs, &node](size_t inputIdx) -> nvinfer1::ITensor * {
        ...
        return constantOfShape(
            ctx, node,
            addConstantScalar(ctx, 0.f, onnx::TensorProto_DataType_FLOAT, nvinfer1::Dims{1, 1})->getOutput(0),
            gateOutputShape);
    };
    ...
    auto const isolateGate = [&ctx, &hiddenSize, &gateOutputShape](nvinfer1::ITensor *gates,
                                                                   int32_t gateIndex) -> nvinfer1::ITensor * {
        nvinfer1::ISliceLayer *isolate = ctx->network()->addSlice(*gates, nvinfer1::Dims3{0, 0, 0},
                                                                  nvinfer1::Dims3{0, 0, 0}, nvinfer1::Dims3{1, 1, 1});
        isolate->setInput(1, *addConstant(ctx, std::vector<int32_t>{0, 0, gateIndex * hiddenSize},
                                          onnx::TensorProto_DataType_INT32, nvinfer1::Dims{1, 3})
                                  ->getOutput(0));  // Start
    ...
    auto const addPeephole = [&ctx, &node, &hiddenSize, &numDirections, &peephole](
                                 nvinfer1::ITensor *gate, nvinfer1::ITensor *cellState,
                                 int32_t gateIndex) -> nvinfer1::ITensor * {
        ...
        return ctx->network()
            ->addElementWise(*gate,
                             *ctx->network()->addElementWise(*peepholeWeights, *cellState, eOp::kPROD)->getOutput(0),
                             eOp::kSUM)
            ->getOutput(0);
    };
...
DEFINE_BUILTIN_OP_IMPORTER(NonMaxSuppression) {
    ...
    if (inputs.size() >= 3 && !inputs.at(2).isNullTensor()) {
        ...
        ASSERT(constantLayer != nullptr &&
                   "Failed to add in constant for "
    ...
    switch (centerPointBox) {
        ...
        case 1:
            ...
            break;
        default:
            ASSERT(0 && "Invalid value provided for the center_point_box attribute", ErrorCode::kUNSUPPORTED_NODE);
    }
...
DEFINE_BUILTIN_OP_IMPORTER(Pad) {
    ...
    if (onnxPadding.empty()) {
        ...
        auto pre =
            ctx->network()
                ->addSlice(*onnxPaddingPtr, nvinfer1::Dims{1, {0}}, nvinfer1::Dims{1, {nbDims}}, nvinfer1::Dims{1, {1}})
                ->getOutput(0);
    ...
    if (mode == "constant") {
        ...
        if (valuePtr) {
            ...
            switch (tensorPtr->getType()) {
                ...
                case DataType::kINT8:
                    fillValue =
                        addConstant(ctx, std::vector<float>{value}, onnx::TensorProto::FLOAT, nvinfer1::Dims{0, {0}})
                            ->getOutput(0);
                    break;
                default:
...
NodeImportResult randomUniformHelper(IImporterContext *ctx, onnx::NodeProto const &node, ShapeTensor const &inputShape,
                                     OnnxAttrs const &attrs, DataType const &inputDType) {
    ...
    if (attrs.count("seed")) {
        LOG_WARNING(
            "TensorRT currently ignores the \"seed\" field in "
...
DEFINE_BUILTIN_OP_IMPORTER(RandomUniformLike) {
    ASSERT((inputs.size() == 1) && "The RandomUniformLike operator requires exactly 1 input.",
...
NodeImportResult randomNormalHelper(IImporterContext *ctx, onnx::NodeProto const &node, ShapeTensor const &inputShape,
                                    OnnxAttrs const &attrs, DataType const &inputDType) {
...
DEFINE_BUILTIN_OP_IMPORTER(RandomNormalLike) {
    ASSERT((inputs.size() == 1) && "The RandomNormalLike operator requires exactly 1 input.",
           ErrorCode::kINTERNAL_ERROR);
    ASSERT((inputs.at(0).is_tensor()) && "The input tensor cannot be an initializer.",
           nvonnxparser::ErrorCode::kUNSUPPORTED_NODE);
    auto &input = inputs.at(0).tensor();
    ...
    if (ctx->getOpsetVersion() >= 5) {
        ...
        if (attrs.count("allowzero")) {
            ...
            if (ctx->getOpsetVersion() < 14) {
                LOG_WARNING(getNodeName(node) << ": Using attribute allowzero with opset < 14 is "
...
DEFINE_BUILTIN_OP_IMPORTER(ReverseSequence) {
    ASSERT((inputs.size() == 2) &&
               "ReverseSequence expects two input tensors: "
    ...
    ASSERT((sequenceAxis >= 0 && sequenceAxis <= inputDims.nbDims) && "Invalid time_axis",
           ErrorCode::kUNSUPPORTED_NODE);
...
DEFINE_BUILTIN_OP_IMPORTER(RNN) {
    ...
    auto const getInitialInputValue = [&ctx, &initialStateShape, &inputs,
    ...
    if (inputs.size() > 4 && inputs.at(4)) {
        ...
        Ht = numDirections == 2
                 ...
                 : maskRNNHidden(ctx, node, loop, seqLens, hiddenState->getOutput(0), Ht, maxLen,
                                 direction == "reverse");
    }
...
DEFINE_BUILTIN_OP_IMPORTER(RoiAlign) {
    ...
    f.emplace_back("coordinate_transformation_mode", &coordinateTransformationMode, nvinfer1::PluginFieldType::kINT32,
                   1);
...
DEFINE_BUILTIN_OP_IMPORTER(TopK) {
    ...
    if (needCast) {
        LOG_WARNING(
            "TensorRT is using FLOAT32 precision to run an INT32 TopK. "
...
template <typename T>
std::tuple<void const *, size_t> copyField(T const &field, std::string const &fieldName,
...
template <typename T>
std::tuple<void const *, size_t> copyField(std::vector<T> const &repeatedField, std::string const &fieldName,
                                           string_map<std::vector<uint8_t>> &fieldData) {
...
std::tuple<void const *, size_t> copyField(std::string const &field, std::string const &fieldName,
                                           string_map<std::vector<uint8_t>> &fieldData) {
...
std::tuple<void const *, size_t> copyField(ShapedWeights const &field, std::string const &fieldName,
                                           string_map<std::vector<uint8_t>> &fieldData) {
    ...
    for (int32_t i = 0; i < fieldNames->nbFields; ++i) {
        ...
        switch (attrs.type(fieldName)) {
            ...
            case onnx::AttributeProto::STRINGS:
                std::tie(data, length) =
                    copyField(attrs.get<std::vector<std::string>>(fieldName), fieldName, fieldData);
                type = nvinfer1::PluginFieldType::kCHAR;
                break;
            ...
            case onnx::AttributeProto::TYPE_PROTOS:
                MAKE_ERROR("Attributes of type: " + onnx::AttributeProto::AttributeType_Name(attrs.type(fieldName)) +
                               " are unsupported",
                           ErrorCode::kUNSUPPORTED_NODE);
        }
...
DEFINE_BUILTIN_OP_IMPORTER(NonZero) {
    nvinfer1::ITensor *x = &convertToTensor(inputs.at(0), ctx);
    ASSERT((x->getType() == DataType::kFLOAT || x->getType() == DataType::kHALF || x->getType() == DataType::kINT32 ||
            x->getType() == DataType::kINT8 || x->getType() == DataType::kBOOL) &&
...
DEFINE_BUILTIN_OP_IMPORTER(FallbackPluginImporter) {
    ...
    ASSERT(creator &&
               "Plugin not found, are the plugin name, version, and "
               "namespace correct?",
           ErrorCode::kUNSUPPORTED_NODE);
